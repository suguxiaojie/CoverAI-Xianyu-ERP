...
```bash
cd /path/to/CoverAI-Xianyu-ERP/frontend
npm install
npm run build
```
...
## Mandatory Roadmap governance — DO NOT SKIP

`ROADMAP.md` is the authoritative dynamic project-status source. Before changing code or project documentation,
agents MUST read its current phase, blockers, pending confirmations, next steps and latest verification. The
architecture master plan remains the only source for the six formal refactoring phases; Roadmap maintenance must
not create, reopen or reinterpret a formal phase.

Every completed development task, fix, project-document update or material investigation that changes project
state MUST update `ROADMAP.md` in the same task. Pure read-only queries and temporary commands do not update it.
Only implemented and verified work may be marked complete; incomplete verification is `待验证`, and uncertain
facts or missing authorization are `待确认`.

`docs/architecture/refactoring-progress.md` and `ROADMAP.md` have different mandatory roles and MUST be updated
together when both apply:

- `refactoring-progress.md` stores detailed implementation scope, raw verification evidence, environment
  exceptions and external-write boundaries;
- `ROADMAP.md` stores the concise current state, completed and in-progress work, blockers, pending confirmations
  and executable next steps;
- the single `验收记录同步至` value in `ROADMAP.md` MUST exactly match the latest `###` maintenance heading in
  `refactoring-progress.md`.

Run `make roadmap` before declaring a state-changing task complete. The check is fail-closed when either document
is missing, required Roadmap sections are missing, the synchronization marker is missing or duplicated, or the
marker is older than the latest detailed progress entry. Run `make roadmap-test` after changing the checker.
Do not bypass a failure by deleting progress evidence, weakening the checker or copying a heading without updating
the Roadmap's actual status.
...
- macOS registers `com.ydisks.xianyu-helper.server` and `com.ydisks.xianyu-helper.tray` LaunchAgents.
  The app is `/Applications/CoverAI闲鱼助手/CoverAI闲鱼助手.app`. Its Swift AppKit shell owns a standard
  Dock window containing a persistent `WKWebView`, while login startup passes `--background` so only the menu bar
  controller starts until the user explicitly opens the window. Normal management must not launch Chrome or Safari;
  explicit external links and the frozen manual CAPTCHA Chromium flow remain separate.
...
The tray behavior contract is shared by Windows and macOS even though Windows uses the Go tray and macOS uses the
native Swift window shell: it serializes actions, shows transition states,
...
packaging/macos/build-native-app.sh 0.0.0-local "$PWD/dist/macos" arm64
packaging/macos/build-pkg.sh 0.0.0-local "$PWD/dist/macos" arm64
packaging/macos/build-dmg.sh 0.0.0-local "$PWD/dist/macos" arm64
```
...
```bash
runtime_app="$PWD/dist/macos/pkgroot-arm64/Applications/CoverAI闲鱼助手/CoverAI闲鱼助手.app"
