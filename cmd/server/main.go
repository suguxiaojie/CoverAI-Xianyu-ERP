...
type serverInfrastructure struct {
	...
	// cardImageRoot 是受管卡密图片目录，跟随桌面数据目录或本地相对数据布局。
	cardImageRoot string
}
...
func main() {
	...
	// ctx 只接收明确停服信号取消；cancel 在主进程退出时解除信号订阅。
...
func openServerInfrastructure(ctx context.Context, startup serverStartupConfig, opts serverOptions) (serverInfrastructure, error) {
	...
	if opts.initAdmin {
		...
		return serverInfrastructure{database: database, store: store, logger: logger, logWriter: logWriter, closeLog: closeLog, initializationOnly: true, cardImageRoot: resolveCardImageRoot(startup.dataDir)}, nil
	}
	if opts.ensureAdmin {
	...
	return serverInfrastructure{database: database, store: store, logger: logger, logWriter: logWriter, closeLog: closeLog, cardImageRoot: resolveCardImageRoot(startup.dataDir)}, nil
}
...
func buildServerRuntime(opts serverOptions, infrastructure serverInfrastructure) (serverRuntime, error) {
	...
	}, compositionruntime.RuntimeInfrastructure{Store: infrastructure.store, Logger: infrastructure.logger, CardImageRoot: infrastructure.cardImageRoot})
	if buildErr != nil {
...
// resolveCardImageRoot 把应用数据目录转换为独立上传目录；空数据目录沿用源码和 Linux 的 data 布局。
func resolveCardImageRoot(dataDir string) string {
	if strings.TrimSpace(dataDir) == "" {
		return filepath.Join("data", "uploads", "card-images")
	}
	return filepath.Join(filepath.Clean(dataDir), "uploads", "card-images")
}
