...
// TestResolveCardImageRootFollowsApplicationDataDir 验证受管卡密图片不会落到数据库数据目录之外的随机工作路径。
func TestResolveCardImageRootFollowsApplicationDataDir(t *testing.T) {
	// dataDir 是桌面或显式 workdir 解析后的应用数据根目录。
	dataDir := filepath.Join(t.TempDir(), "CoverAIData")
	// got 是显式数据目录对应的卡密图片目录。
	got := resolveCardImageRoot(dataDir)
	// want 是图片资产与数据库并列保存时的稳定目标路径。
	want := filepath.Join(dataDir, "uploads", "card-images")
	if got != want {
		t.Fatalf("卡密图片目录异常 got=%q want=%q", got, want)
	}
	// relative 是 Linux 和源码运行未提供数据目录时保持的项目相对布局。
	relative := resolveCardImageRoot("")
	if relative != filepath.Join("data", "uploads", "card-images") {
		t.Fatalf("相对卡密图片目录异常: %q", relative)
	}
}
