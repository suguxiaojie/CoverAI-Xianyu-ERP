...
账号设置中的 `show_browser` 同时是 Token 风控模式选择：开启时进入人工接管路径，关闭时才使用本文既有的自动主／备用引擎。人工接管路径的冻结契约见第 6.1 节。
...
适配层持久化时必须以滑块引擎已经返回的“原 Cookie + 新 `x5*`”结果为扁平 Cookie 基线。持久化 Profile 的退出后快照只负责补充或替换 `x5*` Cookie 的精确 Domain、Path、Expires、HTTPOnly、Secure、SameSite 与 PartitionKey 属性；不得以 Profile 的扁平 Cookie 整体覆盖引擎结果，也不得因 Profile 快照缺项而删除 `unb`、长期登录字段或其他非 `x5*` Cookie。Profile 中的非 `x5*` 值即使不同，也不得覆盖本轮引擎结果。
...
账号 `show_browser=true` 时，Token 风控固定执行人工接管路径：
...
打开独立可见 Chromium 上下文
  ...
  → 不产生任何鼠标、指针、键盘或点击事件
  → 最多等待用户 3 分钟
  → 检测到全新 x5sec 且页面离开 punish/captcha 后返回
```

人工模式约束如下：

- 不调用 Playwright 主滑块引擎，不调用远程滑块服务，也不调用直接 Chromium/CDP 备用引擎。
- 浏览器必须有头显示；测试可以在本地夹具中使用无头模式验证相同的等待和 Cookie 语义。
- 人工浏览器使用独立非持久化上下文，不占用账号持久化 Profile 锁、凭证锁或账号续期互斥锁等待用户。
- 账号停止、服务关闭或调用 Context 取消时必须立即结束等待并同步关闭页面、上下文和浏览器。
- 用户关闭窗口时立即失败，不得自动重开、自动拖动或切换备用引擎。
- 等待上限固定为 3 分钟，轮询间隔固定为 `250ms`；超时后释放全部浏览器资源。
- 成功条件与第 4 节一致：必须出现相对验证前全新的非空 `x5sec`，且页面已经离开 punish/captcha 地址。
- 人工上下文返回的 Cookie 只把新 x5 系列值合并进原 Cookie 字符串；适配层不得再用旧持久化 Profile 快照覆盖人工结果。
- 验证地址过期时，必须先关闭本轮人工浏览器，再按既有上限重取地址；不得持有浏览器或 Profile 锁调用 provider。
...
- 人工模式必须记录“等待人工验证”“人工验证成功”或明确的超时、取消、窗口关闭错误；日志不得包含 Cookie 值或验证地址查询参数。
...
- 人工路径不得产生鼠标、指针、键盘或点击事件；本地真实 Chromium 夹具签发新 `x5sec` 并离开验证地址后，必须成功返回人工结果。
- 人工等待必须覆盖严格成功、停留 punish 地址不误判、三分钟超时、Context 取消、窗口关闭，以及人工模式禁止主／备用／远程自动引擎。
...
用户观察到 `show_browser=true` 后项目仍会自动拖动，并明确要求以后全部由用户手动完成 Token 风控验证。第 6.1、8、9 节据此加入账号级人工接管契约：开启“显示浏览器并手动验证”时，只打开独立可见 Chromium 并等待用户最多 3 分钟，禁止远程服务、Playwright 主引擎和直接 Chromium/CDP 备用引擎产生任何自动输入；关闭开关时保留原自动模式。严格成功条件继续要求新 `x5sec` 与离开验证地址，账号／服务取消必须同步释放浏览器资源。

### 2026-08-19 自动 Profile Cookie 协调授权变更

用户确认修复自动滑块成功后持久化 Cookie 缺少 `unb`、导致 Token 刷新无法生成 deviceId 的问题。根因是适配层把滑块引擎已经正确合并的 Cookie 又替换为退出后 Profile 的扁平 Cookie；Profile 只要缺少身份字段，写回数据库时就会丢失账号身份。第 4 节据此固定为：引擎结果是扁平 Cookie 基线，Profile 只补充 `x5*` 的精确浏览器属性，任何非 `x5*` Profile 值都不得覆盖或删除当前账号凭证。滑块选择器、距离、轨迹、输入事件、重试和严格成功条件均保持不变。
