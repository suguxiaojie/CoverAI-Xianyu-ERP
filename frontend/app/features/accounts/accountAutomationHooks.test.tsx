...
import type { AccountDetail,AccountTaskSettingsResponse } from '../../../shared/api-contract/accounts';
...
const settingsFixture: AccountTaskSettingsResponse = {
  account_id: 'account-1', auto_rate_enabled: true, rate_content: '已保存文案', auto_polish_enabled: true, polish_time: '04:00',
  ...
  last_rate_scan_at: 0, last_polish_date: '', last_polish_at: 0,
};

// onSavedFixture 是保存或执行成功后的页面同步回调替身。
const onSavedFixture = vi.fn();
// onSaveSucceededFixture 是仅在设置保存成功时触发的弹窗收口回调替身。
const onSaveSucceededFixture = vi.fn();
// onSaveFailedFixture 是仅在设置保存失败时触发的页面气泡回调替身。
const onSaveFailedFixture = vi.fn();

// renderAutomationHook 是渲染账号任务 Hook 的具名回调。
const renderAutomationHook = () => useAccountAutomation({ account: accountFixture, onSaved: onSavedFixture, onSaveSucceeded: onSaveSucceededFixture, onSaveFailed: onSaveFailedFixture });
...
describe('useAccountAutomation', /* 当前回调处理账号任务 Hook 的请求与动作状态。 */ () => {
  ...
  test('加载设置后可以保存和立即执行任务', /* 当前回调验证成功加载、保存和执行路径。 */ async () => {
    ...
    expect(onSaveSucceededFixture).toHaveBeenCalledTimes(1);
    expect(hook.result.current.saved).toBe(true);

    act(
      // editAction 模拟用户在保存后再次修改秒级延迟。
      () => hook.result.current.setForm(/* currentSettings 更新保存后的任务草稿。 */ currentSettings => ({ ...currentSettings, request_flower_after_seconds: 12 })),
    );
  ...
  test('保存失败后可通过 retry 重试并清除错误', /* 当前回调验证失败状态和安全重试路径。 */ async () => {
    ...
    expect(onSaveFailedFixture).toHaveBeenCalledWith('保存失败');
