...
test('账号任务默认设置继承账号配置并保持安全文案',
  ...
  () => {
    expect(buildAccountTaskDefaults(accountFixture)).toMatchObject({
      account_id: 'a1', auto_rate_enabled: true, rate_content: '交易愉快', polish_time: '03:00',
      auto_request_flower_enabled: false, request_flower_after_hours: 24, request_flower_after_seconds: 10,
      auto_receive_flower_enabled: false, receive_flower_show_browser: true, receive_flower_timeout_seconds: 120,
	  auto_receipt_reminder_enabled: false, receipt_reminder_after_days: 2, receipt_reminder_time: '10:00',
    });
