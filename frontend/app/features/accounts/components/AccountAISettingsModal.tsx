...
export const AccountAISettingsModal: React.FC<AccountAISettingsModalProps> = ({ account, settings, saving, onChange, onClose, onSave }) => {
  ...
  const handlePromptChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => updateSettings({ custom_prompts: event.target.value });
	...
	// 当前 effect 在 AI 设置弹窗打开期间接管 Escape，关闭后释放监听器。
	useEffect(/* closeAISettingsOnEscape 把 Escape 映射到现有 AI 弹窗关闭动作。 */ () => {
		// handleEscape 是 AI 弹窗唯一的文档级键盘监听器，其他按键保持原行为。
		...
		return /* removeAIEscapeListener 弹窗卸载或 onClose 变化时释放旧监听器。 */ () => document.removeEventListener('keydown', handleEscape);
	}, [onClose]);
