...
export const AccountEditModal: React.FC<AccountEditModalProps> = ({
  ...
  onSettingsDirty,
	...
	// autoConfirmSectionRef 指向自动确认发货设置，供控制中心卡片打开后定位。
	const autoConfirmSectionRef = useRef<HTMLDivElement | null>(null);

	// 当前 effect 只在自动确认卡片入口打开弹窗时滚动 modal-body，普通编辑入口仍从顶部开始。
	useEffect(/* focusAccountEditSection 定位账号编辑弹窗中的目标基础设置。 */ () => {
		if (initialFocus !== 'auto_confirm' || !autoConfirmSectionRef.current) return undefined;
		// target 是完成 Portal 挂载后需要滚入视口的自动确认设置。
		const target = autoConfirmSectionRef.current;
		// frameID 延迟到布局完成后执行，避免 scrollIntoView 作用于尚未渲染的节点。
		const frameID = window.requestAnimationFrame(/* scrollAfterEditLayout 在编辑弹窗布局稳定后滚动自动确认区块。 */ () => target.scrollIntoView({ behavior: 'smooth', block: 'start' }));
		return /* cancelEditScroll 在入口变化或弹窗卸载时取消尚未执行的定位。 */ () => window.cancelAnimationFrame(frameID);
	}, [initialFocus]);

	// 当前 effect 在账号编辑弹窗打开期间接管 Escape，关闭后释放监听器。
	useEffect(/* closeAccountEditOnEscape 把 Escape 映射到现有编辑弹窗关闭动作。 */ () => {
		// handleEscape 是编辑弹窗唯一的文档级键盘监听器，其他按键保持原行为。
		...
		return /* removeEditEscapeListener 弹窗卸载或 onClose 变化时释放旧监听器。 */ () => document.removeEventListener('keydown', handleEscape);
	}, [onClose]);
  ...
  // handleCaptchaModeChange 把明确的账号级滑块模式映射到兼容存储字段 show_browser。
  const handleCaptchaModeChange = (showBrowser: boolean) => updateField('show_browser', showBrowser);
    ...
    <div className="modal-overlay-centered">
      <div className="modal-container" style={{ maxWidth: '600px' }}>
        ...
        <div className="modal-body space-y-6">
          ...
          <div ref={autoConfirmSectionRef} data-testid="account-edit-section-auto-confirm" className="scroll-mt-4 flex items-center justify-between p-4 bg-gray-50 rounded-xl">
            <div>
          ...
          <div className="border-t border-gray-200 pt-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                <div className="font-bold text-gray-900">Token 滑块处理方式</div>
                <div className="mt-2 grid grid-cols-1 gap-2 sm:grid-cols-2" role="radiogroup" aria-label="Token 滑块处理方式">
                  <button type="button" role="radio" aria-checked={!editForm.show_browser} onClick={/* automaticModeClick 保存自动处理策略。 */ () => handleCaptchaModeChange(false)} className={`rounded-xl border p-3 text-left transition-colors ${!editForm.show_browser ? 'border-brand bg-blue-50 text-brand' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                    <span className="block text-sm font-bold">自动处理</span>
                    <span className="mt-1 block text-xs font-medium">使用远程／Playwright／备用自动引擎，不弹人工窗口。</span>
                  </button>
                  <button type="button" role="radio" aria-checked={editForm.show_browser} onClick={/* manualModeClick 保存人工接管策略。 */ () => handleCaptchaModeChange(true)} className={`rounded-xl border p-3 text-left transition-colors ${editForm.show_browser ? 'border-brand bg-blue-50 text-brand' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                    <span className="block text-sm font-bold">弹窗人工处理</span>
                    <span className="mt-1 block text-xs font-medium">打开可见 Chromium，只等待你手动操作，不自动拖动。</span>
                  </button>
                </div>
              </div>
