...
const accountListMocks = vi.hoisted(/* accountListMockFactory 创建页面测试共享替身。 */ () => ({
  accounts: [
    { id: 'account-1', nickname: 'Alpha', remark: '主账号', enabled: true, runtime_state: 'online', runtime_message: '在线' },
    { id: 'account-2', nickname: 'Beta', remark: '备用账号', enabled: false, runtime_state: 'disabled', runtime_message: '已停用' },
  ] as AccountDetail[],
  setAccounts: vi.fn(),
  loadAccounts: vi.fn(),
...
vi.mock('../components/AccountCard', /* accountCardMockFactory 提供账号卡片交互替身。 */ () => {
  ...
  const AccountCardMock: React.FC<any> = (props /* props 表示账号卡片及其事件回调。 */) => (
    <div data-testid="account-command-center">
      <div data-testid={`account-card-${props.account.id}`}>
        ...
        <button onClick={/* automationAction 打开当前账号自动任务弹窗。 */ () => props.onTasks(props.account)}>自动化任务</button>
      </div>
    </div>
  );
...
vi.mock('../components/AccountAutomationModal', /* automationModalMockFactory 提供账号任务弹窗替身。 */ () => ({
  default: /* AccountAutomationModalMock 暴露保存成功和失败回调以验证页面反馈。 */ (props: any) => (
    <div data-testid="account-automation-modal">
      <button onClick={/* saveSuccessAction 模拟设置保存成功。 */ () => props.onSaveSucceeded()}>模拟保存成功</button>
      <button onClick={/* saveFailureAction 模拟设置保存失败。 */ () => props.onSaveFailed('接口失败')}>模拟保存失败</button>
    </div>
  ),
}));
...
describe('AccountList 页面组合行为', /* 当前回调验证账号列表页面的搜索、删除、资料和二维码流程。 */ () => {
  ...
  test('搜索只展示匹配账号并保留总数提示', /* 当前回调验证账号搜索过滤行为。 */ () => {
    ...
    expect(screen.getByTestId('account-selector-account-1')).toBeTruthy();
    expect(screen.getByTestId('account-selector-account-2')).toBeTruthy();
    fireEvent.change(screen.getByPlaceholderText('搜索昵称 / 备注 / 账号ID'), { target: { value: 'Alpha' } });

    expect(screen.getByTestId('account-card-account-1')).toBeTruthy();
    expect(screen.queryByTestId('account-selector-account-2')).toBeNull();
    expect(screen.getByText('当前显示 1 / 2 个账号')).toBeTruthy();
  });

  test('点击左侧账号会切换右侧详情', /* 当前回调验证主从布局只展示当前选中账号的操作区。 */ () => {
    render(<AccountList />);

    expect(screen.getByTestId('account-card-account-1')).toBeTruthy();
    expect(screen.getByTestId('account-command-center').textContent).toContain('Alpha');
    fireEvent.click(screen.getByTestId('account-selector-account-2'));
    expect(screen.getByTestId('account-card-account-2')).toBeTruthy();
    expect(screen.queryByTestId('account-card-account-1')).toBeNull();
    expect(screen.getByTestId('account-command-center').textContent).toContain('Beta');
  });
  ...
  test('自动任务保存成功关闭弹窗并显示气泡，失败时保留弹窗', /* 当前回调验证账号自动任务保存交互闭环。 */ () => {
    render(<AccountList />);

    fireEvent.click(screen.getByText('自动化任务'));
    expect(screen.getByTestId('account-automation-modal')).toBeTruthy();
    fireEvent.click(screen.getByText('模拟保存失败'));
    expect(screen.getByTestId('account-automation-modal')).toBeTruthy();
    expect(screen.getByRole('status').textContent).toContain('Alpha：保存失败：接口失败');

    fireEvent.click(screen.getByText('模拟保存成功'));
    expect(screen.queryByTestId('account-automation-modal')).toBeNull();
    expect(screen.getByRole('status').textContent).toContain('Alpha：自动任务设置已保存');
  });
  ...
  test('二维码授权启动轮询并在成功后保存会话结果', /* 当前回调验证二维码生成、轮询成功和延迟收束流程。 */ async () => {
    ...
    expect(accountListMocks.completeQRVerification).toHaveBeenCalledWith('session-1', undefined, 'automatic');
