...
describe('useAccountQRCodeLogin 二维码登录协调器', /* 当前回调验证二维码生成、轮询、验证和清理边界。 */ () => {
  ...
  test('成功登录后完成持久化并延迟刷新账号列表', /* 当前回调验证二维码成功的完整生命周期。 */ async () => {
    ...
    expect(qrLoginMocks.completeQRVerification).toHaveBeenCalledWith('session-1', 'account-1', 'automatic');
  ...
  test('重新授权继承账号模式并透传用户最后一次选择', /* 当前回调验证二维码期间修改模式不会被旧闭包覆盖。 */ async () => {
    // manualAccount 是已经配置人工接管的重新授权账号。
    const manualAccount: AccountDetail = { ...accountFixture, show_browser: true };
    // hook 保存滑块模式透传场景的二维码协调器。
    const hook = renderHook(/* hookFactory 创建滑块模式透传场景 Hook。 */ () => useAccountQRCodeLogin({ onLoginSuccess: vi.fn() }));
    await act(/* startAction 使用人工账号启动二维码流程。 */ async () => {
      await hook.result.current.startQRLogin(manualAccount);
    });
    expect(hook.result.current.captchaMode).toBe('manual');
    act(/* modeChangeAction 在二维码有效期间改为自动处理。 */ () => {
      hook.result.current.setCaptchaMode('automatic');
    });
    qrLoginMocks.checkQRLoginStatus.mockResolvedValueOnce({ status: 'success' });
    await act(/* pollAction 触发扫码成功并持久化最后选择。 */ async () => {
      await vi.advanceTimersByTimeAsync(2000);
      await flushMicrotasks();
    });
    expect(qrLoginMocks.completeQRVerification).toHaveBeenCalledWith('session-1', 'account-1', 'automatic');
