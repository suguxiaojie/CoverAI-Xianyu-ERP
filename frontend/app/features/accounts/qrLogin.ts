...
export interface AccountQRCodeLoginState {
  ...
  // captchaMode 保存本次扫码成功后写入账号的 Token 风控处理策略。
  captchaMode: CaptchaMode;
  // setCaptchaMode 允许用户在二维码有效期间选择自动处理或弹窗人工处理。
...
export const useAccountQRCodeLogin = ({ onLoginSuccess }: AccountQRCodeLoginOptions): AccountQRCodeLoginState => {
  ...
  // captchaMode 保存当前二维码会话准备写入的账号级 Token 风控模式。
  const [captchaMode, setCaptchaModeState] = useState<CaptchaMode>('automatic');
  // captchaModeRef 保存用户最新选择，避免二维码成功回调读取 startQRLogin 创建时的旧闭包值。
  const captchaModeRef = useRef<CaptchaMode>('automatic');
  ...
  // setCaptchaMode 同步更新界面状态和成功回调读取的最新模式。
  const setCaptchaMode = (mode: CaptchaMode) => {
    captchaModeRef.current = mode;
    setCaptchaModeState(mode);
  };
  ...
  const completeAndPersistQRSession = async (sessionId: string, target?: AccountDetail | null): Promise<string> => {
    ...
    const response = await completeQRVerification(sessionId, target?.id, captchaModeRef.current);
  ...
  const startQRLogin = async (target?: AccountDetail): Promise<void> => {
    ...
    // initialCaptchaMode 让重新授权继承账号现有配置，新账号明确默认自动处理。
    const initialCaptchaMode: CaptchaMode = targetAccount?.show_browser ? 'manual' : 'automatic';
    setCaptchaMode(initialCaptchaMode);
