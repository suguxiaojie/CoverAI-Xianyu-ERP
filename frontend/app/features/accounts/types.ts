...
// AccountAutomationFocus 表示账号自动任务弹窗打开后需要定位的具体设置区块。
export type AccountAutomationFocus = 'auto_rate' | 'auto_polish' | 'auto_request_flower' | 'auto_receive_flower' | 'receipt_reminder';

// AccountEditFocus 表示账号编辑弹窗打开后需要定位的基础设置区块。
export type AccountEditFocus = 'auto_confirm';
...
export interface AccountEditForm {
  ...
  // show_browser 表示登录时展示浏览器，并在 Token 风控中禁用自动滑块、等待用户人工验证。
...
export interface AccountEditModalProps {
  ...
  saving: boolean;
	// initialFocus 是从自动化配置卡片进入时需要平滑定位的编辑区块；普通编辑入口为空。
	initialFocus?: AccountEditFocus;
