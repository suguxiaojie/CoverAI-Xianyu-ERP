...
// imageCardFixture 表示已保存受管引用、可直接预览或替换文件的图片卡密。
const imageCardFixture: Card = {
  ...cardFixture,
  id: 3,
  name: '教程图',
  type: 'image',
  data_content: undefined,
  image_url: `card-image://7/${'a'.repeat(64)}.png`,
};
...
describe('useCardActions 卡密动作协调器', /* 当前回调验证卡密筛选、编辑、新增、删除和展示动作。 */ () => {
  ...
  test('图片卡密把拖拽文件与表单共同提交并允许替换受管图片', /* 当前回调验证文件上传不会要求用户填写 URL。 */ async () => {
    // actionContext 保存卡密图片动作 Hook 和库存刷新替身。
    const { hook, loadCards } = createCardHook();
    // image 是等待随新增和编辑表单共同提交的 PNG 文件。
    const image = new File(['png'], '教程图.png', { type: 'image/png' });
	act(/* imageFormAction 写入新增图片卡密的拖拽文件。 */ () => hook.result.current.setAddForm(/* imageFormUpdater 合并新增图片字段。 */ current => ({ ...current, name: '教程图', type: 'image', image_mode: 'upload', image_file: image, content: '' })));
    await act(/* addImageAction 提交图片卡密和文件。 */ async () => hook.result.current.handleAddCard());
	expect(cardActionMocks.createCard).toHaveBeenCalledWith(expect.objectContaining({ name: '教程图', type: 'image' }), image, expect.objectContaining({ signal: expect.any(AbortSignal) }));
    expect(loadCards).toHaveBeenCalledTimes(1);

    act(/* editImageAction 打开既有受管图片卡密。 */ () => hook.result.current.handleEdit(imageCardFixture));
    act(/* replaceImageAction 选择替换图片。 */ () => hook.result.current.setEditForm(current => ({ ...current, image_file: image })));
    await act(/* saveImageAction 保存替换后的图片卡密。 */ async () => hook.result.current.handleSaveEdit());
	expect(cardActionMocks.updateCard).toHaveBeenCalledWith(3, expect.objectContaining({ name: '教程图', type: 'image' }), image, expect.objectContaining({ signal: expect.any(AbortSignal) }));
