...
import { getDashboardAccounts,getDashboardStats,getDashboardRuntimeStatuses,getItems,getOrderAnalytics,getValidOrders } from './api';
...
vi.mock('./api', /* dashboardApiMockFactory 提供仪表盘 Hook 的确定性 API 替身。 */ () => ({
  ...
  getDashboardRuntimeStatuses: vi.fn(),
  getItems: vi.fn(),
...
// getAccountsMock 是读取 Dashboard 非敏感账号摘要的可控请求替身。
const getAccountsMock = vi.mocked(getDashboardAccounts);
// getRuntimeStatusesMock 是读取账号实时连接状态的可控请求替身。
const getRuntimeStatusesMock = vi.mocked(getDashboardRuntimeStatuses);
...
const dashboardOptions: UseDashboardOptions = { range: 'today', customStartDate: '', customEndDate: '', customRangeVersion: 0, accountID: '' };
...
describe('useDashboard', /* 当前回调处理仪表盘并行请求和派生数据状态。 */ () => {
  beforeEach(/* 当前回调重置仪表盘 API 替身。 */ () => {
    ...
    getAccountsMock.mockResolvedValue([{ id: 'account-1', enabled: true }, { id: 'account-2', enabled: true }]);
  ...
  test('并行加载概览和时间范围数据后生成派生结果', /* 当前回调验证仪表盘成功加载路径。 */ async () => {
    ...
    expect(hook.result.current.maxProductSales).toBe(2);
		expect(hook.result.current.runtimeSummary).toMatchObject({ state: 'healthy', onlineAccounts: 2, enabledAccounts: 2 });
		expect(hook.result.current.accounts).toHaveLength(2);
	...
	test('切换账号后统一重读商品、当前周期、上周期和订单明细', /* 当前回调验证单账号选择不会只在浏览器过滤部分数据。 */ async () => {
		// hook 是可通过属性切换账号范围的 Dashboard Hook。
		const hook = renderHook(
			// props 是当前渲染传入的账号范围。
			(props: { /** accountID 是当前测试账号范围。 */ accountID: string }) => useDashboard({ ...dashboardOptions, accountID: props.accountID }),
			{ initialProps: { accountID: '' } },
		);
		await waitFor(/* initialAssertion 等待默认全账号数据完成。 */ () => expect(hook.result.current.data?.accountID).toBe(''));
		hook.rerender({ accountID: 'account-2' });
		await waitFor(/* scopedAssertion 等待单账号数据完成并替换旧范围。 */ () => expect(hook.result.current.data?.accountID).toBe('account-2'));
		expect(getItemsMock).toHaveBeenLastCalledWith('account-2', expect.objectContaining({ signal: expect.any(AbortSignal) }));
		expect(getAnalyticsMock).toHaveBeenLastCalledWith(expect.objectContaining({ account_id: 'account-2' }), expect.objectContaining({ signal: expect.any(AbortSignal) }));
		expect(getValidOrdersMock).toHaveBeenLastCalledWith(expect.objectContaining({ account_id: 'account-2' }), expect.objectContaining({ signal: expect.any(AbortSignal) }));
		expect(hook.result.current.runtimeSummary).toMatchObject({ state: 'healthy', onlineAccounts: 1, enabledAccounts: 1 });
	});

  test('运行状态请求失败时保留经营数据并标记状态未知', /* 当前回调验证可选运行快照失败不会阻断 Dashboard 主数据。 */ async () => {
    getRuntimeStatusesMock.mockRejectedValueOnce(new Error('运行状态服务失败'));
    // hook 是运行状态降级场景下的 Dashboard Hook。
    const hook = renderHook(renderDashboardHook);
    await waitFor(
      // overviewAssertion 等待经营概览独立完成。
      () => expect(hook.result.current.status.overview).toBe('success'),
    );
    expect(hook.result.current.data?.stats).toEqual(statsFixture);
    expect(hook.result.current.runtimeSummary).toMatchObject({ state: 'unknown', onlineAccounts: 0, enabledAccounts: 0 });
    expect(hook.result.current.status.error).toBe('');
  });
  ...
  test('自定义日期倒置时阻断范围请求并提示错误', /* 当前回调验证日期范围校验错误分支。 */ async () => {
    // invalidOptions 是开始日期晚于结束日期的非法范围参数。
		const invalidOptions: UseDashboardOptions = { range: 'custom', customStartDate: '2026-08-20', customEndDate: '2026-08-01', customRangeVersion: 1, accountID: '' };
