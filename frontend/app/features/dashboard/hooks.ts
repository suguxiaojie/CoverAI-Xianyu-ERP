...
import { getDashboardAccounts,getDashboardRuntimeStatuses,getDashboardStats,getItems,getOrderAnalytics,getValidOrders } from './api';
import type { DashboardAccountSummary,DashboardRuntimeStatus } from './api';
import { buildCategoryData,buildChartData,buildItemNameMap,buildProductSalesData,buildRuntimeSummary,buildSourceData,dashboardRuntimeSummary,getMaxProductSales,getRangeLabel,getTrendPercent,isCurrentDashboardRequest } from './state';
...
export type UseDashboardOptions = DashboardRangeSelection & { /** customRangeVersion 表示自定义范围版本。 */ customRangeVersion: number; /** accountID 是当前选中的账号，空值表示全部账号。 */ accountID: string };

/** Dashboard Hook 暴露给页面的可视化数据与操作。 */
export type UseDashboardResult = {
	/** accounts 是选择器可展示的当前用户非敏感账号摘要。 */
  ...
  /** runtimeSummary 是真实启用账号与 WebSocket 状态生成的运行概览。 */
  runtimeSummary: DashboardRuntimeSummary;
...
export const useDashboard = (options: UseDashboardOptions): UseDashboardResult => {
  // { 解构得到当前 Hook 返回的状态和操作函数。
	const { range, customStartDate, customEndDate, customRangeVersion, accountID } = options;
	// [overview, 解构得到当前 Hook 返回的状态和操作函数。
	const [overview, setOverview] = useState<Pick<DashboardData, 'accountID' | 'stats' | 'items' | 'itemNames'> | null>(null);
	// [rangeData, 解构得到当前 Hook 返回的状态和操作函数。
	const [rangeData, setRangeData] = useState<Pick<DashboardData, 'accountID' | 'analytics' | 'previousAnalytics' | 'validOrders' | 'dateRange'> | null>(null);
  // [status, 解构得到当前 Hook 返回的状态和操作函数。
  const [status, setStatus] = useState<DashboardStatus>({ overview: 'idle', range: 'idle', error: '' });
	// accounts 保存账号选择器的非敏感服务端数据。
	const [accounts, setAccounts] = useState<DashboardAccountSummary[]>([]);
	// runtimeStatuses 保存账号运行快照；undefined 表示加载中，null 表示辅助请求失败。
	const [runtimeStatuses, setRuntimeStatuses] = useState<Record<string,DashboardRuntimeStatus> | null | undefined>(undefined);
  ...
  useEffect(/* 当前回调同步 React 副作用和资源生命周期。 */ () => {
    ...
    setStatus(/* 当前回调处理用户交互或异步状态变化。 */ current => ({ ...current, overview: 'loading', error: '' }));
		Promise.all([
			...
			getItems(accountID || undefined, { signal: controller.signal }),
			...
			setOverview({ accountID, stats, items, itemNames: buildItemNameMap(items) });
      ...
      setStatus(/* 当前回调处理用户交互或异步状态变化。 */ current => ({ ...current, overview: 'error', error: errorMessage(error, '概览加载失败') }));
		...
		Promise.all([
			getDashboardAccounts({ signal: controller.signal }),
			getDashboardRuntimeStatuses({ signal: controller.signal }).catch(/* runtimeError 表示运行快照失败；账号列表仍可用于经营筛选。 */ runtimeError => {
				if (controller.signal.aborted || isAbortError(runtimeError)) throw runtimeError;
				return null;
			}),
		]).then(/* 当前回调保存同一代次的账号摘要和可选运行快照。 */ ([loadedAccounts, loadedRuntimeStatuses]) => {
			if (!isCurrentDashboardRequest(overviewSequence.current, sequence, controller.signal)) return;
			setAccounts(loadedAccounts);
			setRuntimeStatuses(loadedRuntimeStatuses);
		}).catch(/* error 是账号摘要查询失败原因；该辅助查询不影响经营概览。 */ error => {
			if (!isCurrentDashboardRequest(overviewSequence.current, sequence, controller.signal) || isAbortError(error)) return;
			setAccounts([]);
			setRuntimeStatuses(null);
		});
  ...
  useEffect(/* 当前回调同步 React 副作用和资源生命周期。 */ () => {
    ...
    Promise.all([
			getOrderAnalytics({ start_date: dateRange.startDate, end_date: dateRange.endDate, account_id: accountID || undefined }, { signal: controller.signal }),
			getOrderAnalytics({ start_date: previousRange.startDate, end_date: previousRange.endDate, account_id: accountID || undefined }, { signal: controller.signal }),
			getValidOrders({ start_date: dateRange.startDate, end_date: dateRange.endDate, account_id: accountID || undefined }, { signal: controller.signal }),
			...
			setRangeData({ accountID, analytics, previousAnalytics, validOrders, dateRange });
    ...
    return /* 当前回调处理用户交互或异步状态变化。 */ () => controller.abort();
	}, [accountID, customRangeVersion, range, refreshKey]);

  // data 数据。
  const data = useMemo<DashboardData | null>(
    // dashboardData 只有在概览和时间范围请求都成功后才对页面可见。
		() => overview?.accountID === accountID && rangeData?.accountID === accountID ? { ...overview, ...rangeData } : null,
		[accountID, overview, rangeData],
	);
	// scopedAccounts 是当前选择范围内参与运行状态汇总的账号。
	const scopedAccounts = useMemo(() => accountID ? accounts.filter(/* account 是当前检查是否为选中账号的摘要。 */ account => account.id === accountID) : accounts, [accountID, accounts]);
	// runtimeSummary 根据请求阶段和当前账号范围生成真实运行概览。
	const runtimeSummary = useMemo<DashboardRuntimeSummary>(/* runtimeSummaryBuilder 按账号范围合并当前运行快照且不保存可派生状态。 */ () => runtimeStatuses === undefined ? dashboardRuntimeSummary('loading') : runtimeStatuses === null ? dashboardRuntimeSummary('unknown') : buildRuntimeSummary(scopedAccounts, runtimeStatuses), [runtimeStatuses, scopedAccounts]);
