...
import type { OrderAnalytics } from '../../../shared/api-contract/admin';
import { buildCategoryData,buildChartData,buildItemNameMap,buildProductSalesData,buildRuntimeSummary,buildSourceData,getMaxProductSales,getRangeLabel,getTrendPercent,isCurrentDashboardRequest } from './state';
...
test('Dashboard 派生数据覆盖空值、零值和名称截断分支',
  ...
  () => {
    ...
    expect(buildProductSalesData({ ...analytics, item_stats: [{ ...analytics.item_stats![0], item_id: 'item-long' }] }, { 'item-long': longName })[0].name).toBe(longName);
    ...
    expect(buildSourceData(analytics, { 'item-long': longName }, ['red'])[0].name).toBe(longName);
    expect(buildCategoryData(analytics, { 'item-long': longName }, ['blue'])[0].name).toBe(longName);
    // historicalAnalytics 是后端直接返回下架商品历史名称的统计结果。
    const historicalAnalytics = { ...analytics, item_stats: [{ ...analytics.item_stats![0], item_title: '历史商品名称' }] } as OrderAnalytics;
    expect(buildProductSalesData(historicalAnalytics, {})).toEqual([{ name: '历史商品名称', sales: 2 }]);
    expect(buildSourceData(historicalAnalytics, {}, ['red'])[0]).toMatchObject({ name: '历史商品名称', color: 'red' });
    expect(buildCategoryData(historicalAnalytics, {}, ['blue'])[0]).toMatchObject({ name: '历史商品名称', color: 'blue' });
...
test('Dashboard 运行状态区分全在线、部分异常和认证问题',
  // 运行状态测试验证顶部状态和在线账号卡片来自真实启用账号快照。
  () => {
    // accounts 是两个启用账号和一个停用账号的非敏感摘要。
    const accounts = [{ id: 'a1', enabled: true }, { id: 'a2', enabled: true }, { id: 'a3', enabled: false }];
    expect(buildRuntimeSummary(accounts, {
      a1: { state: 'online', connected: true, failures: 0, updated_at: '' },
      a2: { state: 'online', connected: true, failures: 0, updated_at: '' },
    })).toMatchObject({ state: 'healthy', onlineAccounts: 2, enabledAccounts: 2 });
    expect(buildRuntimeSummary(accounts, {
      a1: { state: 'online', connected: true, failures: 0, updated_at: '' },
      a2: { state: 'reconnecting', connected: false, failures: 1, updated_at: '' },
    })).toMatchObject({ state: 'partial', onlineAccounts: 1, enabledAccounts: 2 });
    expect(buildRuntimeSummary(accounts, {
      a1: { state: 'online', connected: true, failures: 0, updated_at: '' },
      a2: { state: 'auth_expired', connected: false, failures: 1, updated_at: '' },
    })).toMatchObject({ state: 'auth_expired', onlineAccounts: 1, enabledAccounts: 2 });
    expect(buildRuntimeSummary(accounts, {
      a1: { state: 'verification_required', connected: false, failures: 1, updated_at: '' },
      a2: { state: 'online', connected: true, failures: 0, updated_at: '' },
    })).toMatchObject({ state: 'verification_required', onlineAccounts: 1, enabledAccounts: 2 });
    expect(buildRuntimeSummary(accounts, {
      a1: { state: 'runtime_conflict', connected: false, failures: 1, updated_at: '' },
      a2: { state: 'stopped', connected: false, failures: 0, updated_at: '' },
    })).toMatchObject({ state: 'runtime_conflict', onlineAccounts: 0, enabledAccounts: 2 });
    expect(buildRuntimeSummary(accounts, {})).toMatchObject({ state: 'offline', onlineAccounts: 0, enabledAccounts: 2 });
    expect(buildRuntimeSummary([{ id: 'a1', enabled: false }], {})).toMatchObject({ state: 'no_enabled_accounts', onlineAccounts: 0, enabledAccounts: 0 });
  });
