...
import type { DashboardAccountSummary,DashboardRuntimeStatus,Item,OrderAnalytics } from './api';
...
/** 商品毛利趋势只使用 feature API adapter 已返回的非敏感聚合字段。 */
export type DashboardDailyProfitPoint = {
  /** 用户本地日期。 */
  date: string;
  /** 当天已匹配成本订单成交额。 */
...
/** Dashboard 账号运行概览的稳定状态集合。 */
export type DashboardRuntimeSummaryState = 'loading' | 'healthy' | 'partial' | 'offline' | 'auth_expired' | 'verification_required' | 'runtime_conflict' | 'no_enabled_accounts' | 'unknown';

/** Dashboard 顶部状态和在线账号卡片共享的运行摘要。 */
export type DashboardRuntimeSummary = {
  /** state 是按异常优先级归一化后的整体运行状态。 */
  state: DashboardRuntimeSummaryState;
  /** onlineAccounts 是同时处于 online 且 WebSocket 已连接的启用账号数。 */
  onlineAccounts: number;
  /** enabledAccounts 是允许运行的账号总数。 */
  enabledAccounts: number;
};

/** 创建运行状态请求尚未完成或不可用时使用的安全摘要。 */
export const dashboardRuntimeSummary = (state: 'loading' | 'unknown'): DashboardRuntimeSummary => ({ state, onlineAccounts: 0, enabledAccounts: 0 });

/** 将非敏感账号摘要和运行状态快照合并为 Dashboard 整体状态。 */
export const buildRuntimeSummary = (accounts: DashboardAccountSummary[], statuses: Record<string, DashboardRuntimeStatus>): DashboardRuntimeSummary => {
  // enabledAccounts 是当前允许启动运行实例的账号集合。
  const enabledAccounts = accounts.filter(
    // account 是当前检查启用标记的账号摘要。
    account => account.enabled,
  );
  if (enabledAccounts.length === 0) return { state: 'no_enabled_accounts', onlineAccounts: 0, enabledAccounts: 0 };
  // enabledStatuses 是启用账号对应的运行状态；缺失状态保留 undefined 并按离线处理。
  const enabledStatuses = enabledAccounts.map(
    // account 是当前读取运行状态的启用账号。
    account => statuses[account.id],
  );
  // onlineAccounts 是同时报告 online 和 connected 的账号数量。
  const onlineAccounts = enabledStatuses.filter(
    // status 是当前检查连接真实性的运行状态快照。
    status => status?.state === 'online' && status.connected === true,
  ).length;
  // resultBase 是所有整体状态共享的在线和启用账号计数。
  const resultBase = { onlineAccounts, enabledAccounts: enabledAccounts.length };
  if (enabledStatuses.some(/* status 是当前检查人工验证状态的账号运行快照。 */ status => status?.state === 'verification_required')) return { state: 'verification_required', ...resultBase };
  if (enabledStatuses.some(/* status 是当前检查登录过期状态的账号运行快照。 */ status => status?.state === 'auth_expired')) return { state: 'auth_expired', ...resultBase };
  if (enabledStatuses.some(/* status 是当前检查运行冲突的账号运行快照。 */ status => status?.state === 'runtime_conflict')) return { state: 'runtime_conflict', ...resultBase };
  if (onlineAccounts === enabledAccounts.length) return { state: 'healthy', ...resultBase };
  if (onlineAccounts > 0) return { state: 'partial', ...resultBase };
  return { state: 'offline', ...resultBase };
};
  ...
  (analytics.item_stats || [])
    .map(
      ...
      item => {
        ...
        const itemName = item.item_title || itemNames[item.item_id] || item.item_id;
        return { name: itemName, sales: item.order_count };
      },
    )
