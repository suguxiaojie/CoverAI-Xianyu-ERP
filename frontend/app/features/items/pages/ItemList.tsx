import { ArrowRight,Box,CheckCircle2,ChevronDown,CircleDashed,Edit,Filter,Link2,LocateFixed,PackagePlus,Plus,RefreshCw,Save,Search,ShoppingBag,Trash2,UploadCloud,User,X,XCircle } from 'lucide-react';
...
import { ItemSKUDetailsModal,type SavedSKUCost } from '../components/ItemSKUDetailsModal';
import { ItemSpecBadge,ItemSpecSummary,itemPriceRange } from '../components/ItemSpecSummary';
...
const ItemList: React.FC<ItemListProps> = ({ onConfigureDelivery }) => {
  ...
  // skuDetailItem 保存当前打开规格详情和本地成本编辑器的商品。
  const [skuDetailItem, setSKUDetailItem] = useState<Item | null>(null);
  // costToast 保存成本保存成功或失败后的页面短暂提示。
  const [costToast, setCostToast] = useState<{ /** type 是提示类别。 */ type: 'success' | 'error'; /** text 是用户可见提示。 */ text: string } | null>(null);
  // costToastTimerRef 保存成本提示自动消失的计时器。
  ...
  // showCostToast 展示三秒成本提示，并清理上一条提示计时器。
  const showCostToast = useCallback(/* costToastAction 更新页面短暂提示。 */ (type: 'success' | 'error', text: string) => {
    setCostToast({ type, text });
    if (costToastTimerRef.current !== null) window.clearTimeout(costToastTimerRef.current);
    costToastTimerRef.current = window.setTimeout(/* costToastCleanup 三秒后清除提示状态。 */ () => {
      costToastTimerRef.current = null;
      setCostToast(null);
    }, 3000);
  }, []);

  // 页面卸载时清理成本提示计时器，避免晚到回调更新失去所有权的页面。
  useEffect(/* costToastLifecycleEffect 建立成本提示计时器生命周期。 */ () => {
    return /* costToastUnmountCleanup 卸载时清理仍未到期的提示计时器。 */ () => {
      if (costToastTimerRef.current !== null) window.clearTimeout(costToastTimerRef.current);
    };
  }, []);

  // handleSKUCostsSaved 将原子保存的全部成本同步回列表卡片和当前弹窗商品。
  const handleSKUCostsSaved = useCallback(/* skuCostsSavedHandler 更新全部 SKU 本地成本。 */ (costs: SavedSKUCost[]) => {
    // updateItemCosts 返回替换已有成本并补入单规格隐式成本行的商品副本。
    const updateItemCosts = (item: Item): Item => {
      // currentSKUs 复制当前商品 SKU，避免原地修改 React 服务端状态。
      const currentSKUs = [...(item.skus || [])];
      for (const /* cost 是当前待回写的已保存成本。 */ cost of costs) {
        // existingIndex 是当前成本对应 SKU 在商品列表中的下标。
        const existingIndex = currentSKUs.findIndex(/* skuMatcher 匹配成本所属 SKU。 */ sku => sku.sku_id === cost.sku_id);
        if (existingIndex >= 0) {
          currentSKUs[existingIndex] = { ...currentSKUs[existingIndex], cost_cents: cost.cost_cents };
        } else {
          currentSKUs.push({ sku_id: cost.sku_id, inventory_id: '', properties: [], price_cents: 0, quantity: 0, initial_quantity: 0, enabled: true, sort_order: 0, cost_cents: cost.cost_cents, synced_at: 0, local_only: true });
        }
      }
      return { ...item, skus: currentSKUs };
    };
    setItems(/* currentItems 更新商品列表中的目标 SKU 成本。 */ currentItems => currentItems.map(/* currentItem 匹配当前打开商品。 */ currentItem => currentItem.cookie_id === skuDetailItem?.cookie_id && currentItem.item_id === skuDetailItem.item_id ? updateItemCosts(currentItem) : currentItem));
    setSKUDetailItem(null);
    showCostToast('success', '成本保存成功');
  }, [showCostToast, skuDetailItem?.cookie_id, skuDetailItem?.item_id]);
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col xl:flex-row xl:justify-between xl:items-center gap-4">
        ...
        <div className="flex flex-wrap items-end gap-3">
            <div className="flex min-w-[200px] flex-col gap-1.5">
			  ...
			  <div className="flex h-[46px] items-center gap-2 overflow-hidden rounded-xl border border-gray-200 bg-white px-4 shadow-sm focus-within:ring-4 focus-within:ring-blue-100">
				<Filter className="h-4 w-4 shrink-0 text-gray-400" />
				<select
                  id="item-account-filter"
                  aria-label="按账号筛选商品列表"
				  className="h-full min-w-0 flex-1 appearance-none border-0 bg-transparent text-sm outline-none"
                  value={accountFilter}
              ...
              <div key={`${item.cookie_id}-${item.item_id}`} className="ios-card p-3 rounded-2xl hover:shadow-lg transition-all group relative flex flex-col">
                  ...
                  <div className="aspect-square bg-gray-100 rounded-xl mb-2.5 overflow-hidden relative">
                      ...
                      <div className="absolute top-1.5 left-1.5 bg-black/50 backdrop-blur-md text-white text-[10px] font-bold px-1.5 py-0.5 rounded-md">
                          {itemPriceRange(item) || formatItemPrice(item.item_price)}
                      </div>
                      <ItemSpecBadge item={item} />
                  </div>
                  ...
                  <ItemSpecSummary item={item} onOpen={/* openSKUDetails 打开当前商品规格详情。 */ () => setSKUDetailItem(item)} />
      ...
      {skuDetailItem && createPortal(
        <ItemSKUDetailsModal item={skuDetailItem} onClose={/* closeSKUDetails 关闭成本设置弹窗。 */ () => setSKUDetailItem(null)} onCostsSaved={handleSKUCostsSaved} onSaveFailed={/* costSaveFailed 展示失败提示但保留弹窗和草稿。 */ message => showCostToast('error', message)} />,
        document.body,
      )}

      {costToast && createPortal(
        <div role={costToast.type === 'error' ? 'alert' : 'status'} className={`fixed right-6 top-6 z-[200] flex items-center gap-2 rounded-xl px-4 py-3 text-sm font-extrabold text-white shadow-xl animate-fade-in ${costToast.type === 'success' ? 'bg-emerald-500' : 'bg-red-500'}`}>
          {costToast.type === 'success' ? <CheckCircle2 className="h-4 w-4" /> : <XCircle className="h-4 w-4" />}{costToast.text}
        </div>,
        document.body,
      )}
