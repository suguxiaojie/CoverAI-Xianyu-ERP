...
export const useNotifications = (isAdmin: boolean): NotificationState => {
  ...
  const handleSave = useCallback(
    ...
    const validationError = validateNotificationForm(form, Boolean(editing));
    if (validationError) {
    ...
    try {
      ...
      if (editing) {
		// 编辑摘要不返回敏感配置；没有填写配置字段时省略 config，避免覆盖已有密钥。
		if (validateNotificationForm(form, false)) delete (payload as Partial<typeof payload>).config;
