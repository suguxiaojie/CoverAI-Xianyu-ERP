...
const Notifications: React.FC<NotificationsProps> = ({ isAdmin = false }) => {
  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      {notificationState.toast && createPortal(<div role="status" className={`fixed left-1/2 top-6 z-[10001] flex -translate-x-1/2 items-center gap-2 rounded-xl px-5 py-3 text-sm font-bold text-white shadow-xl ${notificationState.toast.type === 'success' ? 'bg-success-500' : 'bg-danger-500'}`}>{notificationState.toast.type === 'success' ? <Check className="w-4 h-4" /> : <X className="w-4 h-4" />}{notificationState.toast.text}</div>,document.body)}
    </div>
  );
};
