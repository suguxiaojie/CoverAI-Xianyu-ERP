...
export const notificationChannelTypes: Record<NotificationChannelType, NotificationChannelMeta> = {
  bark: {
    ...
    fields: [
      ...
      { key: 'title', label: '标题（可选）', placeholder: 'CoverAI闲鱼助手' },
...
export const validateNotificationForm = (form: NotificationForm, editing = false): string => {
  // meta 是当前渠道类型的静态校验配置。
  ...
  if (missingField && !editing) return `请填写 ${missingField.label}`;
