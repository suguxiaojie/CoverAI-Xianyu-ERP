...
describe('useOrderQuery 与 useOrderImport', /* 当前回调处理订单查询和导入 Hook 的请求边界。 */ () => {
  beforeEach(/* 当前回调重置订单 API 替身和浏览器提示。 */ () => {
	...
	getOrdersMock.mockResolvedValue({ success: true, data: [orderFixture], total: 1, page: 1, page_size: 20, total_pages: 2, trigger_counts: {}, settlement_summary: { order_count: 3, gross_amount: '405.00', service_fee: '6.48', pending_amount: '398.52', service_fee_rate: '1.6%' } });
  ...
  test('订单查询加载辅助数据并提供展示名称解析', /* 当前回调验证订单列表 Hook 成功路径。 */ async () => {
	...
	expect(hook.result.current.settlementSummary).toEqual({ order_count: 3, gross_amount: '405.00', service_fee: '6.48', pending_amount: '398.52', service_fee_rate: '1.6%' });
  ...
  test('订单查询把实付金额范围转发到服务端并重载第一页', /* 当前回调验证金额筛选进入权威分页查询而非前端假过滤。 */ async () => {
    // hook 是金额范围筛选场景的订单查询 Hook。
    const hook = renderHook(
      // amountRangeHookFactory 创建金额范围筛选场景的 Hook。
      () => useOrderQuery({ pageSize: 20 }),
    );
    await waitFor(
      // initialLoadingAssertion 等待初始订单请求完成。
      () => expect(hook.result.current.loading).toBe(false),
    );
    await act(
      // amountRangeAction 应用包含上下界的实付金额范围。
      () => {
        hook.result.current.setPage(4);
        hook.result.current.setAmountRange({ minAmount: '135.00', maxAmount: '690', label: '¥135.00–¥690' });
        hook.result.current.setPage(1);
      },
    );
    await waitFor(
      // amountRangeAssertion 等待新查询携带金额范围和第一页。
      () => expect(getOrdersMock).toHaveBeenLastCalledWith(undefined, 'all', 1, 20, '', expect.objectContaining({
        signal: expect.any(AbortSignal),
        minAmount: '135.00',
        maxAmount: '690',
      })),
    );
    expect(hook.result.current.amountRange.label).toBe('¥135.00–¥690');
    hook.unmount();
  });
  ...
  test('订单时间范围变化后以第一页和 UTC 边界重新查询', /* 当前回调验证时间筛选与分页及请求取消边界。 */ async () => {
    // hook 是订单时间范围查询场景的 Hook 渲染结果。
    const hook = renderHook(
      // timeRangeHookFactory 创建携带时间筛选状态的订单查询 Hook。
      () => useOrderQuery({ pageSize: 20 }),
    );
    await waitFor(
      // loadingAssertion 等待初始订单请求完成。
      () => expect(hook.result.current.loading).toBe(false),
    );
    await act(
      // timeRangeAction 模拟页面应用时间范围并重置到第一页。
      () => {
        hook.result.current.setPage(4);
        hook.result.current.setCreatedRange({ createdFrom: '2026-08-20T00:00:00.000Z', createdTo: '2026-08-21T00:00:00.000Z', label: '今天' });
        hook.result.current.setPage(1);
      },
    );
    await waitFor(
      // timeRangeAssertion 等待新查询携带时间半开区间。
      () => expect(getOrdersMock).toHaveBeenLastCalledWith(undefined, 'all', 1, 20, '', expect.objectContaining({
        signal: expect.any(AbortSignal),
    ...
    expect(hook.result.current.createdRange.label).toBe('今天');
    hook.unmount();
  });
