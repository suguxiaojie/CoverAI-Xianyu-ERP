...
import type { AccountDetail,Item,Order,OrderSettlementSummary } from './api';
...
import type { OrderAmountRange,OrderCreatedRange,OrderImportState,OrderQueryState } from './types';

// defaultOrderCreatedRange 表示订单列表未限制创建时间的初始筛选状态。
const defaultOrderCreatedRange: OrderCreatedRange = { createdFrom: '', createdTo: '', label: '全部时间' };

// defaultOrderAmountRange 表示订单列表未限制实付金额的初始筛选状态。
const defaultOrderAmountRange: OrderAmountRange = { minAmount: '', maxAmount: '', label: '金额范围' };

// defaultOrderSettlementSummary 表示服务端统计尚未返回或旧服务不含该字段时的零值。
const defaultOrderSettlementSummary: OrderSettlementSummary = { order_count: 0, gross_amount: '0.00', service_fee: '0.00', pending_amount: '0.00', service_fee_rate: '1.6%' };
...
export const useOrderQuery = (options: OrderQueryOptions = { pageSize: 20 }): OrderQueryState => {
  ...
  // createdRange 保存已经应用的订单创建时间范围；setCreatedRange 触发取消旧查询并重新加载第一页。
  const [createdRange, setCreatedRange] = useState<OrderCreatedRange>(defaultOrderCreatedRange);
  // amountRange 保存已经应用的实付金额范围；setAmountRange 触发取消旧查询并重新加载第一页。
  const [amountRange, setAmountRange] = useState<OrderAmountRange>(defaultOrderAmountRange);
  ...
  const [page, setPage] = useState(1);
	...
	// settlementSummary 保存当前账号、时间、金额和搜索范围内已发货订单的待结算统计。
	const [settlementSummary, setSettlementSummary] = useState<OrderSettlementSummary>(defaultOrderSettlementSummary);
  ...
  const loadOrders = useCallback(
    ...
    try {
      ...
      const result = await getOrders(accountFilter || undefined, filter, page, options.pageSize, debouncedSearch, {
        signal: controller.signal,
        createdFrom: createdRange.createdFrom || undefined,
        createdTo: createdRange.createdTo || undefined,
        minAmount: amountRange.minAmount || undefined,
        maxAmount: amountRange.maxAmount || undefined,
      });
      if (!isCurrentOrderRequest(generation, orderGeneration.current)) return;
		setOrders(result.data);
		setTotalPages(result.total_pages);
		setSettlementSummary(result.settlement_summary || defaultOrderSettlementSummary);
    } catch (error /* 订单查询异常 */) {
    ...
    [accountFilter, amountRange.maxAmount, amountRange.minAmount, createdRange.createdFrom, createdRange.createdTo, debouncedSearch, filter, options.pageSize, page],
  );
