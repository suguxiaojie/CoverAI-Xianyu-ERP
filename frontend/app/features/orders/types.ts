...
import type { AccountDetail,Item,Order,OrderSettlementSummary,OrderStatus } from './api';
...
// OrderCreatedRange 描述订单列表使用的 UTC 创建时间半开区间和用户可见摘要。
export interface OrderCreatedRange {
  // createdFrom 是包含下界的 RFC3339 UTC 时间；空字符串表示没有下界。
  createdFrom: string;
  // createdTo 是排除上界的 RFC3339 UTC 时间；空字符串表示没有上界。
  createdTo: string;
  // label 是时间筛选入口展示的本地时间摘要。
  label: string;
}

// OrderAmountRange 描述订单列表已经应用的实付金额元范围和入口摘要。
export interface OrderAmountRange {
  // minAmount 是包含下界的元金额文本；空字符串表示不限制最低金额。
  minAmount: string;
  // maxAmount 是包含上界的元金额文本；空字符串表示不限制最高金额。
  maxAmount: string;
  // label 是金额筛选入口展示的摘要。
  label: string;
}
...
export interface OrderQueryState {
  ...
  // setCreatedRange 更新订单创建时间范围。
  setCreatedRange: Dispatch<SetStateAction<OrderCreatedRange>>;
  // amountRange 是当前已经应用的实付金额范围。
  amountRange: OrderAmountRange;
  // setAmountRange 更新实付金额范围。
  setAmountRange: Dispatch<SetStateAction<OrderAmountRange>>;
