...
const useRuleActionsHarness = () => {
  ...
  const options: RuleActionsOptions = { selectedAccountId, setSelectedAccountId, setActiveTab, items, accounts, setAutomationRules, setCards, setItems: /* setItemsAction 写入外部联动商品列表。 */ linkedItems => setItems(linkedItems), setLoading, loadAutomationRules, loadReferenceData, loadReplyRules, loadDefaultReplies };
...
describe('useRuleActions', /* 当前回调验证规则页面动作协调器的核心状态和副作用。 */ () => {
  ...
  test('关键词和默认回复动作分别读取、保存并刷新', /* 当前回调验证两个回复页签的动作边界。 */ async () => {
    ...
    expect(updateReplyMock).toHaveBeenCalledWith(expect.objectContaining({ keyword: '你好', match_type: 'contains', account_ids: ['account-1'], enabled: true }), '');
