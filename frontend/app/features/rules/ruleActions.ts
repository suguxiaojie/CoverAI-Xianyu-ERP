...
import type { AccountDetail,AutomationTriggerType,Card,DefaultReplyForm,Item,ReplyRule,RulesProps,RulesTab,ShippingRule,ShippingVariant } from './types';
...
export interface RuleActionsState {
  ...
  // replyFeedback 保存关键词规则应用内成功或失败提示。
  replyFeedback: { /** kind 是反馈类型。 */ kind: 'success' | 'error'; /** message 是用户可见文案。 */ message: string } | null;
  // setReplyFeedback 更新关键词规则应用内反馈。
  setReplyFeedback: Dispatch<SetStateAction<{ /** kind 是反馈类型。 */ kind: 'success' | 'error'; /** message 是用户可见文案。 */ message: string } | null>>;
...
export const useRuleActions = ({
  ...
  // replyFeedback 保存关键词规则校验、保存成功或失败的应用内提示。
  const [replyFeedback, setReplyFeedback] = useState<{ /** kind 是反馈类型。 */ kind: 'success' | 'error'; /** message 是用户可见文案。 */ message: string } | null>(null);
  // defaultReplySubmitState 保存默认回复提交状态。
  ...
  const handleAddReplyRule = useCallback(/* addReplyAction 创建关键词回复草稿。 */ () => {
    if (accounts.length === 0) { setReplyFeedback({ kind: 'error', message: '当前没有可用店铺账号' }); return; }
    setReplyFeedback(null);
    setEditingReplyRule({ keyword: '', keywords: [], reply_content: '', image_url: '', item_id: '', type: 'text', match_type: 'contains', message_scope: 'customer', message_scopes: ['customer'], system_types: [], account_ids: accounts.map(/* account 是默认勾选的店铺账号。 */ account => account.id), enabled: true, reply_interval_seconds: 1, send_delay_seconds: 0 });
    setShowReplyModal(true);
  }, [accounts]);

  // handleSaveReplyRule 校验并保存关键词回复规则。
  const handleSaveReplyRule = useCallback(/* saveReplyAction 保存关键词回复。 */ async () => {
    if (!editingReplyRule || replySubmitState.submitting) return;
    ...
    // scopes 是当前规则允许处理的客户／系统消息来源。
    const scopes = editingReplyRule.message_scopes || [editingReplyRule.message_scope || 'customer'];
    // systemOnly 表示具体系统事件本身可以独立作为触发条件。
    const systemOnly = scopes.includes('system') && !scopes.includes('customer');
    if (!systemOnly && !(editingReplyRule.keywords?.length || editingReplyRule.keyword?.trim())) { setReplyFeedback({ kind: 'error', message: '客户消息规则至少需要一个关键词' }); return; }
    if (scopes.includes('system') && !(editingReplyRule.system_types?.length)) { setReplyFeedback({ kind: 'error', message: '请至少选择一个具体系统事件' }); return; }
    if (!(editingReplyRule.account_ids?.length)) { setReplyFeedback({ kind: 'error', message: '请至少选择一个适用店铺' }); return; }
    if (!hasReplyContent) { setReplyFeedback({ kind: 'error', message: '请填写回复内容' }); return; }
    setReplyFeedback(null);
    setReplySubmitState(startRuleSubmission(replySubmitState));
    // succeeded 记录保存是否成功。
    let succeeded = false;
    try { await updateReplyRule({ ...editingReplyRule, enabled: editingReplyRule.enabled !== false }, ''); setShowReplyModal(false); await loadReplyRules(); setReplyFeedback({ kind: 'success', message: '关键词回复规则保存成功' }); succeeded = true; } catch (/* error 表示关键词回复保存异常。 */ error) { setReplyFeedback({ kind: 'error', message: '保存失败：' + (error as Error).message }); } finally { setReplySubmitState(/* current 保存关键词提交状态。 */ current => finishRuleSubmission(current, succeeded)); }
  }, [editingReplyRule, loadReplyRules, replySubmitState]);

  // handleDeleteReply 删除指定关键词回复并刷新列表。
  const handleDeleteReply = useCallback(/* deleteReplyAction 删除关键词回复。 */ async (id: string) => {
    try { await deleteReplyRule(id, ''); await loadReplyRules(); setReplyFeedback({ kind: 'success', message: '关键词回复规则已删除' }); } catch (/* error 表示关键词回复删除异常。 */ error) { setReplyFeedback({ kind: 'error', message: '删除失败：' + (error as Error).message }); }
  }, [loadReplyRules]);
  ...
  return {
    ...
    automationSubmitState, replySubmitState, replyFeedback, setReplyFeedback, defaultReplySubmitState, editingAutomationRule, setEditingAutomationRule,
