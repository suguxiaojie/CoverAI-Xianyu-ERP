    ...
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="space-y-8">
		  ...
		  {/* Location Card Defaults */}
		  <section className="space-y-4">
			<h3 className="flex items-center gap-2 text-lg font-extrabold text-gray-800"><div className="rounded-lg bg-sky-500 p-1.5 text-white"><MapPinned className="h-4 w-4" /></div>位置卡片默认内容</h3>
			<div className="ios-card space-y-4 rounded-xl bg-white p-6">
			  <p className="text-xs leading-5 text-gray-500">聊天窗口打开位置卡片时会自动带入以下内容；每次发送前仍可临时修改，并且必须二次确认。</p>
			  <div className="space-y-2"><label className="block text-sm font-bold text-gray-800">默认标题</label><input aria-label="位置卡片默认标题" maxLength={80} value={settings['chat.location_card_title'] || ''} onChange={/* updateDefaultLocationTitle 更新聊天位置卡片默认标题。 */ event => setSettings({ ...settings, 'chat.location_card_title': event.target.value })} placeholder="例如：CoverAI 实体店" className="ios-input w-full rounded-xl px-4 py-3 text-sm" /></div>
			  <div className="space-y-2"><label className="block text-sm font-bold text-gray-800">默认地址／到店说明</label><textarea aria-label="位置卡片默认说明" maxLength={300} rows={3} value={settings['chat.location_card_content'] || ''} onChange={/* updateDefaultLocationContent 更新聊天位置卡片默认说明。 */ event => setSettings({ ...settings, 'chat.location_card_content': event.target.value })} placeholder="例如：东门电梯上楼右转，营业时间 10:00—21:00" className="ios-input w-full resize-none rounded-xl px-4 py-3 text-sm leading-6" /></div>
			  <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
				<div className="space-y-2"><label className="block text-sm font-bold text-gray-800">默认纬度</label><input aria-label="位置卡片默认纬度" type="number" min="-90" max="90" step="0.000001" value={settings['chat.location_card_latitude'] || ''} onChange={/* updateDefaultLocationLatitude 更新聊天位置卡片默认纬度。 */ event => setSettings({ ...settings, 'chat.location_card_latitude': event.target.value })} placeholder="22.540503" className="ios-input w-full rounded-xl px-4 py-3 text-sm tabular-nums" /></div>
				<div className="space-y-2"><label className="block text-sm font-bold text-gray-800">默认经度</label><input aria-label="位置卡片默认经度" type="number" min="-180" max="180" step="0.000001" value={settings['chat.location_card_longitude'] || ''} onChange={/* updateDefaultLocationLongitude 更新聊天位置卡片默认经度。 */ event => setSettings({ ...settings, 'chat.location_card_longitude': event.target.value })} placeholder="113.934528" className="ios-input w-full rounded-xl px-4 py-3 text-sm tabular-nums" /></div>
			  </div>
			</div>
		  </section>
