...
const settingsFixture: SystemSettings = {
  ...
  renewal_log_retention_days: 15,
	'chat.location_card_title': '默认门店',
	'chat.location_card_content': '默认入口',
	'chat.location_card_latitude': '31.230400',
	'chat.location_card_longitude': '121.473700',
};

test('Settings 保存草稿只提交允许的系统配置字段',
  // 配置裁剪测试验证 SMTP 等兼容字段不会被系统设置批量接口覆盖。
  () => {
	expect(buildPersistableSettings(settingsFixture)).toEqual({ log_level: 'info', ai_api_key: 'api-key', renewal_log_retention_days: 15, 'chat.location_card_title': '默认门店', 'chat.location_card_content': '默认入口', 'chat.location_card_latitude': '31.230400', 'chat.location_card_longitude': '121.473700' });
  });
