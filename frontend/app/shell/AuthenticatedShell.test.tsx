...
describe('AuthenticatedShell 页面组合行为', /* 当前回调验证权限回退和跨页面联动边界。 */ () => {
  ...
  test('旧知识库路由标识安全回退到仪表盘', /* dormantKnowledgeRouteTest 验证下线期间旧页面标识不再加载知识库分片。 */ async () => {
    // appContentModule 是已经移除知识库页面分支的页面组合模块。
    const appContentModule = await loadAppContent();
    // AppContentComponent 是负责让未知旧标识回退仪表盘的页面组合组件。
    const AppContentComponent = appContentModule.AppContent;
    render(
      <AppContentComponent
        activeTab="knowledge"
        isAdmin={false}
        onConfigureDelivery={/* configureTargetAction 在本路由测试中不产生商品联动。 */ () => undefined}
        onDeliveryTargetHandled={/* handledAction 在本路由测试中不消费规则目标。 */ () => undefined}
      />,
    );
    await waitFor(/* dashboardFallbackAssertion 等待旧知识库标识回退后的仪表盘分片。 */ () => expect(screen.getByTestId('dashboard-page')).toBeTruthy());
  });
