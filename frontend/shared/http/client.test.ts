...
import { ApiError, del, get, post, postForm, put, putForm } from './client';
...
describe('request helpers', () => {
  ...
  test('puts FormData without forcing a content type', async () => {
	// fetchMock 返回固定成功响应并记录 multipart PUT 参数。
	const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({ success: true }), {
	  status: 200,
	  headers: { 'content-type': 'application/json' },
	}));
	vi.stubGlobal('fetch', fetchMock);
	// form 是卡密图片编辑请求使用的 multipart 表单。
	const form = new FormData();
	form.set('image', new File(['image'], 'card.png', { type: 'image/png' }));
	await expect(putForm('/cards/1', form)).resolves.toEqual({ success: true });
	expect(fetchMock).toHaveBeenCalledWith('/cards/1', expect.objectContaining({
	  method: 'PUT',
	  credentials: 'include',
	  body: form,
	}));
	// requestInit 是 fetch 收到的请求配置，multipart 不应由客户端手工设置 Content-Type。
	const requestInit = fetchMock.mock.calls[0]?.[1] as RequestInit | undefined;
	expect(requestInit?.headers).toBeUndefined();
  } /* 测试回调验证：puts FormData without forcing a content type。 */);
