...
// putForm 使用认证 PUT 请求上传 FormData；浏览器负责生成 multipart boundary，调用方可传入取消信号。
export const putForm = async <T>(url: string, body: FormData, options: RequestControlOptions = {}): Promise<T> => {
	// control 组合调用方取消信号和上传超时，完成后必须清理定时器与监听器。
	const control = controlledSignal(options.signal, options.timeoutMs ?? uploadRequestTimeoutMs);
	// res 保存 multipart PUT 请求响应，异常时按取消或超时语义转换。
	let res: Response;
	try {
	  res = await fetch(url, {
		method: 'PUT',
	...
	} catch (/* error 是上传请求的网络异常；取消原因由 control.signal 统一判断。 */ error) {
	  if (control.signal.aborted) {
		throw new Error(options.signal?.aborted ? '请求已取消' : '上传超时，请稍后重试');
	  }
	  throw error;
	...
	// contentType 和 isJson 分别保存响应媒体类型及其是否可以解析为 JSON。
	const contentType = res.headers.get('content-type') || '';
	const isJson = contentType.includes('application/json');
	// payload 是上传接口返回的成功 DTO 或统一错误载荷。
	const payload = await readResponsePayload(res, isJson);
	if (!res.ok) {
	  if (res.status === 401) notifyAuthExpired();
	  throw new ApiError(res.status, payload);
