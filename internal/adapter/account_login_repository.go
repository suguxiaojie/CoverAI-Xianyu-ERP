...
// UpdateCaptchaModeOwned 在调用方持有账号凭证锁时保存 Token 风控模式，并复核账号归属。
func (r *AccountLoginRepository) UpdateCaptchaModeOwned(ctx context.Context, userID int64, accountID string, showBrowser bool) error {
	// validationErr 表示账号 Cookie 仓储没有完成装配。
	validationErr := r.validateCookies()
	if validationErr != nil {
		return validationErr
	}
	// updateErr 保存复用账号设置事务写入 show_browser 的归属检查结果。
	_, updateErr := r.store.Cookies.UpdateSettings(ctx, accountID, db.AccountSettingsUpdate{UserID: userID, ShowBrowser: &showBrowser})
	return normalizeAccountSettingsError(updateErr)
}
...
// UpdateCaptchaModeOwned 保存扫码界面选择的账号级 Token 风控模式。
func (r QRLoginRepository) UpdateCaptchaModeOwned(ctx context.Context, userID int64, accountID string, showBrowser bool) error {
	return r.repository.UpdateCaptchaModeOwned(ctx, userID, accountID, showBrowser)
}
