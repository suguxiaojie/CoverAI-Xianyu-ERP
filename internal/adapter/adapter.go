...
// browserTokenCaptchaManualRecoverer 只打开可见浏览器并等待用户完成人工验证，不产生鼠标或键盘输入。
type browserTokenCaptchaManualRecoverer interface {
	// TokenCaptchaManualRecover 返回合并新 x5sec 的 Cookie；ctx 取消、窗口关闭或三分钟超时必须同步释放浏览器。
	TokenCaptchaManualRecover(ctx context.Context, cookieID, cookieStr, verificationURL string, provider browser.TokenCaptchaURLProvider) (cookies string, err error)
}
...
func NewRuntimeBundle(store *db.Store, bm *browser.Manager, logger *slog.Logger, cardImages *CardImageStore) (*RuntimeBundle, error) {
	if store == nil {
	...
	// automationSenders 为自动化图片卡密注入远程下载或受管读取、平台上传和 WebSocket 发送链路。
	automationSenders := NewAutomationImageSenderProvider(store, manager, func() mtop.Client { return mtop.NewClient() }, cardImages)
	...
	autoCenter := automation.NewWithDependencies(store, automationSenders, logger, automation.CenterDependencies{
		...
		Notifier:             notifier,
		FlowerReceiveBrowser: flowerReceiveBrowserOrNil(bm),
	})
...
// flowerReceiveBrowserOrNil 把浏览器管理器转换为自动收花窄接口，并保留 nil 接口语义。
func flowerReceiveBrowserOrNil(manager *browser.Manager) automation.FlowerReceiveBrowser {
	if manager == nil {
		return nil
	}
	return manager
