...
// automationManagedImageReader 定义自动发货读取受管图片引用所需的最小本地文件能力。
type automationManagedImageReader interface {
	// ReadReference 读取已由卡券应用服务生成的内部引用，返回值不得包含凭证或业务数据库模型。
	ReadReference(ctx context.Context, reference string) (cardsapp.ImageAsset, error)
}
...
type automationImageSenderProvider struct {
	...
	// managedReader 读取用户拖拽上传的受管图片；为空时远程 URL 发送仍保持可用。
	managedReader automationManagedImageReader
}
...
type automationImageSender struct {
	...
	downloader automationImageDownloader
	// managedReader 只在 imageURL 为受管引用时读取本地不可变文件。
	managedReader automationManagedImageReader
}

// NewAutomationImageSenderProvider 创建自动化中心使用的图片发送器来源。
// 图片 URL 不持久化为本地文件，下载和上传都限定在单次自动化动作的上下文内。
func NewAutomationImageSenderProvider(store *db.Store, manager *accountmanager.Manager, clientProvider func() mtop.Client, managedReader automationManagedImageReader) automation.SenderProvider {
	...
	return automationImageSenderProvider{
		manager: manager, uploader: uploader, downloader: downloadAutomationImage, managedReader: managedReader,
	}
}
...
func (p automationImageSenderProvider) Sender(accountID string) (automation.MessageSender, bool) {
	...
	return automationImageSender{
		accountID: accountID, sender: sender, uploader: p.uploader, downloader: p.downloader, managedReader: p.managedReader,
	}, true
}
...
// SendImage 将远程 URL 下载结果或受管图片内容上传到闲鱼图片服务后，再交给账号运行时发送。
// 图片读取或上传失败发生在 WebSocket 写入之前，必须标记为确定未发送以允许自动化安全重试。
func (s automationImageSender) SendImage(ctx context.Context, chatID, toUserID, imageURL string, cardID int64, _, _ int) error {
	...
	// data、contentType、filename 和 err 分别保存远程或受管图片的瞬时字节、媒体类型、上传名称和读取错误。
	data, contentType, filename, err := s.readImage(ctx, imageURL)
	if err != nil {
		return fmt.Errorf("%w: 读取图片卡密失败: %v", automation.ErrMessageNotSent, err)
	}
...
// readImage 按引用类型选择本地受管文件或原有公网下载器，二者都返回受大小和媒体类型校验的图片字节。
func (s automationImageSender) readImage(ctx context.Context, imageURL string) ([]byte, string, string, error) {
	if cardsapp.IsManagedImageReference(imageURL) {
		if s.managedReader == nil {
			return nil, "", "", cardsapp.ErrImageStoreUnavailable
		}
		// asset、readErr 分别保存受管图片内容和本地文件读取错误。
		asset, readErr := s.managedReader.ReadReference(ctx, imageURL)
		if readErr != nil {
			return nil, "", "", readErr
		}
		return asset.Data, asset.ContentType, managedAutomationUploadFilename(asset.ContentType), nil
	}
	return s.downloader(ctx, imageURL)
}

// managedAutomationUploadFilename 为受管图片生成短平台文件名，避免内容哈希存储名触发闲鱼文件名长度限制。
func managedAutomationUploadFilename(contentType string) string {
	// normalizedType 是去除参数并转为小写后的真实图片媒体类型。
