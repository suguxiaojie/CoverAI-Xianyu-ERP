...
// BindPlatformMessageID 满足领域订阅测试的平台 ID 绑定能力。
func (subscriptionDomainRepository) BindPlatformMessageID(context.Context, string, string, string) (*db.ChatMessage, error) {
	return &db.ChatMessage{}, nil
}

// MarkMessageRecalled 满足领域订阅测试的撤回状态能力。
func (subscriptionDomainRepository) MarkMessageRecalled(context.Context, string, string, int, string, int64) (*db.ChatMessage, error) {
	return &db.ChatMessage{}, nil
}
...
// LatestReadState 为订阅测试返回没有本地消息的状态。
func (subscriptionDomainRepository) LatestReadState(context.Context, string, string) (db.ChatLatestReadState, error) {
...
func TestChatRepositoryMapsSessionMaintenance(t *testing.T) {
	...
	// searchPort 是经类型断言确认的历史消息会话搜索端口。
	searchPort, searchSupported := repository.(chatapp.SessionSearchRepository)
	if !searchSupported {
		t.Fatal("聊天适配器未覆盖 SessionSearchRepository")
	}
	// searched、searchErr 是按买家摘要搜索并转换后的应用会话。
	searched, searchErr := searchPort.SearchSessions(ctx, owner.ID, "cid", "买家", 20)
	if searchErr != nil || len(searched) != 1 || searched[0].ChatID != "chat-1" || searched[0].BuyerName != "买家" {
		t.Fatalf("会话搜索映射异常 searched=%+v err=%v", searched, searchErr)
	}
	if // pinErr 是通过应用端口持久化测试会话置顶偏好的错误。
	pinErr := port.SetSessionPinned(ctx, owner.ID, "cid", "chat-1", true, 1000); pinErr != nil {
		t.Fatal(pinErr)
	}
	// pinnedSessions、pinnedListErr 是置顶后重新读取的应用会话列表和映射错误。
	pinnedSessions, pinnedListErr := port.ListSessions(ctx, owner.ID, "cid", 20)
	if pinnedListErr != nil || len(pinnedSessions) != 1 || !pinnedSessions[0].IsPinned {
		t.Fatalf("会话置顶映射异常 sessions=%+v err=%v", pinnedSessions, pinnedListErr)
	}
