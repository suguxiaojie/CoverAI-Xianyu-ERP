...
func (r *ItemSyncRepository) saveItems(ctx context.Context, cookieID string, items []mtop.ItemListItem) int {
	...
	for _, item := range items {
		...
		if item.SKUDetailLoaded {
			// multiErr 保存权威多规格标记写入错误。
			if multiErr := r.store.Items.SetMultiSpec(ctx, cookieID, item.ID, item.IsMultiSpec); multiErr != nil {
				r.logger.Warn("保存商品多规格状态失败", "cookie_id", cookieID, "item_id", item.ID, "err", multiErr)
			}
			// skuErr 保存当前商品 SKU 快照写入错误；本地成本由数据库 upsert 明确保留。
			if skuErr := r.store.Items.SyncSKUs(ctx, cookieID, item.ID, itemSKURows(cookieID, item)); skuErr != nil {
				r.logger.Warn("保存商品 SKU 失败", "cookie_id", cookieID, "item_id", item.ID, "err", skuErr)
			}
		} else if item.IsMultiSpec {
			// multiErr 保存降级识别到的多规格标记写入错误。
...
func (r *ItemSyncRepository) syncItems(ctx context.Context, cookieID string, items []mtop.ItemListItem) (db.ItemSyncResult, error) {
	// records 保存待在一个事务中写入的商品和可选 SKU 快照。
	records := make([]db.RemoteItemRecord, 0, len(items))
	// item 表示当前待转换的平台商品。
	for _, item := range items {
		// price 保存优先使用格式化价格的商品价格文本。
		price := item.PriceText
		...
		records = append(records, db.RemoteItemRecord{
			Item:            db.ItemInfoRow{CookieID: cookieID, ItemID: item.ID, ItemTitle: item.Title, ItemCategory: item.CategoryID, ItemPrice: price, ItemDetail: item.ItemDetail, IsMultiSpec: item.IsMultiSpec},
			SKUDetailLoaded: item.SKUDetailLoaded, SKUs: itemSKURows(cookieID, item),
		})
	}
	return r.store.Items.SyncFromRemoteRecords(ctx, cookieID, records)
}

// itemSKURows 将平台 SKU 转换为数据库快照；CostCents 留空且数据库同步不会覆盖已有成本。
func itemSKURows(cookieID string, item mtop.ItemListItem) []db.ItemSKURow {
	// rows 保存按平台顺序转换的 SKU。
	rows := make([]db.ItemSKURow, 0, len(item.SKUs))
	// sku 表示当前待转换的平台 SKU。
	for _, sku := range item.SKUs {
		// propertiesJSON 保存规格名称和值的稳定 JSON；序列化失败时使用空数组。
		propertiesJSON, marshalErr := json.Marshal(sku.Properties)
		if marshalErr != nil {
			propertiesJSON = []byte("[]")
		}
		rows = append(rows, db.ItemSKURow{
			CookieID: cookieID, ItemID: item.ID, SKUID: sku.SKUID, InventoryID: sku.InventoryID,
			PropertiesJSON: string(propertiesJSON), PriceCents: sku.PriceCents, Quantity: sku.Quantity,
			InitialQuantity: sku.InitialQuantity, Enabled: sku.Enabled, SortOrder: sku.SortOrder, SyncedAt: item.SKUSyncedAt,
		})
	}
	return rows
}
...
func (r *ItemSyncRepository) enrichMultiSpec(ctx context.Context, cookies, cookieID string, items []mtop.ItemListItem) error {
	// editFetcher、hasEditFetcher 保存卖家编辑详情能力；它是当前库存和完整 SKU 的权威来源。
	editFetcher, hasEditFetcher := r.mtopClient().(mtop.ItemEditDetailFetcher)
	if hasEditFetcher {
		return r.enrichSKUDetails(ctx, cookies, cookieID, items, editFetcher)
	}
...
// enrichSKUDetails 以最多四并发读取卖家商品编辑详情；普通单品失败时保留旧 SKU，会话过期则终止同步。
func (r *ItemSyncRepository) enrichSKUDetails(ctx context.Context, cookies, cookieID string, items []mtop.ItemListItem, fetcher mtop.ItemEditDetailFetcher) error {
	// probeCtx、cancel 控制全部卖家编辑详情请求的共同生命周期。
	probeCtx, cancel := context.WithCancel(ctx)
	defer cancel()
	// semaphore 限制并发详情请求，避免对卖家接口产生突发流量。
	semaphore := make(chan struct{}, 4)
	// waitGroup 等待当前商品批次全部详情请求收束。
	var waitGroup sync.WaitGroup
	// errorMu 保护首个会话过期错误。
	var errorMu sync.Mutex
	// sessionErr 是需要让上层触发账号恢复的会话过期错误。
	var sessionErr error
	// index 表示当前待启动详情请求的商品下标。
	for index := range items {
		waitGroup.Add(1)
		go func(index int) {
			defer waitGroup.Done()
			...
			defer func() { <-semaphore }()
			// detail、fetchErr 是当前商品的卖家 SKU 快照和读取错误。
			detail, fetchErr := fetcher.FetchItemEditDetail(probeCtx, cookies, items[index].ID)
			if fetchErr != nil {
				...
				r.logger.Warn("读取卖家商品 SKU 失败，保留本地旧库存", "cookie_id", cookieID, "item_id", items[index].ID, "err", fetchErr)
				return
			}
			if detail == nil {
				return
			}
			items[index].SKUDetailLoaded = true
			items[index].SKUSyncedAt = detail.SyncedAt
			items[index].SKUs = detail.SKUs
			items[index].IsMultiSpec = len(detail.SKUs) > 1 || itemSKUsContainProperties(detail.SKUs)
		}(index)
	}
	waitGroup.Wait()
	return sessionErr
...
// itemSKUsContainProperties 判断单 SKU 商品是否仍包含需要展示的销售规格。
func itemSKUsContainProperties(skus []mtop.ItemSKU) bool {
	// sku 表示当前待检查的 SKU。
