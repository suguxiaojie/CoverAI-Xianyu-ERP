...
// MiscDependencies 封装通知、订单分析、卡券和知识库应用服务所需的数据库适配器构造能力。
...
// NewMiscDependencies 构造通知、分析、卡券和知识库领域专用依赖，并拒绝缺少数据库入口的半初始化实例。
...
// NewKnowledgeRepository 创建人工知识库、分块和脱敏检索日志适配器。
func (d *MiscDependencies) NewKnowledgeRepository() *KnowledgeRepository {
	if d == nil {
		return nil
	}
	return NewKnowledgeRepository(d.store)
}
