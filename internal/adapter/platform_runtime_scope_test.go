...
func TestOnTokenCaptchaVerificationUsesPlatformRuntimeData(t *testing.T) {
	...
	// browser 是记录人工风控恢复调用参数的浏览器桩；损坏的密码密文不得被平台运行时查询读取。
	browser := &fakeManualBrowser{manualResult: "unb=1; x5sec=recovered;"}
	...
	if !ok || result == nil || browser.manualCalls != 1 || browser.tokenCaptchaCalls != 0 {
		t.Fatalf("token captcha result=%+v ok=%v manual=%d automatic=%d", result, ok, browser.manualCalls, browser.tokenCaptchaCalls)
	}
}
