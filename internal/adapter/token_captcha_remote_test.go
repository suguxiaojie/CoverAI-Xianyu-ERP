...
// TestManualCaptchaModeSkipsConfiguredRemoteService 验证账号选择人工接管后不会读取密钥或调用已配置的远程滑块服务。
func TestManualCaptchaModeSkipsConfiguredRemoteService(t *testing.T) {
	// remoteCalls 统计远程服务收到的请求数；人工模式必须始终保持为零。
	var remoteCalls atomic.Int32
	// remote 是若被错误调用就记录请求的本地远程滑块替身。
	remote := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		remoteCalls.Add(1)
		_, _ = io.WriteString(w, `{"success":true,"data":{"cookies":{"x5sec":"remote-should-not-run"}}}`)
	}))
	defer remote.Close()
	// store、cleanup 是隔离 SQLite 仓储及其释放函数。
	store, cleanup := newAdapterTestStore(t)
	defer cleanup()
	// ctx 是设置人工模式、配置远程服务和执行风控恢复共用的调用 Context。
	ctx := context.Background()
	// showBrowserErr 表示把测试账号切换为人工接管模式时的数据库错误。
	if _, showBrowserErr := store.DB.ExecContext(ctx, `UPDATE cookies SET show_browser=1 WHERE id='cid'`); showBrowserErr != nil {
		t.Fatal(showBrowserErr)
	}
	// settingsErr 表示保存测试用远程滑块地址和非真实密钥时的数据库错误。
	if settingsErr := store.Settings.SetMany(ctx, map[string]string{
		"captcha.remote_service_url": remote.URL,
		"captcha.remote_secret_key":  "test-secret",
	}); settingsErr != nil {
		t.Fatal(settingsErr)
	}
	// manualBrowser 返回人工完成后的测试 Cookie，同时记录自动入口是否被错误调用。
	manualBrowser := &fakeManualBrowser{manualResult: "unb=1; x5sec=manual-fresh"}
	// adapter 是同时具备人工浏览器和远程配置的待测编排器。
	adapter := New(store, nil, nil)
	adapter.SetBrowser(manualBrowser)
	// result、ok 是人工恢复结果和成功标记。
	...
	if remoteCalls.Load() != 0 || manualBrowser.manualCalls != 1 || manualBrowser.tokenCaptchaCalls != 0 {
		t.Fatalf("remote=%d manual=%d automatic=%d", remoteCalls.Load(), manualBrowser.manualCalls, manualBrowser.tokenCaptchaCalls)
	}
}
