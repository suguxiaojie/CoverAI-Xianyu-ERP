...
type TransportApplicationServiceOptions struct {
	...
	// CardImages 保存并读取用户拖拽上传的受管图片；为空时仅保留远程 URL 卡密能力。
	CardImages cardsapp.ImageStore
}
...
type TransportApplicationServices struct {
	...
	// Knowledge 提供人工 FAQ／文档、审核与离线检索用例。
...
func NewTransportApplicationServices(options TransportApplicationServiceOptions) (*TransportApplicationServices, error) {
	...
	services := &TransportApplicationServices{
		...
		Cards:                  cardsapp.NewServiceWithImages(options.MiscDependencies.NewCardsRepository(), options.CardImages),
		...
		Knowledge:              knowledgeapp.NewService(options.MiscDependencies.NewKnowledgeRepository()),
	}
