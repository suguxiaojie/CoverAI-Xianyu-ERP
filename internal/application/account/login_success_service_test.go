...
func TestLoginSuccessServiceSkipsMissingOptionalPorts(t *testing.T) {
	...
	service := NewLoginSuccessService(nil, nil, nil)
...
func TestLoginSuccessServiceRestartsEnabledAccount(t *testing.T) {
	...
	service := NewLoginSuccessService(loginSuccessStatusFake{enabled: true}, runtime, nil)
	service.AfterSuccessfulLogin(context.Background(), 1, "account")
	...
	disabledService := NewLoginSuccessService(loginSuccessStatusFake{}, disabledRuntime, nil)
