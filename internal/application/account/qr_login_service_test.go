...
type fakeQRLoginRepository struct {
	...
	clearCount int
	// modeUpdates 记录显式滑块模式写入次数。
	modeUpdates int
	// showBrowser 保存端口收到的兼容人工模式布尔值。
	showBrowser bool
}
...
// UpdateCaptchaModeOwned 记录账号级自动或人工模式写入。
func (r *fakeQRLoginRepository) UpdateCaptchaModeOwned(_ context.Context, _ int64, _ string, showBrowser bool) error {
	r.modeUpdates++
	r.showBrowser = showBrowser
	return r.modeErr
}
...
// TestQRLoginPersistSuccessPassesExplicitCaptchaMode 验证扫码添加和重新授权都保存用户明确选择的模式。
func TestQRLoginPersistSuccessPassesExplicitCaptchaMode(t *testing.T) {
	// testCases 覆盖自动与人工枚举到兼容 show_browser 字段的映射。
	testCases := []struct {
		// name 是当前模式场景名称。
		name string
		// mode 是扫码请求传入的显式模式。
		mode CaptchaMode
		// showBrowser 是仓储应收到的人工模式布尔值。
		showBrowser bool
	}{
		{name: "自动处理", mode: CaptchaModeAutomatic},
		{name: "弹窗人工", mode: CaptchaModeManual, showBrowser: true},
	}
	// testCase 表示当前待验证的模式映射。
	for _, testCase := range testCases {
		t.Run(testCase.name, func(t *testing.T) {
			// repository 模拟当前用户拥有的既有账号。
			repository := &fakeQRLoginRepository{account: QRLoginAccount{ID: "unb-7", UserID: 7}}
			// service 是待验证显式模式透传的扫码服务。
			service, serviceErr := NewQRLoginService(repository, &fakeQRLoginLifecycle{})
			if serviceErr != nil {
				t.Fatal(serviceErr)
			}
			// _, persistErr 保存当前模式扫码持久化结果。
			_, persistErr := service.PersistSuccess(context.Background(), QRLoginInput{UserID: 7, ScannedAccountID: "unb-7", Cookies: "secret", CaptchaMode: testCase.mode})
			if persistErr != nil || repository.modeUpdates != 1 || repository.showBrowser != testCase.showBrowser {
				t.Fatalf("mode=%s updates=%d show_browser=%v err=%v", testCase.mode, repository.modeUpdates, repository.showBrowser, persistErr)
			}
		})
	}

	// legacyRepository 验证旧客户端省略模式时不会覆盖已有账号配置。
	legacyRepository := &fakeQRLoginRepository{account: QRLoginAccount{ID: "unb-7", UserID: 7}}
	// legacyService 是兼容旧客户端输入的扫码服务。
	legacyService, legacyServiceErr := NewQRLoginService(legacyRepository, &fakeQRLoginLifecycle{})
	if legacyServiceErr != nil {
		t.Fatal(legacyServiceErr)
	}
	// _, legacyErr 保存省略模式的兼容持久化结果。
	_, legacyErr := legacyService.PersistSuccess(context.Background(), QRLoginInput{UserID: 7, ScannedAccountID: "unb-7", Cookies: "secret"})
	if legacyErr != nil || legacyRepository.modeUpdates != 0 {
		t.Fatalf("旧客户端不应覆盖模式: updates=%d err=%v", legacyRepository.modeUpdates, legacyErr)
	}
}

// TestQRLoginPersistRejectsInvalidCaptchaMode 验证未知枚举在任何凭证写入前失败。
func TestQRLoginPersistRejectsInvalidCaptchaMode(t *testing.T) {
	// repository 记录无效模式是否错误触发凭证写入。
	repository := &fakeQRLoginRepository{account: QRLoginAccount{ID: "unb-7", UserID: 7}}
	// service 是待验证枚举校验的扫码服务。
	service, serviceErr := NewQRLoginService(repository, &fakeQRLoginLifecycle{})
	if serviceErr != nil {
		t.Fatal(serviceErr)
	}
	// _, persistErr 保存未知模式的拒绝结果。
	_, persistErr := service.PersistSuccess(context.Background(), QRLoginInput{UserID: 7, ScannedAccountID: "unb-7", Cookies: "secret", CaptchaMode: "unknown"})
	if persistErr == nil || repository.updatedCookies != "" || repository.modeUpdates != 0 {
		t.Fatalf("无效模式未在写入前拒绝: repository=%+v err=%v", repository, persistErr)
	}
}
