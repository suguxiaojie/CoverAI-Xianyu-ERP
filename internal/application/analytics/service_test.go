...
type analyticsRepositoryFake struct {
	...
	// items 保存商品聚合原始记录。
	items []ItemRecord
	// profitRecords 保存订单成本覆盖与商品毛利聚合原始记录。
...
// QueryProfitRecords 返回预置的订单成本覆盖记录。
func (f *analyticsRepositoryFake) QueryProfitRecords(context.Context, Filter) ([]ProfitRecord, error) {
	if f.profitErr != nil {
		return nil, f.profitErr
	}
	return f.profitRecords, f.err
}

// QueryManualCostCandidates 返回空人工候选，具体建议算法由独立测试直接覆盖。
func (f *analyticsRepositoryFake) QueryManualCostCandidates(context.Context, Filter) ([]ManualCostCandidateRecord, error) {
	return nil, f.err
}

// QueryManualHistoricalCostCandidates 返回空的无 SKU 历史成本候选，独立用例会注入具体记录。
func (f *analyticsRepositoryFake) QueryManualHistoricalCostCandidates(context.Context, Filter) ([]ManualHistoricalCostCandidateRecord, error) {
	return nil, f.err
}

// ConfirmManualCostCandidate 返回预置人工确认结果。
func (f *analyticsRepositoryFake) ConfirmManualCostCandidate(context.Context, ManualCostConfirmation) (ManualCostConfirmationResult, error) {
	return ManualCostConfirmationResult{}, f.err
}

// ConfirmManualHistoricalCostCandidate 返回预置错误语义，供纯人工成本分支校验应用路由。
func (f *analyticsRepositoryFake) ConfirmManualHistoricalCostCandidate(context.Context, ManualCostConfirmation) (ManualCostConfirmationResult, error) {
	return ManualCostConfirmationResult{}, f.err
}
...
func TestOrderAnalyticsAggregatesAndConvertsLocalDate(t *testing.T) {
	...
	repository := &analyticsRepositoryFake{
		...
		items:    []ItemRecord{{ItemID: "item-1", ItemTitle: "历史商品", Count: 2, TotalAmount: 10.5, AvgAmount: 5.25}},
		profitRecords: []ProfitRecord{
			{OrderID: "one", ItemID: "item-1", ItemTitle: "历史商品", Amount: "10.50", Status: "completed", CreatedAt: "2026-06-27 16:30:00", UnitCostCents: 300, CostQuantity: 2, Covered: true},
			{OrderID: "two", ItemID: "item-1", ItemTitle: "历史商品", Amount: "8.00", Status: "shipped", CreatedAt: "2026-06-27 17:00:00", Covered: false},
		},
	}
	// service 是待验证的订单分析应用服务。
	service := NewService(repository)
	...
	result, err := service.OrderAnalytics(context.Background(), Query{UserID: 7, AccountID: " account-1 ", StartDate: "2026-06-28", EndDate: "2026-06-28", Location: location})
	if err != nil {
	...
	if len(result.ItemStats) != 1 || result.ItemStats[0].ItemTitle != "历史商品" {
		t.Fatalf("历史商品名称未进入统计结果: %+v", result.ItemStats)
	}
	if result.ProfitStats.CoveredOrders != 1 || result.ProfitStats.UnknownOrders != 1 || result.ProfitStats.CostAmount != 6 || result.ProfitStats.PlatformFee != 0.17 || result.ProfitStats.GrossProfit != 4.33 || result.ProfitStats.CoveragePercent != 50 {
		t.Fatalf("商品毛利统计异常: %+v", result.ProfitStats)
	}
	if len(result.DailyProfitStats) != 1 || result.DailyProfitStats[0].RevenueAmount != 18.5 || len(result.ItemProfitStats) != 1 || result.ItemProfitStats[0].GrossMarginPercent != 41.24 {
		t.Fatalf("商品毛利维度聚合异常: daily=%+v item=%+v", result.DailyProfitStats, result.ItemProfitStats)
	}
	if repository.receivedFilter.UserID != 7 || repository.receivedFilter.AccountID != "account-1" || repository.receivedFilter.StartAt != "2026-06-27 16:00:00" || repository.receivedFilter.EndBefore != "2026-06-28 16:00:00" {
		t.Fatalf("筛选条件异常: %+v", repository.receivedFilter)
	}
	// receivedStatuses 是应用服务复制给所有统计查询的有效成交状态集合。
	receivedStatuses := repository.receivedFilter.Statuses
	// includesReceived 表示有效成交状态中是否包含 Schema 39 的已收货状态。
	includesReceived := false
	// status 是当前检查的有效成交状态候选。
	for _, status := range receivedStatuses {
		if status == "received" {
	...
	if !includesReceived {
		t.Fatalf("已收货状态未进入有效成交统计: %+v", receivedStatuses)
	}
}
...
func TestAnalyticsErrorStagesCoverAllQueries(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "profit", repository: &analyticsRepositoryFake{profitErr: expected}, run: func(service *Service) error {
			// err 是商品毛利查询阶段返回的错误。
			_, err := service.OrderAnalytics(context.Background(), Query{})
			return err
		}, message: "查询商品毛利失败"},
