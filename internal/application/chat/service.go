...
// ErrSessionForbidden 表示当前 ERP 用户不拥有目标闲鱼账号，不得修改其会话偏好。
var ErrSessionForbidden = errors.New("无权修改聊天会话")

// ErrSessionNotFound 表示目标会话不存在或不属于已校验账号。
var ErrSessionNotFound = errors.New("聊天会话不存在")
...
type Message struct {
	...
	// ReplyPreview 是同账号同会话内解析的最小引用目标展示快照。
	...
	SentAt int64
	// RecalledAt 是平台确认撤回的 Unix 毫秒时间；零值表示未撤回。
	RecalledAt int64
	// RecallOperatorType 区分发送者、群主、系统和安全撤回；未撤回时为 -1。
	RecallOperatorType int
	// RecallOperatorID 是平台撤回操作者标识。
	RecallOperatorID string
	// SystemCard 是 contentType=26 交易消息的结构化只读卡片；普通消息为空。
	SystemCard *SystemCard
	// LocationCard 是 contentType=30 的结构化位置卡片；其他消息为空。
	LocationCard *LocationCard
}

// ReplyPreview 是应用层允许向 HTTP 和实时订阅公开的非敏感引用目标摘要模型。
type ReplyPreview struct {
	...
	// Content 是被引用消息的展示内容。
	Content string
	// Status 是被引用消息当前是否已撤回的状态。
	Status string
}

// SystemCard 是聊天应用层允许向前端公开的非敏感交易卡片。
type SystemCard struct {
	// Kind 当前固定为 trade，用于区别普通系统通知。
	Kind string
	// Event 是归一化后的交易状态。
	Event string
	// Title 是平台交易卡片标题。
	...
	Description string
	// OrderID 是从明确平台字段或动作地址解析出的订单标识。
	OrderID string
	// ItemID 是从明确平台字段或动作地址解析出的商品标识。
	ItemID string
	// Action 是白名单动作提示；收花动作只允许前端打开官方页面。
	Action string
}
...
type Repository interface {
	...
	// FindMessage 按用户归属读取单条撤回候选消息。
	FindMessage(ctx context.Context, userID int64, accountID, messageKey string) (Message, error)
}

// SessionSearchRepository 定义账号范围内按摘要和完整历史消息定位会话的窄查询能力。
type SessionSearchRepository interface {
	// SearchSessions 只返回匹配会话摘要，不把命中的消息内容传播到列表响应。
	SearchSessions(ctx context.Context, userID int64, accountID, search string, limit int) ([]Session, error)
}
...
type SessionRepository interface {
	...
	// SetSessionPinned 按用户归属幂等设置精确会话的 ERP 本地置顶偏好。
	SetSessionPinned(ctx context.Context, userID int64, accountID, chatID string, pinned bool, pinnedAt int64) error
}
...
type Service struct {
	...
	// credit 负责当前会话按需信用查询、二十四小时缓存、全局串行限速和风控熔断。
	credit *creditCoordinator
	...
	// now 是撤回窗口测试可替换的时钟；生产为空时使用系统 UTC 时间。
	now func() time.Time
}
...
// SearchSessions 校验用户、账号和关键词后，在本地完整历史消息中定位匹配会话。
func (s *Service) SearchSessions(ctx context.Context, userID int64, accountID, search string, limit int) ([]Session, error) {
	accountID, search = strings.TrimSpace(accountID), strings.TrimSpace(search)
	if s == nil || s.repository == nil || userID <= 0 || accountID == "" || search == "" || len([]rune(search)) > 200 {
		return nil, ErrInvalidInput
	}
	// searchRepository、supported 是历史会话搜索仓储和能力装配状态。
	searchRepository, supported := s.repository.(SessionSearchRepository)
	if !supported {
		return nil, ErrSessionUnavailable
	}
	return searchRepository.SearchSessions(ctx, userID, accountID, search, limit)
}

// SetSessionPinned 校验当前用户的账号归属后，幂等更新精确会话的 ERP 本地置顶偏好。
func (s *Service) SetSessionPinned(ctx context.Context, userID int64, accountID, chatID string, pinned bool) error {
	accountID = strings.TrimSpace(accountID)
	chatID = strings.TrimSpace(chatID)
	if s == nil || s.repository == nil || userID <= 0 || accountID == "" || chatID == "" {
		return ErrInvalidInput
	}
	// repository 是当前同时提供账号归属和会话偏好写入的窄仓储端口。
	repository, supported := s.repository.(SessionRepository)
	if !supported {
		return ErrSessionUnavailable
	}
	// owned、ownershipErr 是不解密任何凭证的账号归属结果及仓储错误。
	owned, ownershipErr := repository.ExistsOwned(ctx, userID, accountID)
	if ownershipErr != nil {
		return ownershipErr
	}
	if !owned {
	...
	// pinnedAt 是本次从未置顶转为置顶时使用的 Unix 秒；取消时保持零值。
	var pinnedAt int64
	if pinned {
		pinnedAt = time.Now().UTC().Unix()
	}
	return repository.SetSessionPinned(ctx, userID, accountID, chatID, pinned, pinnedAt)
}
