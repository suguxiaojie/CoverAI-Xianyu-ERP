...
type fakeRepository struct {
	...
	// owned 是模拟账号归属查询结果。
	owned bool
	// ownedAccounts 在信用跨店铺测试中按账号标识返回不同归属；为空时继续使用 owned。
	ownedAccounts map[string]bool
	// ownedErr 是模拟账号归属查询失败的错误。
	ownedErr error
	// updatedSessions 保存最近写入的会话身份，供断言应用端口调用参数。
	updatedSessions []Session
	// searchSessions 保存历史消息搜索需要返回的会话摘要。
	searchSessions []Session
	// searchedAccountID、searchedText 和 searchedLimit 保存最近一次搜索输入。
	...
	markReadCalls int
	// pinErr 是会话置顶写入端口需要返回的预设错误。
	pinErr error
	// pinCalls 记录归属校验通过后的会话置顶写入次数。
	pinCalls int
	// pinnedAccountID、pinnedChatID、pinned和pinnedAt 保存最近一次置顶写入的非敏感输入。
...
// SearchSessions 返回预设搜索结果并记录账号、关键词和限制。
func (r *fakeRepository) SearchSessions(_ context.Context, _ int64, accountID, search string, limit int) ([]Session, error) {
	r.searchedAccountID, r.searchedText, r.searchedLimit = accountID, search, limit
	return r.searchSessions, r.sessionErr
}

// FindMessage 返回测试仓储中的首条消息，供撤回用例归属查询复用。
func (r *fakeRepository) FindMessage(_ context.Context, userID int64, _, _ string) (Message, error) {
	r.userID = userID
	if len(r.messages) == 0 {
		return Message{}, ErrRecallNotFound
	}
	return r.messages[0], r.messageErr
}
...
func (r *fakeRepository) ExistsOwned(_ context.Context, _ int64, accountID string) (bool, error) {
	if r.ownedAccounts != nil {
		return r.ownedAccounts[accountID], r.ownedErr
	}
...
// SetSessionPinned 记录应用服务下发的会话置顶输入并返回预设错误。
func (r *fakeRepository) SetSessionPinned(_ context.Context, _ int64, accountID, chatID string, pinned bool, pinnedAt int64) error {
	r.pinCalls++
	r.pinnedAccountID = accountID
	r.pinnedChatID = chatID
	r.pinned = pinned
	r.pinnedAt = pinnedAt
	return r.pinErr
}
...
// FindMessage 返回不可用错误，满足只读历史仓储接口。
func (historyOnlyRepository) FindMessage(context.Context, int64, string, string) (Message, error) {
	return Message{}, ErrRecallNotFound
}
...
// TestSearchSessionsValidatesAndDelegatesHistoricalQuery 验证历史会话搜索规范化输入并使用窄仓储端口。
func TestSearchSessionsValidatesAndDelegatesHistoricalQuery(t *testing.T) {
	// repository 是记录搜索参数并返回目标会话的测试仓储。
	repository := &fakeRepository{searchSessions: []Session{{AccountID: "account-1", ChatID: "chat-history"}}}
	// service 是当前搜索用例应用服务。
	service := New(repository)
	// sessions、searchErr 是去除空白后得到的历史搜索结果和错误。
	sessions, searchErr := service.SearchSessions(context.Background(), 7, " account-1 ", " TEST-CARD-CODE-001 ", 25)
	if searchErr != nil || len(sessions) != 1 || sessions[0].ChatID != "chat-history" || repository.searchedAccountID != "account-1" || repository.searchedText != "TEST-CARD-CODE-001" || repository.searchedLimit != 25 {
		t.Fatalf("sessions=%+v account=%q search=%q limit=%d err=%v", sessions, repository.searchedAccountID, repository.searchedText, repository.searchedLimit, searchErr)
	}
	// tooLong 是超过应用层二百字符边界的搜索词。
	tooLong := strings.Repeat("长", 201)
	if // _, invalidErr 是超长搜索必须返回的输入错误。
	_, invalidErr := service.SearchSessions(context.Background(), 7, "account-1", tooLong, 20); !errors.Is(invalidErr, ErrInvalidInput) {
		t.Fatalf("invalid error=%v", invalidErr)
	}
	// unavailable 使用未实现历史搜索窄端口的仓储替身。
	unavailable := New(historyOnlyRepository{})
	if // _, unavailableErr 是搜索能力未装配时的明确错误。
	_, unavailableErr := unavailable.SearchSessions(context.Background(), 7, "account-1", "token", 20); !errors.Is(unavailableErr, ErrSessionUnavailable) {
...
// TestSetSessionPinnedValidatesOwnershipAndNormalizesState 验证置顶用例隔离账号归属、规范标识并正确生成置顶时间。
func TestSetSessionPinnedValidatesOwnershipAndNormalizesState(t *testing.T) {
	// repository 是归属成功且记录写入参数的会话仓储替身。
	repository := &fakeRepository{owned: true}
	// service 是待验证的聊天应用服务。
	service := New(repository)
	if // pinErr 是启用置顶返回的应用错误。
	pinErr := service.SetSessionPinned(context.Background(), 7, " account-1 ", " chat-1 ", true); pinErr != nil {
		t.Fatal(pinErr)
	}
	if repository.pinCalls != 1 || repository.pinnedAccountID != "account-1" || repository.pinnedChatID != "chat-1" || !repository.pinned || repository.pinnedAt <= 0 {
		t.Fatalf("pin calls=%d account=%q chat=%q pinned=%v at=%d", repository.pinCalls, repository.pinnedAccountID, repository.pinnedChatID, repository.pinned, repository.pinnedAt)
	}
	if // unpinErr 是取消置顶返回的应用错误。
	unpinErr := service.SetSessionPinned(context.Background(), 7, "account-1", "chat-1", false); unpinErr != nil {
		t.Fatal(unpinErr)
	}
	if repository.pinCalls != 2 || repository.pinned || repository.pinnedAt != 0 {
		t.Fatalf("unpin calls=%d pinned=%v at=%d", repository.pinCalls, repository.pinned, repository.pinnedAt)
	}
	// foreignRepository 模拟当前用户不拥有目标账号。
	foreignRepository := &fakeRepository{owned: false}
	if // forbiddenErr 是越权置顶应返回的稳定应用错误。
	forbiddenErr := New(foreignRepository).SetSessionPinned(context.Background(), 7, "account-1", "chat-1", true); !errors.Is(forbiddenErr, ErrSessionForbidden) || foreignRepository.pinCalls != 0 {
		t.Fatalf("forbidden err=%v calls=%d", forbiddenErr, foreignRepository.pinCalls)
	}
	if // invalidErr 是缺少会话标识时的输入错误。
	invalidErr := service.SetSessionPinned(context.Background(), 7, "account-1", "", true); !errors.Is(invalidErr, ErrInvalidInput) {
		t.Fatalf("invalid err=%v", invalidErr)
	}
	if // unavailableErr 是只读仓储未实现置顶端口时的能力错误。
	unavailableErr := New(historyOnlyRepository{}).SetSessionPinned(context.Background(), 7, "account-1", "chat-1", true); !errors.Is(unavailableErr, ErrSessionUnavailable) {
		t.Fatalf("unavailable err=%v", unavailableErr)
	}
}
