...
type CatalogMutationRepository interface {
	...
	// UpdateSKUCost 更新一个 SKU 的本地单件成本，不触发平台调用。
	UpdateSKUCost(context.Context, string, string, string, *int64) error
	// UpdateSKUCosts 原子保存一个商品的全部本地 SKU 成本。
	UpdateSKUCosts(context.Context, string, string, []SKUCostInput) error
}

// SKUCostInput 是本地成本批量保存中的单个 SKU 分值。
type SKUCostInput struct {
	// SKUID 是平台 SKU 或单规格商品的本地隐式 SKU 标识。
	SKUID string
	// CostCents 是可空人民币分值；nil 表示清除。
	CostCents *int64
}
...
// UpdateSKUCost 保存 SKU 的本地单件成本；nil 表示清除，非 nil 值使用人民币分且不得为负数。
func (service *CatalogMutationService) UpdateSKUCost(ctx context.Context, cookieID, itemID, skuID string, costCents *int64) error {
	return service.UpdateSKUCosts(ctx, cookieID, itemID, []SKUCostInput{{SKUID: skuID, CostCents: costCents}})
}

// UpdateSKUCosts 校验并原子保存商品全部本地成本；平台库存和售价不在该用例中修改。
func (service *CatalogMutationService) UpdateSKUCosts(ctx context.Context, cookieID, itemID string, costs []SKUCostInput) error {
	if service == nil || service.repository == nil {
		return errors.New("商品写入服务未初始化")
	}
	if strings.TrimSpace(cookieID) == "" || strings.TrimSpace(itemID) == "" || len(costs) == 0 {
		return errors.New("商品 SKU 标识不能为空")
	}
	// seen 保存当前请求内已经出现的 SKU，防止同一成本重复定义。
	seen := make(map[string]struct{}, len(costs))
	// cost 表示当前待校验的 SKU 成本。
	for _, cost := range costs {
		// skuID 是规范化后的 SKU 标识。
		skuID := strings.TrimSpace(cost.SKUID)
		if skuID == "" || cost.CostCents != nil && *cost.CostCents < 0 {
			return errors.New("SKU 成本参数无效")
		}
		// duplicated 表示当前请求已定义过同一 SKU 成本。
		if _, duplicated := seen[skuID]; duplicated {
			return errors.New("SKU 成本重复")
		}
		seen[skuID] = struct{}{}
	}
	return service.repository.UpdateSKUCosts(ctx, cookieID, itemID, costs)
}
