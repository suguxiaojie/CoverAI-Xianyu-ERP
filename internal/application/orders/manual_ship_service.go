...
type ManualShipRepository interface {
	...
	// MarkOrderShippedAt 写入明确远端发货成功的时间事实。
...
type ManualShipRuntime interface {
	...
	// ScheduleRedFlowerAfterShipment 在本地发货事实完整后创建秒级求花任务。
...
func (s *ManualShipService) manualStatusShip(ctx context.Context, userID int64, order *Order, orderID string, result *ManualShipResult) {
	...
	if upsertErr == nil {
		upsertErr = s.repository.MarkOrderShippedAt(ctx, orderID)
	}
	if upsertErr != nil {
	...
	} else if // scheduleErr 是秒级求花任务创建失败原因，发货成功事实不因此回退。
	scheduleErr := s.runtime.ScheduleRedFlowerAfterShipment(ctx, orderID); scheduleErr != nil {
		message = "闲鱼已确认发货；自动求花任务将由分钟补偿扫描恢复"
		reconciliationWarning = scheduleErr.Error()
	}
