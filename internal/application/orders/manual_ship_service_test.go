...
type manualShipRepositoryFake struct {
	...
	// markShippedCalls 统计明确发货时间写入次数。
	markShippedCalls int
	// markShippedErr 保存发货时间写入错误。
...
// MarkOrderShippedAt 记录明确发货时间写入。
func (f *manualShipRepositoryFake) MarkOrderShippedAt(context.Context, string) error {
	f.markShippedCalls++
...
type manualShipRuntimeFake struct {
	...
	// scheduleCalls 统计发货成功后自动求花调度次数。
	scheduleCalls int
	// scheduleErr 保存自动求花任务创建错误。
	scheduleErr error
}
...
// ScheduleRedFlowerAfterShipment 记录发货成功后的自动求花调度。
func (f *manualShipRuntimeFake) ScheduleRedFlowerAfterShipment(context.Context, string) error {
	f.scheduleCalls++
...
func TestManualShipServiceStatusSuccess(t *testing.T) {
	...
	if runtime.updatedCookie != "new-cookie" || repository.upsertID != "order-1" || repository.upsertOptions.OrderStatus != "shipped" || repository.markShippedCalls != 1 || runtime.scheduleCalls != 1 {
...
// TestManualShipServiceShipmentTimeFailureSkipsFlowerScheduling 验证发货时间事实写入失败时进入补偿且不创建求花任务。
func TestManualShipServiceShipmentTimeFailureSkipsFlowerScheduling(t *testing.T) {
	// repository 返回发货时间事实写入失败，但订单状态写入成功。
	repository := &manualShipRepositoryFake{ownedIDs: []string{"cookie-1"}, markShippedErr: errors.New("发货时间写入失败"), orders: map[string]*Order{
		"order-1": {OrderID: "order-1", CookieID: "cookie-1", ItemID: "item-1", OrderStatus: "pending_ship"},
	}}
	// runtime 返回远端确认发货成功和可用补偿记录标识。
	runtime := &manualShipRuntimeFake{mtopAvailable: true, consign: ConsignResult{Success: true}, reconciliationID: "reconcile-shipped-at"}
	// result、runErr 是手动确认发货的三态结果和错误。
	result, runErr := NewManualShipService(repository, runtime).ManualShip(context.Background(), ManualShipRequest{UserID: 7, OrderIDs: []string{"order-1"}})
	if runErr != nil || result.Results[0].Status != "reconciliation_required" || runtime.scheduleCalls != 0 {
		t.Fatalf("result=%+v runtime=%+v err=%v", result, runtime, runErr)
	}
}

// TestManualShipServiceScheduleFailureKeepsShipmentSuccess 验证求花任务创建失败不会回退已经明确成功的发货结果。
func TestManualShipServiceScheduleFailureKeepsShipmentSuccess(t *testing.T) {
	// repository 完整保存订单状态和发货时间。
	repository := &manualShipRepositoryFake{ownedIDs: []string{"cookie-1"}, orders: map[string]*Order{
		"order-1": {OrderID: "order-1", CookieID: "cookie-1", ItemID: "item-1", OrderStatus: "pending_ship"},
	}}
	// runtime 模拟秒级求花任务创建失败，分钟补偿扫描随后可以恢复。
	runtime := &manualShipRuntimeFake{mtopAvailable: true, consign: ConsignResult{Success: true}, scheduleErr: errors.New("调度存储暂不可用")}
	// result、runErr 是手动确认发货的成功结果和错误。
	result, runErr := NewManualShipService(repository, runtime).ManualShip(context.Background(), ManualShipRequest{UserID: 7, OrderIDs: []string{"order-1"}})
	if runErr != nil || result.Results[0].Status != "succeeded" || !strings.Contains(result.Results[0].ReconciliationWarning, "调度存储暂不可用") || runtime.scheduleCalls != 1 {
		t.Fatalf("result=%+v runtime=%+v err=%v", result, runtime, runErr)
	}
}
