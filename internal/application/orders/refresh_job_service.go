...
func (service *RefreshJobService) CreateAndStart(requestCtx, lifecycleCtx context.Context, userID int64, cookieID, status string) (RefreshJobStartResult, error) {
	return service.CreateAndStartInMode(requestCtx, lifecycleCtx, userID, cookieID, status, RefreshModeIncremental)
}

// CreateAndStartInMode 创建带同步范围的可恢复后台任务；full 以外的值统一按 incremental 保存。
func (service *RefreshJobService) CreateAndStartInMode(requestCtx, lifecycleCtx context.Context, userID int64, cookieID, status, mode string) (RefreshJobStartResult, error) {
	...
	job := &RefreshJob{ID: service.options.NewJobID(), UserID: userID, CookieID: cookieID, FilterStatus: status, SyncMode: normalizeRefreshMode(mode)}
