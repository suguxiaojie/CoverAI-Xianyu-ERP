...
func TestRefreshJobServiceCreateAndStart(t *testing.T) {
	...
	result, startErr := service.CreateAndStartInMode(context.Background(), context.Background(), 7, "cookie-1", "pending_ship", RefreshModeFull)
	if startErr != nil {
	...
	if len(createdJobs) != 1 || createdJobs[0].UserID != 7 || createdJobs[0].SyncMode != RefreshModeFull || len(claimCalls) != 1 || claimCalls[0].leaseExpiresAt != fixedNow.Add(time.Minute).Unix() {
