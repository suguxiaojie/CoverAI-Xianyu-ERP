...
// RefreshJobProgressRefresher 是支持逐阶段进度上报的订单刷新能力；旧实现仍可只实现 Refresh。
type RefreshJobProgressRefresher interface {
	// RefreshWithProgress 执行订单刷新，并在账号发现和逐单详情处理后发布轻量进度。
	RefreshWithProgress(context.Context, int64, string, string, RefreshJobProgressReporter) (RefreshResult, error)
}

// RefreshJobModeRefresher 是支持任务持久化同步模式的刷新能力。
type RefreshJobModeRefresher interface {
	// RefreshInMode 按任务创建时保存的增量或全量模式执行刷新。
	RefreshInMode(context.Context, int64, string, string, string) (RefreshResult, error)
}

// RefreshJobModeProgressRefresher 是同时支持同步模式和逐阶段进度的刷新能力。
type RefreshJobModeProgressRefresher interface {
	// RefreshWithProgressInMode 按任务同步模式执行刷新并发布轻量进度。
	RefreshWithProgressInMode(context.Context, int64, string, string, string, RefreshJobProgressReporter) (RefreshResult, error)
}
...
type RefreshJobResult struct {
	// Progress 保存运行中或完成后的轻量进度；客户端不必等待完整 Results 才能更新进度条。
	Progress RefreshJobProgress `json:"progress"`
...
type RefreshJobSummary struct {
	...
	// ConflictSkipped 是保持其他账号所有权而安全跳过的订单数量。
...
func NewRefreshJobResult(result RefreshResult) RefreshJobResult {
	...
	for _, item := range result.Results {
		...
		if item.CookieID != "" {
			...
			// conflictSkipped 保存账号发现阶段隔离的冲突数量，即使为零也按账号结果显式持久化。
			conflictSkipped := item.ConflictSkipped
			converted.ConflictSkipped = &conflictSkipped
			if item.Success {
	...
	// completedMessage 根据业务结果区分完整成功和部分完成，任务 succeeded 只表示 worker 已正常收束。
	completedMessage := "订单同步完成"
	if result.PartialFailure {
		completedMessage = "订单同步部分完成，请查看失败详情"
	}
	return RefreshJobResult{
		Progress: RefreshJobProgress{
			Stage: "completed", Message: completedMessage, Processed: result.Summary.Total,
			Total: result.Summary.Total, Succeeded: result.Summary.Updated + result.Summary.NoChange,
			Failed: result.Summary.Failed, Percent: 100, CurrentAccount: result.Summary.AccountTotal, TotalAccounts: result.Summary.AccountTotal,
		},
...
func (runner *RefreshJobRunner) RunJob(ctx context.Context, job *RefreshJob, token string) error {
	...
	// reporter 保存当前 worker 的进度持久化闭包；单次失败后停止继续写进度，但不终止实际订单刷新。
	reporter := runner.progressReporter(ctx, job.ID, token)
	// result 保存刷新业务结果，后续编码为最终完整任务响应。
	var result RefreshResult
	// err 保存刷新业务或进度实现返回的错误。
	var err error
	// progressRefresher、supportsProgress 表示刷新服务是否支持逐阶段进度；旧实现继续走一次性兼容路径。
	modeProgressRefresher, supportsModeProgress := runner.refresher.(RefreshJobModeProgressRefresher)
	// modeRefresher、supportsMode 表示刷新服务是否支持无进度的模式化调用。
	modeRefresher, supportsMode := runner.refresher.(RefreshJobModeRefresher)
	// progressRefresher、supportsProgress 表示刷新服务是否至少支持旧逐阶段进度接口。
	progressRefresher, supportsProgress := runner.refresher.(RefreshJobProgressRefresher)
	if supportsModeProgress {
		result, err = modeProgressRefresher.RefreshWithProgressInMode(ctx, job.UserID, job.CookieID, job.FilterStatus, job.SyncMode, reporter)
	} else if supportsMode {
		result, err = modeRefresher.RefreshInMode(ctx, job.UserID, job.CookieID, job.FilterStatus, job.SyncMode)
	} else if supportsProgress {
		result, err = progressRefresher.RefreshWithProgress(ctx, job.UserID, job.CookieID, job.FilterStatus, reporter)
	} else {
...
// progressReporter 把应用层进度编码到任务 result_json；租约失配或数据库失败后停止继续写进度并报告一次异步错误。
func (runner *RefreshJobRunner) progressReporter(ctx context.Context, jobID, token string) RefreshJobProgressReporter {
	// disabled 表示本轮进度持久化已经失败，后续业务进度不再重复访问数据库或刷屏告警。
	disabled := false
	return func(progress RefreshJobProgress) {
		if disabled {
			return
		}
		// payload 是只包含 Progress 和空结果集合的稳定任务 JSON，不暴露逐单结果。
		payload := RefreshJobResult{Progress: progress, Results: []RefreshJobResultItem{}}
		// encoded、marshalErr 保存进度 JSON 和编码错误。
		encoded, marshalErr := runner.options.MarshalResult(payload)
		if marshalErr != nil {
			disabled = true
			runner.reportProgressError(jobID, fmt.Errorf("编码订单刷新进度: %w", marshalErr))
			return
		}
		// applied、updateErr 保存进度是否命中当前租约和数据库错误。
		applied, updateErr := runner.repository.UpdateProgress(ctx, jobID, token, string(encoded))
		if updateErr != nil || !applied {
			disabled = true
			if updateErr == nil {
				updateErr = ErrRefreshJobCompletionNotApplied
			}
			runner.reportProgressError(jobID, fmt.Errorf("写入订单刷新进度: %w", updateErr))
		}
	}
}

// reportProgressError 把进度持久化错误交给现有 worker 观测回调，不影响实际订单刷新和最终结果。
func (runner *RefreshJobRunner) reportProgressError(jobID string, err error) {
	if runner.options.OnWorkerError != nil {
		runner.options.OnWorkerError(jobID, err)
	}
}
