...
// refreshRunnerProgressRefresher 在返回最终结果前发布两次确定性进度，用于验证 runner 持久化轻量快照。
type refreshRunnerProgressRefresher struct{}

// Refresh 满足兼容刷新接口；支持进度的 runner 应优先调用 RefreshWithProgress。
func (refreshRunnerProgressRefresher) Refresh(context.Context, int64, string, string) (RefreshResult, error) {
	return RefreshResult{}, errors.New("不应调用兼容刷新入口")
}

// RefreshWithProgress 发布发现和详情进度后返回固定最终统计。
func (refreshRunnerProgressRefresher) RefreshWithProgress(_ context.Context, _ int64, _, _ string, reporter RefreshJobProgressReporter) (RefreshResult, error) {
	reporter(RefreshJobProgress{Stage: "discovering", Message: "正在从闲鱼发现订单", Processed: 1, Total: 2, Succeeded: 1, Percent: 15})
	reporter(RefreshJobProgress{Stage: "syncing_details", Message: "正在逐单同步订单详情", Processed: 3, Total: 5, Succeeded: 2, Failed: 1, Percent: 69})
	return RefreshResult{Summary: RefreshSummary{Total: 5, Updated: 2, NoChange: 2, Failed: 1}}, nil
}
...
type refreshRunnerTestRepository struct {
	...
	// progressCalls 保存运行中轻量进度 JSON，顺序与 reporter 调用一致。
...
// UpdateProgress 记录运行中进度 JSON，并沿用 completeApplied 控制租约是否命中。
func (repository *refreshRunnerTestRepository) UpdateProgress(_ context.Context, _, _ string, progressJSON string) (bool, error) {
	repository.mu.Lock()
	defer repository.mu.Unlock()
	repository.progressCalls = append(repository.progressCalls, progressJSON)
	return repository.completeApplied, repository.completeErr
}
...
func TestNewRefreshJobResultUsesStableSnakeCase(t *testing.T) {
	...
	if string(encoded) != `{"progress":{"stage":"completed","message":"订单同步完成","processed":0,"total":0,"succeeded":0,"failed":0,"percent":100},"partial_failure":false,"message":"","summary":{"account_total":0,"account_succeeded":0,"account_failed":0,"discovered":0,"list_updated":0,"soft_deleted":0,"detail_total":0,"total":0,"updated":0,"no_change":0,"failed":0,"conflict_skipped":0},"results":[]}` {
		t.Fatalf("刷新结果 JSON 不兼容: %s", encoded)
	}

	// source 是包含账号和订单结果的应用刷新结果。
	source := RefreshResult{Results: []RefreshOrderResult{{CookieID: "cookie-1", Success: true, Discovered: 2, Updated: 3, SoftDeleted: 1, ConflictSkipped: 4}, {OrderID: "order-1", Stage: "detail"}}}
	// converted 是带可选字段的任务结果模型。
	converted := NewRefreshJobResult(source)
	if converted.Results[0].Discovered == nil || converted.Results[0].SoftDeleted == nil || converted.Results[0].ConflictSkipped == nil || *converted.Results[0].ConflictSkipped != 4 || converted.Results[1].Discovered != nil {
	...
	// partial 是 worker 成功收束但业务存在失败的结果，必须保留账号维度和部分完成文案。
	partial := NewRefreshJobResult(RefreshResult{PartialFailure: true, Message: "1 项失败", Summary: RefreshSummary{AccountTotal: 2, AccountSucceeded: 1, AccountFailed: 1, Total: 1, NoChange: 1, Failed: 1}})
	if partial.Progress.Message != "订单同步部分完成，请查看失败详情" || partial.Progress.CurrentAccount != 2 || partial.Progress.TotalAccounts != 2 || partial.Summary.AccountFailed != 1 {
...
// TestRefreshJobRunnerPersistsIncrementalProgress 验证支持进度的刷新器逐次写入轻量 JSON，最终仍写完整 succeeded 结果。
func TestRefreshJobRunnerPersistsIncrementalProgress(t *testing.T) {
	// repository 保存进度和终态调用记录。
	repository := completeAppliedRepository()
	// runner、constructErr 保存进度刷新器对应的运行器和构造错误。
	runner, constructErr := NewRefreshJobRunner(repository, refreshRunnerProgressRefresher{}, RefreshJobRunnerOptions{})
	if constructErr != nil {
		t.Fatal(constructErr)
	}
	// job 是本次进度持久化测试的任务快照。
	job := &RefreshJob{ID: "job-progress", UserID: 7}
	// runErr 表示执行带增量进度任务时的运行或终态写入错误。
	if runErr := runner.RunJob(context.Background(), job, "token-progress"); runErr != nil {
		t.Fatal(runErr)
	}
	repository.mu.Lock()
	// progressCalls 是锁内复制出的进度记录，解锁后用于断言。
	progressCalls := append([]string(nil), repository.progressCalls...)
	// completionCalls 是锁内复制出的终态记录，解锁后用于断言。
	completionCalls := append([]refreshRunnerCompleteCall(nil), repository.completeCalls...)
	repository.mu.Unlock()
	if len(progressCalls) != 2 || !strings.Contains(progressCalls[1], `"processed":3`) || strings.Contains(progressCalls[1], `"order_id"`) {
		t.Fatalf("progress calls=%v", progressCalls)
	}
	if len(completionCalls) != 1 || completionCalls[0].status != "succeeded" || !strings.Contains(completionCalls[0].resultJSON, `"percent":100`) {
		t.Fatalf("completion calls=%+v", completionCalls)
	}
}
