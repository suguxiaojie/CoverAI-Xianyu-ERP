...
func NormalizeOrderStatus(status string) string {
	...
	case "买家已确认收货", "已收货", "确认收货成功":
		return "received"
	case "4", "11", "交易成功", "交易完成", "已完成":
		return "completed"
	case "5", "7", "9", "退款中", "退款申请中":
		return "refunding"
	case "退款成功", "已退款":
		return "refunded"
	case "6", "8", "10", "12", "交易关闭", "已关闭":
		return "cancelled"
	case "退款关闭":
		return "unknown"
	case "":
...
func ValidEditableOrderStatus(status string) bool {
	switch status {
	case "processing", "pending_ship", "shipped", "received", "completed", "cancelled", "refunding", "refunded":
		return true
	default:
		return false
	}
