...
type UpsertOptions struct {
	...
	// ChatID 是聊天会话标识。
	ChatID string
	// CreatedAt 是平台订单创建时间；为空时保留数据库现值。
	CreatedAt string
	// IsBargain 表示是否为砍价订单。
	IsBargain *bool
	// SystemShipped 表示是否由系统完成发货。
	SystemShipped *bool
	// ReceivedAt 是买家明确确认收货的 UTC RFC3339 时间。
	ReceivedAt string
	// CompletedAt 是平台明确交易完成的 UTC RFC3339 时间。
	CompletedAt string
	// RefundedAt 是平台明确退款成功的 UTC RFC3339 时间。
	RefundedAt string
	// CancelledAt 是未付款取消或普通关闭的 UTC RFC3339 时间。
	CancelledAt string
}
