...
type AccountTaskRepository interface {
	...
	// ListFlowerRequestCandidates 按订单游标读取自动求花候选。
	ListFlowerRequestCandidates(ctx context.Context, cookieID, afterOrderID string, limit int) ([]db.FlowerRequestCandidate, error)
	// ListReceiptReminderCandidates 按订单游标读取自动确认收货提醒候选。
	ListReceiptReminderCandidates(ctx context.Context, cookieID, afterOrderID string, limit int) ([]db.ReceiptReminderCandidate, error)
	// GetReceiptReminderCandidate 在实际发送前重新读取订单状态和会话目标。
	GetReceiptReminderCandidate(ctx context.Context, cookieID, orderID string) (db.ReceiptReminderCandidate, error)
	...
	// GetRunByKey 读取稳定幂等键对应的任务运行。
	GetRunByKey(ctx context.Context, runKey string) (db.AccountTaskRun, bool, error)
	// QuarantineRunningRuns 隔离进程重启时遗留的未知运行。
	QuarantineRunningRuns(ctx context.Context, taskTypes []string, message string) (int64, error)
...
// ListFlowerRequestCandidates 委托自动求花候选订单查询。
func (r storeAccountTaskRepository) ListFlowerRequestCandidates(ctx context.Context, cookieID, afterOrderID string, limit int) ([]db.FlowerRequestCandidate, error) {
	return r.store.AccountTasks.ListFlowerRequestCandidates(ctx, cookieID, afterOrderID, limit)
}

// ListReceiptReminderCandidates 委托自动确认收货提醒候选查询。
func (r storeAccountTaskRepository) ListReceiptReminderCandidates(ctx context.Context, cookieID, afterOrderID string, limit int) ([]db.ReceiptReminderCandidate, error) {
	return r.store.AccountTasks.ListReceiptReminderCandidates(ctx, cookieID, afterOrderID, limit)
}

// GetReceiptReminderCandidate 委托发送前订单事实复核。
func (r storeAccountTaskRepository) GetReceiptReminderCandidate(ctx context.Context, cookieID, orderID string) (db.ReceiptReminderCandidate, error) {
	return r.store.AccountTasks.GetReceiptReminderCandidate(ctx, cookieID, orderID)
}
...
// GetRunByKey 委托账号任务幂等运行查询。
func (r storeAccountTaskRepository) GetRunByKey(ctx context.Context, runKey string) (db.AccountTaskRun, bool, error) {
	return r.store.AccountTasks.GetRunByKey(ctx, runKey)
}

// QuarantineRunningRuns 委托进程重启遗留运行隔离。
func (r storeAccountTaskRepository) QuarantineRunningRuns(ctx context.Context, taskTypes []string, message string) (int64, error) {
	return r.store.AccountTasks.QuarantineRunningRuns(ctx, taskTypes, message)
}
