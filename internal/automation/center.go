...
type Center struct {
	...
	// flowers 拥有自动收花队列、隔离浏览器生命周期和 WS 结果协调。
	flowers *flowerAutomationCoordinator
	store   *db.Store
...
type centerDependencies struct {
	...
	// notifier 是发货结果通知器。
	notifier Notifier
	// flowerBrowser 是自动收花使用的隔离官方页面浏览器。
	flowerBrowser FlowerReceiveBrowser
...
type CenterDependencies struct {
	...
	Notifier Notifier
	// FlowerReceiveBrowser 提供独立 Chromium 官方收花页能力；为空时自动收花保持不可用。
	FlowerReceiveBrowser FlowerReceiveBrowser
...
func NewWithDependencies(store *db.Store, senders SenderProvider, logger *slog.Logger, dependencies CenterDependencies) *Center {
	...
	center := &Center{
		...
		dependencies: centerDependencies{
			...
			flowerBrowser:     dependencies.FlowerReceiveBrowser,
	...
	center.taskRunner = accountTaskCoordinator{
		...
		senders:    senders,
		notifier:   center.dependencies.notifier,
		recoverer: func() CredentialRecoverer {
			return center.dependencies.recoverer
		},
		logger: center.logger,
	}
	center.flowers = newFlowerAutomationCoordinator(
		newStoreAccountTaskRepository(store), center.dependencies.flowerBrowser, center.dependencies.notifier, center.logger,
	)
	center.runs = automationRunCoordinator{
		...
		scheduleRedFlower:        center.scheduleRedFlowerAfterShipmentTask,
	}
	return center
}
...
func (c *Center) handleTask(ctx context.Context, task Task) (bool, error) {
	...
	// resolvedTask、matched、resolveErr 是发货或退款成功无订单号事件的安全关联结果。
	resolvedTask, matched, resolveErr := c.resolveAssociatedOrderTask(ctx, task)
	if resolveErr != nil {
	...
	if task.TriggerType == TriggerRedFlowerSent || task.TriggerType == TriggerRedFlowerReceived {
		if c.flowers == nil {
			return false, nil
		}
		return true, c.flowers.HandleTask(ctx, task)
	}
	if task.OrderID != "" {
	...
	if task.TriggerType == TriggerRedFlowerRequestDue {
		return c.handleScheduledRedFlowerRequest(ctx, task)
	}
	if task.TriggerType == TriggerOrderShipped {
		return false, c.handleOrderShippedTask(ctx, task)
	}
...
func (c *Center) ManualFullDelivery(ctx context.Context, order *db.Order) (int, error) {
	...
	for _, rule := range rules {
		...
		cancel()
		if err == nil {
			// scheduleErr 是手动完整发货最终成功后创建秒级求花任务的错误；失败由分钟补偿扫描兜底。
			if scheduleErr := c.ScheduleRedFlowerAfterShipment(ctx, order.OrderID); scheduleErr != nil {
				c.logger.Warn("手动完整发货成功后创建自动求花任务失败，等待分钟补偿扫描", "order_id", order.OrderID, "err", scheduleErr)
			}
		}
		if err != nil {
			return sent, err
