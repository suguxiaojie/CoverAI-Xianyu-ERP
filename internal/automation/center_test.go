...
// TestDuplicateAutomationEventSkipsExistingRun 验证同一幂等事件未取得新运行执行权时安全跳过，不解引用空运行或重复发送。
func TestDuplicateAutomationEventSkipsExistingRun(t *testing.T) {
	// store、cleanup 分别是当前测试独占的自动化数据库和资源释放函数。
	store, cleanup := newAutomationTestStore(t)
	defer cleanup()
	// ctx 是两次相同自动化事件共用的非取消测试上下文。
	ctx := context.Background()
	// admin 是固定测试账号对应的本地管理员用户。
	admin, _ := store.Users.GetByUsername(ctx, "admin")
	// createErr 是创建单动作幂等规则时的数据库错误。
	_, createErr := store.Automation.Create(ctx, db.AutomationRuleInput{
		UserID: admin.ID, CookieID: "cid", Name: "duplicate-event", TriggerType: TriggerBuyerReviewed, Enabled: true,
		Actions: []db.AutomationActionInput{{ActionType: ActionSendText, MessageTemplate: "once", Enabled: true}},
	})
	if createErr != nil {
		t.Fatalf("创建重复事件测试规则失败: %v", createErr)
	}
	// sender 记录实际外部消息调用次数，重复事件不得追加第二条。
	sender := &testSender{}
	// center 是装配固定发送器的自动化中心。
	center := New(store, testSenderProvider{sender: sender}, nil)
	// task 是两次投递完全相同、因 trigger key 幂等合并的事件。
	task := Task{AccountID: "cid", TriggerType: TriggerBuyerReviewed, OrderID: "duplicate-order", ChatID: "chat", BuyerID: "buyer"}
	// firstErr 是首次事件正常创建运行并执行动作的结果。
	firstErr := center.HandleTask(ctx, task)
	if firstErr != nil {
		t.Fatalf("首次自动化事件执行失败: %v", firstErr)
	}
	// duplicateErr 是相同事件再次进入时的跳过结果；旧实现会在这里解引用 nil run 并使进程崩溃。
	duplicateErr := center.HandleTask(ctx, task)
	if duplicateErr != nil {
		t.Fatalf("重复自动化事件应安全跳过: %v", duplicateErr)
	}
	if len(sender.texts) != 1 || sender.texts[0] != "once" {
		t.Fatalf("重复事件不得再次发送，texts=%v", sender.texts)
	}
}
...
func TestCenterOrderPaidSendsCardBeforeConfirmShipment(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		fmt.Fprint(w, `{"ret":["SUCCESS::调用成功"],"data":{"orderId":"token-refresh-order"}}`)
	}))
	defer server.Close()
...
func TestManualFullDeliveryIsImmediateIdempotentAndForcesConfirmation(t *testing.T) {
	...
	// now 是手动完整发货订单的付款时间基准。
	now := time.Now().UTC()
	_, _ = store.DB.ExecContext(ctx, `INSERT INTO orders (order_id,cookie_id,item_id,buyer_id,chat_id,order_status,paid_at)
		VALUES (?,?,?,?,?,?,?)`, "manual-order", "cid", "manual-item", "buyer", "chat", "pending_ship", now.Add(-time.Hour).Format(time.RFC3339Nano))
	if // settingsErr 是手动完整发货自动求花配置写入错误。
	...
	// pendingFlowerTasks 是手动完整发货成功后创建的秒级求花任务数量。
	var pendingFlowerTasks int
	if // countErr 是手动完整发货后秒级任务数量查询错误。
	countErr := store.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM automation_pending_tasks WHERE task_key=?`, scheduledFlowerRequestKey("cid", "manual-order")).Scan(&pendingFlowerTasks); countErr != nil || pendingFlowerTasks != 1 {
...
// TestAutomaticDeliverySchedulesFlowerOnlyAfterSuccessfulRun 验证自动发货运行与确认发货均成功后才创建秒级求花任务。
func TestAutomaticDeliverySchedulesFlowerOnlyAfterSuccessfulRun(t *testing.T) {
	// store、cleanup 是隔离数据库及释放函数。
	store, cleanup := newAutomationTestStore(t)
	defer cleanup()
	// ctx 是自动发货和后续任务断言使用的上下文。
	ctx := context.Background()
	// admin 是创建卡密和付款自动化规则的用户。
	admin, adminErr := store.Users.GetByUsername(ctx, "admin")
	if adminErr != nil {
		t.Fatal(adminErr)
	}
	if // updateErr 是打开测试账号自动确认发货的数据库错误。
	_, updateErr := store.DB.ExecContext(ctx, `UPDATE cookies SET auto_confirm=1 WHERE id='cid'`); updateErr != nil {
		t.Fatal(updateErr)
	}
	if // itemErr 是自动发货商品夹具写入错误。
	_, itemErr := store.DB.ExecContext(ctx, `INSERT INTO item_info (cookie_id,item_id,item_title) VALUES ('cid','auto-flower-item','会员')`); itemErr != nil {
		t.Fatal(itemErr)
	}
	// cardID、cardErr 是自动发货使用的单张文本卡密及创建错误。
	cardID, cardErr := store.Cards.Create(ctx, &db.CardFull{Name: "auto-flower-card", Type: "text", TextContent: "CARD", Enabled: true, UserID: admin.ID})
	if cardErr != nil {
		t.Fatal(cardErr)
	}
	if // ruleErr 是付款自动发货规则创建错误。
	_, ruleErr := store.Automation.Create(ctx, db.AutomationRuleInput{
		UserID: admin.ID, CookieID: "cid", ItemID: "auto-flower-item", Name: "付款自动发货后求花", TriggerType: TriggerOrderPaid, Enabled: true,
		Actions: []db.AutomationActionInput{
	...
	// now 是订单付款时间基准。
	now := time.Now().UTC()
	if // orderErr 是付款订单夹具写入错误。
	_, orderErr := store.DB.ExecContext(ctx, `INSERT INTO orders
		(order_id,cookie_id,item_id,buyer_id,chat_id,order_status,paid_at,quantity,amount) VALUES(?,?,?,?,?,?,?,?,?)`,
		"automatic-flower-order", "cid", "auto-flower-item", "buyer", "chat", "pending_ship", now.Add(-time.Minute).Format(time.RFC3339Nano), "1", "9.9"); orderErr != nil {
		t.Fatal(orderErr)
	}
	// center 是远端确认发货明确成功的自动化中心。
	center := NewWithDependencies(store, testSenderProvider{sender: &testSender{}}, nil, CenterDependencies{MTop: &fakeMTop{consignOk: true}})
	// handleErr 是付款自动化完整运行结果。
	handleErr := center.HandleTask(ctx, Task{
		Source: "ws", AccountID: "cid", TriggerType: TriggerOrderPaid, OrderID: "automatic-flower-order",
		ItemID: "auto-flower-item", BuyerID: "buyer", ChatID: "chat", Quantity: "1", Amount: "9.9",
	})
	if handleErr != nil {
		t.Fatal(handleErr)
	}
	// runStatus 是付款自动化最终持久化状态。
	var runStatus string
	if // runErr 是付款自动化最终状态查询错误。
	runErr := store.DB.QueryRowContext(ctx, `SELECT status FROM automation_runs WHERE order_id=?`, "automatic-flower-order").Scan(&runStatus); runErr != nil || runStatus != "success" {
		t.Fatalf("run status=%q err=%v", runStatus, runErr)
	}
	// pending 是自动化运行成功后创建的秒级求花任务数量。
	var pending int
	if // countErr 是自动发货成功后秒级任务数量查询错误。
	countErr := store.DB.QueryRowContext(ctx, `SELECT COUNT(*) FROM automation_pending_tasks WHERE task_key=?`, scheduledFlowerRequestKey("cid", "automatic-flower-order")).Scan(&pending); countErr != nil || pending != 1 {
