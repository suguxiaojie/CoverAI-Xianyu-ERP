...
type automationRunCoordinator struct {
	...
	// scheduleRedFlower 在付款自动化最终成功且订单已有发货事实时创建秒级求花任务。
	scheduleRedFlower func(context.Context, Task) error
}
...
func (r automationRunCoordinator) executeRule(ctx context.Context, task Task, rule db.AutomationRule) (resultErr error) {
	...
	defer func() {
		...
		// runFinished 表示自动化运行最终成功写入，只有该状态允许创建后续求花任务。
		runFinished := false
		if // finishErr 是自动化运行最终状态写入错误。
		finishErr := r.store.Automation.FinishRun(finishCtx, run.ID, run.AttemptCount, status, sent, errMsg); finishErr != nil {
			...
			runFinished = status == "success"
		}
		if runFinished && r.scheduleRedFlower != nil {
			if // scheduleErr 是发货成功后秒级求花任务创建失败原因。
			scheduleErr := r.scheduleRedFlower(finishCtx, task); scheduleErr != nil {
				r.logger.Warn("发货成功后创建自动求花任务失败，等待分钟补偿扫描", "run_id", run.ID, "order_id", task.OrderID, "err", scheduleErr)
			}
		}
		if r.hasNotifier() {
