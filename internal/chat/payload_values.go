...
// extractReplyMessageID 从实时或历史扩展对象中读取原生引用目标，只接受可精确定位的 PNM ID。
func extractReplyMessageID(value any) string {
	// candidate 是外层扩展字段或 extJson 中首个非空引用消息标识。
	candidate := strings.TrimSpace(extractString(value, "replyMessageId"))
