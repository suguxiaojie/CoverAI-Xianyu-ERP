...
func (s *Service) RecordConversationPage(ctx context.Context, accountID, myID string, body map[string]any) (ConversationPage, error) {
	...
	for _, item := range items {
		...
		unreadCount := s.conversationUnreadCount(ctx, accountID, cid, peerID, conv, last, summary, lastMessageAt)
...
func (s *Service) conversationUnreadCount(ctx context.Context, accountID, chatID, peerID string, conv, last map[string]any, summary string, lastMessageAt int64) int {
	...
	// systemLast 表示联系人摘要已由协议或稳定文案识别为平台系统消息。
	systemLast := historyMessageIsSystem(last, summary)
	// latest、latestErr 是本地最新消息的时间和已读状态；覆盖平台摘要时本地状态更权威。
	latest, latestErr := s.repository.LatestReadState(ctx, accountID, chatID)
	// sameSystemEvent 允许同一系统卡片的消息时间和联系人摘要时间存在最多一秒协议偏差。
	sameSystemEvent := systemLast && latest.SentAt > 0 && latest.SentAt+1000 >= lastMessageAt
	if latestErr == nil && latest.ReadStatus == 2 && (latest.SentAt >= lastMessageAt || sameSystemEvent) {
		return 0
	}
...
func (s *Service) RecordIncoming(ctx context.Context, in Incoming) (*db.ChatMessage, bool, error) {
	...
	message := db.ChatMessage{MessageKey: key, Direction: "incoming", SenderID: in.BuyerID,
		...
		ReplyToPlatformMessageID: extractReplyMessageID(in.Raw)}
	if messageType == "location" {
		message.PlatformContentType = 30
		message.Summary = locationCardSummary(content)
	}
	if // card、ok 是实时 contentType=26 的结构化交易卡片及识别结果。
	card, ok := parseSystemCard(in.Raw, content); ok {
		applySystemCard(&message, card)
		message.MessageType = "system"
		message.Content = card.Title
	}
	// buyerID、buyerName、buyerAvatar 是当前消息被允许补入会话的真实对端身份。
	buyerID, buyerName, buyerAvatar := incomingSessionIdentity(in, message.MessageType)
	// session 只在可信买家入站消息中携带身份；系统或卖家消息只更新摘要和商品。
	session := db.ChatSession{CookieID: in.AccountID, ChatID: in.ChatID, BuyerID: buyerID,
		BuyerName: buyerName, BuyerAvatar: buyerAvatar, ItemID: in.ItemID, ItemTitle: extractString(in.Raw, "itemTitle", "title")}
...
// incomingSessionIdentity 只让真实非系统买家入站消息补入会话身份。
//
// 平台的交易卡片和部分卖家回显会把卖家账号放在 senderUserId，
// 这些值可以保留在消息行，但不得覆盖 chat_sessions 的买家身份。
func incomingSessionIdentity(in Incoming, messageType string) (string, string, string) {
	// accountID 是去除平台会话后缀后的当前卖家账号。
	accountID := strings.TrimSuffix(strings.TrimSpace(in.AccountID), "@goofish")
	// buyerID 是去除平台会话后缀后的候选对端账号。
	buyerID := strings.TrimSuffix(strings.TrimSpace(in.BuyerID), "@goofish")
	if buyerID == "1400" {
		return buyerID, "闲小蜜", xianxiaomiAvatar
	}
	if messageType == "system" || buyerID == "" || buyerID == accountID {
		return "", "", ""
	}
	return buyerID, strings.TrimSpace(in.BuyerName), extractString(in.Raw, "avatar", "avatarUrl", "senderAvatar")
}
...
func extractMessageContent(raw map[string]any, fallback string) (string, string) {
	...
	inspect = func(value any) (string, string) {
		...
		case map[string]any:
			...
			if contentType == "30" {
				// locationContent、ok 是位置卡片稳定 JSON 及字段完整性标记。
				if locationContent, ok := normalizeLocationCardContent(typed["locationCard"]); ok {
					return "location", locationContent
				}
			}
			if contentType == "2" {
...
// contentType=14 is a platform notice, contentType=25 can carry a red-flower
// prompt, and contentType=26 is an official trade card. User 1400 is 闲小蜜.
...
func isOfficialSystemMessage(raw map[string]any, senderID, fallback string) bool {
	...
	// normalizedFallback 是平台联系人摘要缺失卡片载荷时保留的稳定系统文案。
	normalizedFallback := strings.Trim(strings.TrimSpace(fallback), "[]")
	return normalizedFallback == "发来一条新消息" ||
		strings.Contains(normalizedFallback, "已求买家送我闲鱼小红花") ||
		strings.Contains(normalizedFallback, "可以送我闲鱼小红花吗")
}
...
func (s *Service) CreateOutgoing(ctx context.Context, session db.ChatSession, text, replyToPlatformMessageID string) (*db.ChatMessage, error) {
	return s.createOutgoingMedia(ctx, session, "text", strings.TrimSpace(text), strings.TrimSpace(replyToPlatformMessageID))
}
...
func (s *Service) CreateOutgoingMedia(ctx context.Context, session db.ChatSession, messageType, content string) (*db.ChatMessage, error) {
	return s.createOutgoingMedia(ctx, session, messageType, content, "")
}

// createOutgoingMedia 创建可选携带原生引用目标的本地待发送消息并广播可观测状态。
func (s *Service) createOutgoingMedia(ctx context.Context, session db.ChatSession, messageType, content, replyToPlatformMessageID string) (*db.ChatMessage, error) {
	// key 用于本次流程后续判断的key
	key := "local-" + randomID()
	// message 用于本次流程后续判断的消息
	message := db.ChatMessage{MessageKey: key, Direction: "outgoing", SenderID: session.CookieID,
		...
		SentAt: time.Now().UTC().UnixMilli(), ReplyToPlatformMessageID: replyToPlatformMessageID}
	if strings.EqualFold(strings.TrimSpace(messageType), "location") {
		message.PlatformContentType = 30
		message.Summary = locationCardSummary(content)
	}
	// stored、err 用于本次流程后续判断的stored、err
...
// RecordOutgoingMediaSent 保存平台已确认成功的自动化媒体消息并立即广播；platformMessageID 作为跨历史回显的幂等标识。
func (s *Service) RecordOutgoingMediaSent(ctx context.Context, session db.ChatSession, key, platformMessageID, messageType, content string, sentAt int64) (*db.ChatMessage, error) {
	...
	// message 是平台已确认成功且可直接展示的出站媒体模型。
	message := db.ChatMessage{MessageKey: key, PlatformMessageID: platformMessageID, Direction: "outgoing",
		SenderID: session.CookieID, SenderName: "我", MessageType: messageType, Content: content,
		Status: "sent", SentAt: sentAt}
	if strings.EqualFold(messageType, "location") {
		message.PlatformContentType = 30
		message.Summary = locationCardSummary(content)
	}
	// stored、_, saveErr 分别是平台 PNM 幂等保存后的消息、未使用的首次插入标记和持久化错误。
	stored, _, saveErr := s.repository.SaveMessage(ctx, session, message, false)
	if saveErr == nil {
		s.Publish(session.CookieID, Event{Type: "message.created", Message: stored, Session: &session})
	}
