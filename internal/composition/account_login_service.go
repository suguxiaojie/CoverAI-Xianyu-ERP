...
func (service *accountLoginService) PersistQRLoginSuccess(ctx context.Context, userID int64, sessionID string, result map[string]any, targetAccountID, captchaMode string) (CookieLoginResult, error) {
	...
	persisted, persistErr := service.qrSessions.PersistOnce(sessionID, userID, func() (accountapp.QRLoginSessionPersistence, error) {
		...
		input := accountapp.QRLoginInput{UserID: userID, ScannedAccountID: scannedAccountID, TargetAccountID: targetAccountID, Cookies: cookies, CaptchaMode: accountapp.CaptchaMode(captchaMode)}
		if snapshotComplete {
