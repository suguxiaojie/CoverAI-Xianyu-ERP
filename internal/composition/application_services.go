...
type Services struct {
	...
	// knowledge 是人工 FAQ／文档、审核门禁和离线检索应用服务。
...
type AccountLogin interface {
	...
	PersistQRLoginSuccess(context.Context, int64, string, map[string]any, string, string) (CookieLoginResult, error)
...
func (services *Services) TransportPorts() TransportPorts {
	...
	return TransportPorts{
		...
		Keywords: services.keywords, Knowledge: services.knowledge, Settings: services.settings, Admin: services.admin,
	}
}
...
func New(dependencies Dependencies) (*Services, error) {
	...
	// loginSuccess 负责登录成功后立即重启启用账号；资料刷新保持独立，避免平台请求阻塞恢复 WS。
	loginSuccess := accountapp.NewLoginSuccessService(accountRepository, accountRuntime, func(message string, err error) {
	...
	services := &Services{
		...
		knowledge:              dependencies.TransportApplications.Knowledge,
...
func buildOrderServices(dependencies Dependencies) (*orderapp.ServiceSet, *orderapp.RefreshJobService, error) {
	...
	orderServices := orderapp.NewServiceSet(orderRepository, orderRepository, orderRepository, orderRepository, orderRepository, orderRepository, orderRepository,
		orderRuntime, orderRuntime, orderRuntime, orderRuntime, orderRuntime, orderRuntime, orderRuntime,
		dependencies.OrderDependencies.NewOrderRefreshJobRepository(), refreshOrderChunkSize)
