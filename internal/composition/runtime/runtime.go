...
type RuntimeInfrastructure struct {
	...
	// Logger 是进程级脱敏结构化日志器。
	Logger *slog.Logger
	// CardImageRoot 是用户拖拽上传卡密图片的受管数据目录，不由 HTTP 请求覆盖。
...
func BuildRuntime(options RuntimeOptions, infrastructure RuntimeInfrastructure) (Runtime, error) {
	...
	// cardImages 是 HTTP 卡券用例与自动发货链路共享的不可变受管图片存储。
	cardImages := adapter.NewCardImageStore(infrastructure.CardImageRoot)
	// runtimeBundle、bundleErr 分别是账号运行时依赖集合及其构造失败原因。
	runtimeBundle, bundleErr := adapter.NewRuntimeBundle(infrastructure.Store, browserManager, infrastructure.Logger, cardImages)
	if bundleErr != nil {
	...
	for _, component := range []lifecycle.NamedComponent{
		...
		{Name: "flower-automation", Component: lifecycle.FuncComponent{StartFunc: runtimeBundle.Automation.StartFlowerAutomation, CloseFunc: runtimeBundle.Automation.CloseFlowerAutomation}},
