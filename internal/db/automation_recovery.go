...
func (a *AutomationRules) MarkOrderEventTime(ctx context.Context, orderID, field string) error {
	return a.MarkOrderEventTimeAt(ctx, orderID, field, time.Now().UTC())
}

// MarkOrderEventTimeAt 使用给定 UTC 事件时间幂等写入订单事实；零值时间回退到当前时间。
func (a *AutomationRules) MarkOrderEventTimeAt(ctx context.Context, orderID, field string, occurredAt time.Time) error {
	switch field {
	case "paid_at", "shipped_at", "received_at", "completed_at", "refunded_at", "cancelled_at", "buyer_reviewed_at", "last_review_request_at":
	default:
	...
	// eventTime 是跨方言统一的 RFC3339 UTC 事件时间。
	eventTime := occurredAt.UTC().Format(time.RFC3339Nano)
	// err 用于本次流程后续判断的err
	_, err := a.DB.ExecContext(ctx, "UPDATE orders SET "+field+"=?, updated_at=CURRENT_TIMESTAMP WHERE order_id=? AND COALESCE("+field+",'')=''", eventTime, orderID)
	return err
}
