...
type KeywordRow struct {
	...
	MessageScope string
	SystemTypes  string
	// Enabled 表示所属规则组当前是否参与运行时匹配。
	Enabled bool
	// ReplyIntervalSeconds 是同会话同规则组成功回复后的冷却秒数。
	ReplyIntervalSeconds int64
	// SendDelaySeconds 是关键词命中后的发送等待秒数。
...
func (k *Keywords) AllRows(ctx context.Context, cookieID string) ([]KeywordRow, error) {
	...
	rows, err := k.DB.QueryContext(ctx,
		`SELECT id, keyword, reply, COALESCE(item_id,''), COALESCE(type,'text'), COALESCE(image_url,''), COALESCE(group_id,''), COALESCE(match_type,'contains'), COALESCE(message_scope,'customer'), COALESCE(system_types,''), enabled,reply_interval_seconds,send_delay_seconds
	...
	for rows.Next() {
		...
		err := rows.Scan(&r.ID, &r.Keyword, &r.Reply, &r.ItemID, &r.Type, &r.ImageURL, &r.GroupID, &r.MatchType, &r.MessageScope, &r.SystemTypes, &r.Enabled, &r.ReplyIntervalSeconds, &r.SendDelaySeconds); err != nil {
			return nil, err
		}
		r.CookieID = cookieID
		out = append(out, r)
...
func (k *Keywords) Add(ctx context.Context, cookieID, keyword, reply, itemID, kwType, imageURL string) (int64, error) {
	...
	// groupID 让兼容单关键词创建天然成为可编辑的单关键词组。
	groupID := fmt.Sprintf("legacy-%d", time.Now().UnixNano())
	return insertReturningID(ctx, k.DB, k.Dialect,
		`INSERT INTO keywords (cookie_id, keyword, reply, item_id, type, image_url, group_id) VALUES (?,?,?,?,?,?,?)`,
		cookieID, keyword, reply, nullable(itemID), kwType, nullable(imageURL), groupID)
}
...
func (k *Keywords) ReplaceForCookie(ctx context.Context, cookieID string, rows []KeywordRow) error {
	...
	// rowIndex、row 是当前兼容批量替换中的规则下标和内容。
	for rowIndex, row := range rows {
		...
		// groupID 让兼容批量替换中的每条旧规则保持独立组。
		groupID := fmt.Sprintf("legacy-%d-%d", time.Now().UnixNano(), rowIndex)
		if // err 用于本次流程后续判断的err
		_, err := tx.ExecContext(ctx,
			`INSERT INTO keywords (cookie_id, keyword, reply, item_id, type, image_url, group_id) VALUES (?,?,?,?,?,?,?)`,
			cookieID, row.Keyword, row.Reply, nullable(row.ItemID), kwType, nullable(row.ImageURL), groupID); err != nil {
			return err
		}
	}
	return tx.Commit()
