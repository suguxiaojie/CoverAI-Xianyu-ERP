...
func (i *Items) SyncFromRemote(ctx context.Context, cookieID string, rows []ItemInfoRow) (ItemSyncResult, error) {
	// records 将旧调用方的基础商品行转换为“不更新 SKU”的兼容远端记录。
	records := make([]RemoteItemRecord, 0, len(rows))
	// row 表示当前待转换的商品基础行。
	for _, row := range rows {
		records = append(records, RemoteItemRecord{Item: row})
	}
	return i.SyncFromRemoteRecords(ctx, cookieID, records)
}

// SyncFromRemoteRecords 在同一事务中对账商品全集和已经成功读取的 SKU 快照。
func (i *Items) SyncFromRemoteRecords(ctx context.Context, cookieID string, records []RemoteItemRecord) (ItemSyncResult, error) {
	cookieID = strings.TrimSpace(cookieID)
	if cookieID == "" {
		return ItemSyncResult{}, errors.New("cookie_id 不能为空")
	}
	// rows 保存用于沿用商品基础字段 reconcile 的行模型。
	rows := make([]ItemInfoRow, 0, len(records))
	// recordsByItem 保存每个商品成功读取的可选 SKU 快照。
	recordsByItem := make(map[string]RemoteItemRecord, len(records))
	// record 表示当前待转换的远端商品和 SKU 快照。
	for _, record := range records {
		rows = append(rows, record.Item)
		recordsByItem[strings.TrimSpace(record.Item.ItemID)] = record
	}
	...
	for index := range validRows {
		...
		// record、hasRecord 保存当前商品是否带有权威 SKU 详情。
		record, hasRecord := recordsByItem[validRows[index].ItemID]
		if hasRecord && record.SKUDetailLoaded {
			if // err 保存权威多规格状态写入失败。
			_, err := tx.ExecContext(ctx, `UPDATE item_info SET is_multi_spec=? WHERE cookie_id=? AND item_id=?`,
				boolToInt(record.Item.IsMultiSpec), cookieID, validRows[index].ItemID); err != nil {
				return rollback(err)
			}
			// skuErr 是当前商品 SKU 与基础字段同事务对账错误。
			if skuErr := i.syncSKUsTx(ctx, tx, cookieID, validRows[index].ItemID, record.SKUs); skuErr != nil {
				return rollback(skuErr)
			}
		} else if validRows[index].IsMultiSpec {
			if // err 用于本次流程后续判断的err
	...
	// skuDeleteQuery 将已不在平台在售全集中的商品 SKU 同步逻辑删除，保留本地成本审计值。
	skuDeleteQuery := `UPDATE item_skus SET deleted_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP WHERE cookie_id=? AND deleted_at IS NULL`
	// skuDeleteArguments 复用商品全集标识构造 SKU 删除范围。
	skuDeleteArguments := make([]any, 0, len(remoteIDs)+1)
	skuDeleteArguments = append(skuDeleteArguments, cookieID)
	if len(remoteIDs) > 0 {
		// skuPlaceholders 是远端在售商品标识的占位符列表。
		skuPlaceholders := make([]string, 0, len(remoteIDs))
		// itemID 表示当前加入 SKU 保留集合的远端在售商品标识。
		for itemID := range remoteIDs {
			skuPlaceholders = append(skuPlaceholders, "?")
			skuDeleteArguments = append(skuDeleteArguments, itemID)
		}
		skuDeleteQuery += ` AND item_id NOT IN (` + strings.Join(skuPlaceholders, ",") + `)`
	}
	// skuDeleteErr 是不在远端在售全集中的商品 SKU 逻辑删除错误。
	if _, skuDeleteErr := tx.ExecContext(ctx, skuDeleteQuery, skuDeleteArguments...); skuDeleteErr != nil {
		return rollback(skuDeleteErr)
	}
