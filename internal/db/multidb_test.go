	...
	for _, tg := range allTestTargets(t) {
		t.Run(tg.name, func(t *testing.T) {
			...
			for _, c := range []struct {
				...
				{"chat_messages", "recall_operator_type"},
				{"chat_messages", "recall_operator_id"},
			} {
...
func TestMultiDB_ChatAndAccountTasks(t *testing.T) {
	...
	for _, tg := range allTestTargets(t) {
		t.Run(tg.name, func(t *testing.T) {
			...
			if err != nil || settings.RateContent == "" || settings.PolishTime != "03:00" || settings.RequestFlowerAfterHours != 24 || settings.RequestFlowerAfterSeconds != 10 ||
				!settings.ReceiveFlowerShowBrowser || settings.ReceiveFlowerTimeoutSeconds != 120 {
				t.Fatalf("default settings=%+v err=%v", settings, err)
			}
			settings.AutoRateEnabled = true
			settings.AutoPolishEnabled = true
			settings.AutoRequestFlowerEnabled = true
			settings.RequestFlowerAfterHours = 6
			settings.RequestFlowerAfterSeconds = 12
			settings.AutoReceiveFlowerEnabled = true
			settings.ReceiveFlowerShowBrowser = false
			settings.ReceiveFlowerTimeoutSeconds = 180
			...
			if err != nil || !stored.AutoRateEnabled || !stored.AutoPolishEnabled || !stored.AutoRequestFlowerEnabled || stored.RequestFlowerAfterHours != 6 || stored.RequestFlowerAfterSeconds != 12 ||
				!stored.AutoReceiveFlowerEnabled || stored.ReceiveFlowerShowBrowser || stored.ReceiveFlowerTimeoutSeconds != 180 ||
				stored.RateContent != "交易愉快" || stored.PolishTime != "04:30" {
			...
			// storedRun、runExists、runErr 是按幂等键读取的运行状态及查询结果。
			storedRun, runExists, runErr := s.AccountTasks.GetRunByKey(ctx, run.RunKey)
			if runErr != nil || !runExists || storedRun.Status != "failed" || storedRun.ErrorMessage != "retry" {
				t.Fatalf("stored run=%+v exists=%v err=%v", storedRun, runExists, runErr)
			}
			if // claimed、err 用于本次流程后续判断的claimed、err
...
func TestMultiDB_MarkOrderEventTime(t *testing.T) {
	...
	for _, tg := range allTestTargets(t) {
		t.Run(tg.name, func(t *testing.T) {
			...
			// 白名单字段为空时全部写入当前时间。
			for _, f := range []string{"paid_at", "shipped_at", "received_at", "completed_at", "refunded_at", "cancelled_at", "buyer_reviewed_at", "last_review_request_at"} {
				if // err 用于本次流程后续判断的err
