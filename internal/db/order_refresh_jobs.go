...
type OrderRefreshJob struct {
	...
	FilterStatus string
	// SyncMode 是任务恢复后继续使用的 incremental 或 full 模式。
	SyncMode string
...
func (j *OrderRefreshJobs) Create(ctx context.Context, job *OrderRefreshJob) error {
	...
	_, err := j.DB.ExecContext(ctx,
		`INSERT INTO order_refresh_jobs
		 (id,user_id,cookie_id,filter_status,sync_mode,status,result_json,error_message,worker_token,lease_expires_at)
		 VALUES (?,?,?,?,?,?,?,?,?,?)`,
		job.ID, job.UserID, job.CookieID, job.FilterStatus, job.SyncMode, job.Status, job.ResultJSON,
...
func (j *OrderRefreshJobs) Get(ctx context.Context, userID int64, id string) (*OrderRefreshJob, error) {
	...
	err := j.DB.QueryRowContext(ctx,
		`SELECT id,user_id,cookie_id,filter_status,sync_mode,status,COALESCE(result_json,'{}'),
...
// UpdateProgress 在任务仍由当前 worker 租约拥有时更新轻量进度 JSON。
func (j *OrderRefreshJobs) UpdateProgress(ctx context.Context, id, token, progressJSON string) (bool, error) {
	// result、err 保存进度更新结果及数据库错误。
	result, err := j.DB.ExecContext(ctx,
		`UPDATE order_refresh_jobs
		    SET result_json=?,updated_at=CURRENT_TIMESTAMP
		  WHERE id=? AND status='running' AND worker_token=?`, progressJSON, id, token)
	if err != nil {
		return false, err
	}
	// affected、rowsErr 保存进度更新影响行数及读取错误。
	affected, rowsErr := result.RowsAffected()
	return affected == 1, rowsErr
}
...
func (j *OrderRefreshJobs) Recoverable(ctx context.Context, now int64, limit int) ([]OrderRefreshJob, error) {
	...
	for rows.Next() {
		...
		// err 表示扫描可恢复任务的数据库错误。
		if err := rows.Scan(&job.ID, &job.UserID, &job.CookieID, &job.FilterStatus, &job.SyncMode, &job.Status,
