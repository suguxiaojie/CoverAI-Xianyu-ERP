...
func TestOrderRefreshJobsLifecycle(t *testing.T) {
	...
	// job 保存待测试的订单刷新任务。
	job := &OrderRefreshJob{ID: "refresh-job-1", UserID: userID, CookieID: cookieID, FilterStatus: "pending_ship", SyncMode: "full"}
	// err 表示创建任务的数据库错误。
	...
	if err != nil || loaded.Status != "queued" || loaded.ResultJSON != "{}" || loaded.SyncMode != "full" {
	...
	// progressJSON 是运行中任务写入的轻量进度，不包含逐单完整结果。
	progressJSON := `{"progress":{"stage":"syncing_details","message":"正在逐单同步订单详情","processed":2,"total":5,"succeeded":2,"failed":0,"percent":56},"results":[]}`
	// progressApplied、err 保存正确租约写入进度的结果和错误。
	progressApplied, err := store.OrderRefreshJobs.UpdateProgress(ctx, job.ID, "worker-1", progressJSON)
	if err != nil || !progressApplied {
		t.Fatalf("update progress: applied=%v err=%v", progressApplied, err)
	}
	// wrongProgress、err 保存错误租约写入进度的结果和错误。
	wrongProgress, err := store.OrderRefreshJobs.UpdateProgress(ctx, job.ID, "worker-2", `{"progress":{}}`)
	if err != nil || wrongProgress {
		t.Fatalf("wrong worker progress must fail: applied=%v err=%v", wrongProgress, err)
	}
	// progressJob、err 保存进度写入后的运行任务快照及读取错误。
	progressJob, err := store.OrderRefreshJobs.Get(ctx, userID, job.ID)
	if err != nil || progressJob.ResultJSON != progressJSON || progressJob.Status != "running" {
		t.Fatalf("progress job=%+v err=%v", progressJob, err)
	}
	// err 表示重复抢占的数据库错误。
