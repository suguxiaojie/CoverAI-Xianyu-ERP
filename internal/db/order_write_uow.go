...
type OrderWriteTransaction struct {
	...
	// items 保存商品写入仓储。
	items *Items
	// orderCosts 保存成本快照写入仓储。
	orderCosts *OrderCosts
}

// newOrderWriteUnitOfWork 使用同一数据库连接池和两个窄仓储构造订单写入事务实现。
// database、orders 或 items 缺失时仍返回对象，由 WithTransaction 返回明确初始化错误。
func newOrderWriteUnitOfWork(database *sql.DB, orders *Orders, items *Items, orderCosts *OrderCosts) *OrderWriteUnitOfWork {
	return &OrderWriteUnitOfWork{database: database, orders: orders, items: items, orderCosts: orderCosts}
}
...
func (unit *OrderWriteUnitOfWork) WithTransaction(ctx context.Context, work func(*OrderWriteTransaction) error) error {
	if unit == nil || unit.database == nil || unit.orders == nil || unit.items == nil || unit.orderCosts == nil {
	...
	writeTransaction := &OrderWriteTransaction{transaction: transaction, orders: unit.orders, items: unit.items, orderCosts: unit.orderCosts}
...
func (transaction *OrderWriteTransaction) PatchOrder(ctx context.Context, orderID string, patch OrderPatch) error {
	if transaction == nil || transaction.transaction == nil || transaction.orders == nil || transaction.orderCosts == nil {
		return errors.New("订单写入事务未初始化")
	}
	if // patchErr 是事务内订单字段更新错误。
	patchErr := transaction.orders.PatchTx(ctx, transaction.transaction, orderID, patch); patchErr != nil {
		return patchErr
	}
	return transaction.orderCosts.CaptureExactTx(ctx, transaction.transaction, orderID)
}
...
func (transaction *OrderWriteTransaction) UpsertOrder(ctx context.Context, orderID string, options OrderUpsertOpts) error {
	...
	if // upsertErr 是事务内单订单写入错误。
	upsertErr := transaction.orders.UpsertTx(ctx, transaction.transaction, orderID, options); upsertErr != nil {
		return upsertErr
	}
	return transaction.orderCosts.CaptureExactTx(ctx, transaction.transaction, orderID)
}
...
func (transaction *OrderWriteTransaction) UpsertOrders(ctx context.Context, rows []BatchOrderUpsert) error {
	...
	if // upsertErr 是事务内批量订单写入错误。
	upsertErr := transaction.orders.UpsertManyTx(ctx, transaction.transaction, rows); upsertErr != nil {
		return upsertErr
	}
	// row 是当前尝试锁定成本快照的批量订单行。
	for _, row := range rows {
		if // snapshotErr 是当前订单成本快照写入错误。
		snapshotErr := transaction.orderCosts.CaptureExactTx(ctx, transaction.transaction, row.OrderID); snapshotErr != nil {
			return snapshotErr
		}
	}
	return nil
