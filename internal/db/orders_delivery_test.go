...
func TestOrdersUpsertMany(t *testing.T) {
	...
	bargain := true
	// err 表示初始订单写入错误。
	if err := store.Orders.Upsert(ctx, "batch-existing", OrderUpsertOpts{CookieID: cookieID, OrderStatus: "shipped", Amount: "10", CreatedAt: "2025-01-01T00:00:00Z", IsBargain: &bargain}); err != nil {
	...
	rows := []BatchOrderUpsert{
		{OrderID: "batch-existing", Options: OrderUpsertOpts{CookieID: cookieID, OrderStatus: "pending_ship", SpecName: "颜色", SpecValue: "蓝", Amount: "¥12.50", CreatedAt: "2024-12-01T00:00:00Z"}},
		{OrderID: "batch-new", Options: OrderUpsertOpts{CookieID: cookieID, OrderStatus: "pending_ship", Quantity: "2", Amount: "5.00", CreatedAt: "2025-02-01T00:00:00Z", IsBargain: &bargain}},
	}
	// err 保存批量订单写入错误。
	...
	if existing.OrderStatus != "shipped" || existing.SpecValue != "蓝" || existing.Amount != "12.50" || existing.IsBargain != 1 || existing.CreatedAt != "2024-12-01T00:00:00Z" || existing.Version <= before.Version {
		t.Fatalf("batch existing order=%+v before=%+v", existing, before)
	}
	if newOrder.OrderStatus != "pending_ship" || newOrder.Quantity != "2" || newOrder.Amount != "5.00" || newOrder.IsBargain != 1 || newOrder.CreatedAt != "2025-02-01T00:00:00Z" {
	...
	// preservedTimeOrder、err 保存空时间详情更新后的订单，created_at 必须保持平台值。
	preservedTimeOrder, err := store.Orders.Get(ctx, "batch-existing")
	if err != nil || preservedTimeOrder.CreatedAt != "2024-12-01T00:00:00Z" {
		t.Fatalf("empty created_at overwrote platform time: order=%+v err=%v", preservedTimeOrder, err)
	}
...
func TestOrders_ListForUserSearchesAcrossJoinedFields(t *testing.T) {
	...
	if // err 用于本次流程后续判断的err
	err := s.Orders.Upsert(ctx, "search-order", OrderUpsertOpts{CookieID: cid, ItemID: "item-1", BuyerID: "Buyer-ABC", ReceiverName: "张三", ChatID: "search-chat"}); err != nil {
		t.Fatal(err)
	}
	if // sessionErr 是写入买家昵称和头像聊天会话的错误。
	sessionErr := s.Chats.UpsertSession(ctx, ChatSession{CookieID: cid, ChatID: "search-chat", BuyerID: "Buyer-ABC", BuyerName: "昵称小王", BuyerAvatar: "https://img.example/buyer.png"}); sessionErr != nil {
		t.Fatal(sessionErr)
	}
	if // err 用于本次流程后续判断的err
	...
	// search 表示当前遍历过程中的搜索
	for _, search := range []string{"special", "buyer-abc", "昵称小王", "张三", "search-order"} {
		...
		if rows[0].BuyerName != "昵称小王" || rows[0].BuyerAvatar != "https://img.example/buyer.png" {
			t.Fatalf("search %q buyer identity=%+v", search, rows[0])
		}
	}
}

// TestOrdersListForUserUsesExactChatIdentityWhenBuyerIDIsContaminated 验证精确会话优先恢复订单昵称和头像。
func TestOrdersListForUserUsesExactChatIdentityWhenBuyerIDIsContaminated(t *testing.T) {
	// store、cleanup 是隔离 SQLite 订单查询仓储及其释放函数。
	store, cleanup := newTestDB(t)
	defer cleanup()
	// ctx 是预置订单、会话和执行列表查询的共享上下文。
	ctx := context.Background()
	// userID、cookieID 是当前用户和其卖家账号标识。
	userID, cookieID := seedAccount(t, store)
	// orderErr 是写入带精确 chat_id 订单的数据库错误。
	orderErr := store.Orders.Upsert(ctx, "chat-fallback-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-correct", ChatID: "chat-exact"})
	if orderErr != nil {
		t.Fatalf("写入精确会话订单: %v", orderErr)
	}
	// contaminated 模拟历史系统／卖家消息把会话 buyer_id 覆盖为卖家账号，但昵称和头像仍属于该精确会话。
	contaminated := ChatSession{CookieID: cookieID, ChatID: "chat-exact", BuyerID: cookieID, BuyerName: "精确会话昵称", BuyerAvatar: "https://img.example/exact.png"}
	// contaminatedSessionErr 是写入历史污染会话的数据库错误。
	contaminatedSessionErr := store.Chats.UpsertSession(ctx, contaminated)
	if contaminatedSessionErr != nil {
		t.Fatalf("写入历史污染会话: %v", contaminatedSessionErr)
	}
	// olderBuyerSession 是买家 ID 匹配但 chat_id 不同的较旧会话，不得抢走精确会话优先级。
	olderBuyerSession := ChatSession{CookieID: cookieID, ChatID: "chat-old", BuyerID: "buyer-correct", BuyerName: "旧买家昵称", BuyerAvatar: "https://img.example/old.png"}
	// olderSessionErr 是写入买家 ID 匹配但会话不同记录的数据库错误。
	olderSessionErr := store.Chats.UpsertSession(ctx, olderBuyerSession)
	if olderSessionErr != nil {
		t.Fatalf("写入旧买家会话: %v", olderSessionErr)
	}
	// rows、total、listErr 是未设置搜索条件时的订单列表结果。
	rows, total, listErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, Limit: 20})
	if listErr != nil || total != 1 || len(rows) != 1 {
		t.Fatalf("订单列表 rows=%+v total=%d err=%v", rows, total, listErr)
	}
	if rows[0].BuyerName != "精确会话昵称" || rows[0].BuyerAvatar != "https://img.example/exact.png" {
		t.Fatalf("未优先精确会话身份: %+v", rows[0])
	}
	// searched、searchedTotal、searchErr 验证同一精确会话昵称也可用于订单搜索。
	searched, searchedTotal, searchErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, Search: "精确会话昵称", Limit: 20})
	if searchErr != nil || searchedTotal != 1 || len(searched) != 1 || searched[0].OrderID != "chat-fallback-order" {
		t.Fatalf("精确会话昵称搜索 rows=%+v total=%d err=%v", searched, searchedTotal, searchErr)
	}
}

// TestOrdersListForUserResolvesOnlyUniqueHistoricalChat 验证订单缺少持久化 chat_id 时只采用唯一且无歧义的历史关联。
func TestOrdersListForUserResolvesOnlyUniqueHistoricalChat(t *testing.T) {
	// store、cleanup 是隔离 SQLite 订单与聊天仓储及其释放函数。
	store, cleanup := newTestDB(t)
	defer cleanup()
	// ctx 是本测试订单、会话、系统卡片写入和列表读取共用的上下文。
	ctx := context.Background()
	// userID、cookieID 是当前用户和其卖家账号标识。
	userID, cookieID := seedAccount(t, store)

	// explicitOrderErr 是写入已经持久化精确会话订单的错误；历史卡片不得覆盖该值。
	explicitOrderErr := store.Orders.Upsert(ctx, "explicit-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-explicit", ItemID: "item-explicit", ChatID: "chat-explicit"})
	if explicitOrderErr != nil {
		t.Fatalf("写入持久化会话订单: %v", explicitOrderErr)
	}
	// explicitCardSession 是同一订单后来出现系统卡片的另一会话，用于证明持久化关联优先。
	explicitCardSession := ChatSession{CookieID: cookieID, ChatID: "chat-explicit-card", BuyerID: "buyer-explicit", ItemID: "item-explicit"}
	// explicitCard 是不应覆盖持久化会话的订单系统卡片。
	explicitCard := ChatMessage{MessageKey: "explicit-card.PNM", Direction: "incoming", SenderID: "buyer-explicit", MessageType: "system", Status: "received", SentAt: 1000, SystemCardKind: "trade", SystemCardEvent: "order_paid", SystemCardOrderID: "explicit-order"}
	if // _, _, saveErr 是保存持久化关联订单卡片的结果和错误。
	_, _, saveErr := store.Chats.SaveMessage(ctx, explicitCardSession, explicitCard, false); saveErr != nil {
		t.Fatalf("保存持久化关联订单卡片: %v", saveErr)
	}

	// cardOrderErr 是写入缺少 chat_id、但稍后存在唯一订单卡片的订单错误。
	cardOrderErr := store.Orders.Upsert(ctx, "card-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-card", ItemID: "item-card"})
	if cardOrderErr != nil {
		t.Fatalf("写入订单卡片关联订单: %v", cardOrderErr)
	}
	// cardSession 是唯一携带精确订单号的历史会话。
	cardSession := ChatSession{CookieID: cookieID, ChatID: "chat-card", BuyerID: "buyer-card", ItemID: "item-card"}
	// cardMessage 是把订单号与唯一会话精确关联的历史系统卡片。
	cardMessage := ChatMessage{MessageKey: "card-order.PNM", Direction: "incoming", SenderID: "buyer-card", MessageType: "system", Status: "received", SentAt: 2000, SystemCardKind: "trade", SystemCardEvent: "order_paid", SystemCardOrderID: "card-order"}
	if // _, _, saveErr 是保存唯一订单卡片的结果和错误。
	_, _, saveErr := store.Chats.SaveMessage(ctx, cardSession, cardMessage, false); saveErr != nil {
		t.Fatalf("保存唯一订单卡片: %v", saveErr)
	}

	// buyerOrderErr 是写入完全没有订单卡片、但买家和商品仅有一个会话的订单错误。
	buyerOrderErr := store.Orders.Upsert(ctx, "buyer-item-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-item", ItemID: "item-buyer"})
	if buyerOrderErr != nil {
		t.Fatalf("写入买家商品关联订单: %v", buyerOrderErr)
	}
	// buyerSession 是同账号、买家和商品组合下的唯一会话。
	buyerSession := ChatSession{CookieID: cookieID, ChatID: "chat-buyer-item", BuyerID: "buyer-item", ItemID: "item-buyer"}
	if // sessionErr 是保存唯一买家商品会话的错误。
	sessionErr := store.Chats.UpsertSession(ctx, buyerSession); sessionErr != nil {
		t.Fatalf("保存唯一买家商品会话: %v", sessionErr)
	}

	// ambiguousCardOrderErr 是写入同一订单号存在两个历史会话的歧义订单错误。
	ambiguousCardOrderErr := store.Orders.Upsert(ctx, "ambiguous-card-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-ambiguous-card", ItemID: "item-ambiguous-card"})
	if ambiguousCardOrderErr != nil {
		t.Fatalf("写入订单卡片歧义订单: %v", ambiguousCardOrderErr)
	}
	// ambiguousCardSessions 保存同一订单号出现的两个不同会话；即使买家商品回退唯一，也必须因订单卡片冲突而禁用。
	ambiguousCardSessions := []ChatSession{
		{CookieID: cookieID, ChatID: "chat-ambiguous-card-a", BuyerID: "buyer-ambiguous-card", ItemID: "item-ambiguous-card"},
		{CookieID: cookieID, ChatID: "chat-ambiguous-card-b", BuyerID: "other-buyer", ItemID: "other-item"},
	}
	// ambiguousCardSession 是当前待保存歧义订单卡片的会话。
	for index, ambiguousCardSession := range ambiguousCardSessions {
		// ambiguousCardMessage 是当前会话携带同一订单号的系统卡片。
		ambiguousCardMessage := ChatMessage{MessageKey: fmt.Sprintf("ambiguous-card-%d.PNM", index), Direction: "incoming", SenderID: ambiguousCardSession.BuyerID, MessageType: "system", Status: "received", SentAt: int64(3000 + index), SystemCardKind: "trade", SystemCardEvent: "order_paid", SystemCardOrderID: "ambiguous-card-order"}
		if // _, _, saveErr 是保存当前歧义订单卡片的结果和错误。
		_, _, saveErr := store.Chats.SaveMessage(ctx, ambiguousCardSession, ambiguousCardMessage, false); saveErr != nil {
			t.Fatalf("保存歧义订单卡片 %d: %v", index, saveErr)
		}
	}

	// ambiguousBuyerOrderErr 是写入没有订单卡片、但同买家商品存在两个会话的歧义订单错误。
	ambiguousBuyerOrderErr := store.Orders.Upsert(ctx, "ambiguous-buyer-order", OrderUpsertOpts{CookieID: cookieID, BuyerID: "buyer-ambiguous", ItemID: "item-ambiguous"})
	if ambiguousBuyerOrderErr != nil {
		t.Fatalf("写入买家商品歧义订单: %v", ambiguousBuyerOrderErr)
	}
	// ambiguousBuyerSessions 保存同一账号、买家和商品下的两个不同会话。
	ambiguousBuyerSessions := []ChatSession{
		{CookieID: cookieID, ChatID: "chat-ambiguous-buyer-a", BuyerID: "buyer-ambiguous", ItemID: "item-ambiguous"},
		{CookieID: cookieID, ChatID: "chat-ambiguous-buyer-b", BuyerID: "buyer-ambiguous", ItemID: "item-ambiguous"},
	}
	// ambiguousBuyerSession 是当前待保存的买家商品歧义会话。
	for _, ambiguousBuyerSession := range ambiguousBuyerSessions {
		if // sessionErr 是保存当前买家商品歧义会话的错误。
		sessionErr := store.Chats.UpsertSession(ctx, ambiguousBuyerSession); sessionErr != nil {
			t.Fatalf("保存买家商品歧义会话 %s: %v", ambiguousBuyerSession.ChatID, sessionErr)
		}
	}

	// rows、total、listErr 是读取全部测试订单后的解析结果、总数和错误。
	rows, total, listErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, Limit: 20})
	if listErr != nil || total != 5 || len(rows) != 5 {
		t.Fatalf("订单列表 rows=%+v total=%d err=%v", rows, total, listErr)
	}
	// chatsByOrder 保存每笔订单只读解析出的精确会话标识。
	chatsByOrder := make(map[string]string, len(rows))
	// row 是当前待索引的订单列表行。
	for _, row := range rows {
		chatsByOrder[row.OrderID] = row.ChatID
	}
	if chatsByOrder["explicit-order"] != "chat-explicit" || chatsByOrder["card-order"] != "chat-card" || chatsByOrder["buyer-item-order"] != "chat-buyer-item" {
		t.Fatalf("唯一关联解析错误: %+v", chatsByOrder)
	}
	if chatsByOrder["ambiguous-card-order"] != "" || chatsByOrder["ambiguous-buyer-order"] != "" {
		t.Fatalf("歧义关联不应启用联系买家: %+v", chatsByOrder)
	}
	// persistedChatID 保存唯一订单卡片订单在 orders 表内的原值，证明列表查询没有隐式修改业务数据。
	var persistedChatID string
	if // readErr 是读取订单持久化 chat_id 原值的错误。
	readErr := store.DB.QueryRowContext(ctx, `SELECT COALESCE(chat_id,'') FROM orders WHERE order_id=?`, "card-order").Scan(&persistedChatID); readErr != nil || persistedChatID != "" {
		t.Fatalf("列表查询不应写回 chat_id value=%q err=%v", persistedChatID, readErr)
	}
}

// TestOrdersListForUserCombinesCreatedRangeWithExistingFilters 验证时间半开区间与账号、状态和搜索条件共同生效。
func TestOrdersListForUserCombinesCreatedRangeWithExistingFilters(t *testing.T) {
	// store、cleanup 是当前 SQLite 回归使用的隔离仓储和释放函数。
	store, cleanup := newTestDB(t)
	defer cleanup()
	// ctx 是订单时间范围查询使用的测试上下文。
	ctx := context.Background()
	// userID、cookieID 是隔离订单归属使用的本地用户和账号。
	userID, cookieID := seedAccount(t, store)
	// fixtures 保存边界前、范围内和排除上界三个订单及其平台创建时间。
	fixtures := []struct {
		// orderID 是测试订单标识，同时提供搜索命中词。
		orderID string
		// createdAt 是写入订单表的 UTC RFC3339 平台时间。
		createdAt string
	}{
		{orderID: "range-before", createdAt: "2026-08-20T00:00:00Z"},
		{orderID: "range-inside", createdAt: "2026-08-20T01:00:00Z"},
		{orderID: "range-upper-bound", createdAt: "2026-08-20T02:00:00Z"},
	}
	// fixture 是当前待写入的时间边界订单。
	for _, fixture := range fixtures {
		// upsertErr 是当前边界订单写入隔离数据库的错误。
		if upsertErr := store.Orders.Upsert(ctx, fixture.orderID, OrderUpsertOpts{CookieID: cookieID, BuyerID: "range-buyer", OrderStatus: "shipped", CreatedAt: fixture.createdAt}); upsertErr != nil {
			t.Fatalf("upsert %s: %v", fixture.orderID, upsertErr)
		}
	}
	// createdFrom、createdTo 定义只包含一小时整点订单的半开区间。
	createdFrom, createdTo := time.Date(2026, 8, 20, 0, 30, 0, 0, time.UTC), time.Date(2026, 8, 20, 2, 0, 0, 0, time.UTC)
	// rows、total、listErr 是组合筛选得到的订单行、总数和查询错误。
	rows, total, listErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, CookieID: cookieID, Status: "shipped", Search: "range", CreatedFrom: createdFrom, CreatedTo: createdTo, Limit: 20})
	if listErr != nil {
		t.Fatalf("ListForUser: %v", listErr)
	}
	if total != 1 || len(rows) != 1 || rows[0].OrderID != "range-inside" {
		t.Fatalf("rows=%+v total=%d, want only range-inside", rows, total)
	}
}

// TestOrdersListForUserCombinesInclusiveAmountRange 验证实付金额上下界按整数分包含匹配并与既有筛选组合。
func TestOrdersListForUserCombinesInclusiveAmountRange(t *testing.T) {
	// store、cleanup 是当前 SQLite 金额筛选回归使用的隔离仓储和释放函数。
	store, cleanup := newTestDB(t)
	defer cleanup()
	// ctx 是金额范围查询使用的测试上下文。
	ctx := context.Background()
	// userID、cookieID 是隔离订单归属使用的本地用户和账号。
	userID, cookieID := seedAccount(t, store)
	// fixtures 保存低于范围、两个包含边界和高于范围的规范金额。
	fixtures := []struct {
		// orderID 是当前金额订单标识，同时提供搜索命中词。
		orderID string
		// amount 是当前订单实付金额元文本。
		amount string
	}{
		{orderID: "amount-below", amount: "134.99"},
		{orderID: "amount-lower", amount: "135.00"},
		{orderID: "amount-upper", amount: "690.00"},
		{orderID: "amount-above", amount: "690.01"},
	}
	// fixture 是当前待写入的金额边界订单。
	for _, fixture := range fixtures {
		// upsertErr 是当前金额订单写入隔离数据库的错误。
		if upsertErr := store.Orders.Upsert(ctx, fixture.orderID, OrderUpsertOpts{CookieID: cookieID, BuyerID: "amount-buyer", OrderStatus: "shipped", Amount: fixture.amount}); upsertErr != nil {
			t.Fatalf("upsert %s: %v", fixture.orderID, upsertErr)
		}
	}
	// minAmountCents、maxAmountCents 定义包含 135 元和 690 元的整数分范围。
	minAmountCents, maxAmountCents := int64(13500), int64(69000)
	// rows、total、listErr 是组合筛选得到的订单行、总数和查询错误。
	rows, total, listErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, CookieID: cookieID, Status: "shipped", Search: "amount", MinAmountCents: &minAmountCents, MaxAmountCents: &maxAmountCents, Limit: 20})
	if listErr != nil {
		t.Fatalf("ListForUser: %v", listErr)
	}
	if total != 2 || len(rows) != 2 || rows[0].OrderID != "amount-upper" || rows[1].OrderID != "amount-lower" {
		t.Fatalf("rows=%+v total=%d, want inclusive boundaries", rows, total)
	}
	// maxOnlyRows、maxOnlyTotal、maxOnlyErr 验证只填最高金额也能独立生效。
	maxOnlyRows, maxOnlyTotal, maxOnlyErr := store.Orders.ListForUser(ctx, OrderListFilter{UserID: userID, CookieID: cookieID, Search: "amount", MaxAmountCents: &minAmountCents, Limit: 20})
	if maxOnlyErr != nil || maxOnlyTotal != 2 || len(maxOnlyRows) != 2 {
		t.Fatalf("max-only rows=%+v total=%d err=%v", maxOnlyRows, maxOnlyTotal, maxOnlyErr)
	}
}
