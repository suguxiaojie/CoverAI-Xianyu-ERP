...
func (k *Keywords) AllWithType(ctx context.Context, cookieID string) ([]Keyword, error) {
	...
	rows, err := k.DB.QueryContext(ctx,
		`SELECT keyword, reply, COALESCE(item_id,''), COALESCE(type,'text'), COALESCE(image_url,''), COALESCE(group_id,''), COALESCE(match_type,'contains'), COALESCE(message_scope,'customer'), COALESCE(system_types,''),reply_interval_seconds,send_delay_seconds
			 FROM keywords WHERE cookie_id=? AND enabled=? ORDER BY LENGTH(keyword) DESC,id ASC`, cookieID, true)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		err := rows.Scan(&kw.Keyword, &kw.Reply, &kw.ItemID, &kw.Type, &kw.ImageURL, &kw.GroupID, &kw.MatchType, &kw.MessageScope, &kw.SystemTypes, &kw.ReplyIntervalSeconds, &kw.SendDelaySeconds); err != nil {
			return nil, err
		}
		out = append(out, kw)
	}
