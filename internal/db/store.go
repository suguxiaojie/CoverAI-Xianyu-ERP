...
type Store struct {
	...
	// OrderShipmentProofs 保存 ERP 明确发货成功后的描述和官方图片地址。
	OrderShipmentProofs *OrderShipmentProofs
	// OrderSyncCursors 保存订单增量同步的账号级高水位，不包含平台凭证。
	OrderSyncCursors *OrderSyncCursors
	Keywords         *Keywords
	DefaultReps      *DefaultReplies
	ItemReps         *ItemReplies
	AIReply          *AIReply
	// Knowledge 保存人工知识库、FAQ／文档分块和脱敏检索日志。
...
func NewStore(db *sql.DB, dialect Dialect) *Store {
	...
	// items、orders 和 orderCosts 保存商品、订单与成本快照仓储；Unit of Work 必须复用它们。
	items, orders := &Items{DB: db, Dialect: dialect}, &Orders{DB: db, Dialect: dialect}
	// orderCosts 是订单写入事务共享的成本快照仓储。
	orderCosts := &OrderCosts{DB: db, Dialect: dialect}
	return &Store{
		DB:                  db,
		Dialect:             dialect,
		Users:               &Users{DB: db},
		...
		Orders:              orders,
		OrderCosts:          orderCosts,
		OrderWrites:         newOrderWriteUnitOfWork(db, orders, items, orderCosts),
		Reconciliations:     &OrderReconciliations{DB: db, Dialect: dialect},
		OrderRefreshJobs:    &OrderRefreshJobs{DB: db},
		OrderShipmentProofs: &OrderShipmentProofs{DB: db, Dialect: dialect},
		OrderSyncCursors:    &OrderSyncCursors{DB: db, Dialect: dialect},
		...
		Knowledge:           &KnowledgeStore{DB: db, Dialect: dialect},
