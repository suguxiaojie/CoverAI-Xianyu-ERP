...
// SystemKeywordReplyTargetRequest 是卖家自身订单事件解析买家回复目标的非敏感输入。
type SystemKeywordReplyTargetRequest struct {
	// AccountID 是收到实时系统事件的卖家账号。
	AccountID string
	// EventType 是稳定系统事件；当前只允许 order_shipped。
	EventType string
	// OrderID 是平台卡片明确提供的订单号；可能为空。
	OrderID string
	// ChatID 是系统卡片所属会话标识。
	ChatID string
	// BuyerID 是非自身通知已提供的买家标识；卖家自身卡片可能为空或等于账号。
	BuyerID string
	// OccurredAt 是平台事件时间，Unix 毫秒。
	OccurredAt int64
}

// SystemKeywordReplyTarget 是经订单归属和唯一性校验后的买家回复目标。
type SystemKeywordReplyTarget struct {
	// OrderID 是幂等记录使用的精确订单号。
	OrderID string
	// ChatID 是回复所属会话标识。
	ChatID string
	// BuyerID 是平台发送协议的目标买家标识。
	BuyerID string
}

// SystemKeywordReplyTargetResolver 是 Engine 消费的可选订单目标解析端口。
// 实现方只能返回当前账号精确订单或同会话时间窗内唯一已发货订单，不得猜测买家。
type SystemKeywordReplyTargetResolver interface {
	// ResolveSystemKeywordReplyTarget 返回安全目标和匹配标记；无法唯一关联时 matched=false。
	ResolveSystemKeywordReplyTarget(context.Context, SystemKeywordReplyTargetRequest) (target SystemKeywordReplyTarget, matched bool, err error)
}
...
// MessageRecallHandler 是可选的聊天撤回事件端口，旧 Handler 实现不必承担该能力。
type MessageRecallHandler interface {
	// HandleMessageRecall 接收不包含凭证的平台撤回事件。
	HandleMessageRecall(context.Context, MessageRecallEvent) error
}
...
// MessageRecallEvent 是平台 40104 撤回状态的非敏感事件载体。
type MessageRecallEvent struct {
	// AccountID 是当前运行时账号标识。
	AccountID string
	// ChatID 是平台会话标识。
	ChatID string
	// MessageID 是被撤回的 PNM 消息标识。
	MessageID string
	// OperatorType 区分发送者、群主、系统和安全撤回。
	OperatorType int
	// OperatorID 是撤回执行者平台标识。
	OperatorID string
	// RecalledAt 是平台事件时间，Unix 毫秒。
	RecalledAt int64
}
...
type ChatMessage struct {
	...
	// IsSystem 标记平台结构化系统卡片；该类消息立即持久化且绝不进入回复防抖。
	IsSystem bool
	// IsSelf 标记消息是否由当前闲鱼账号在任一官方客户端发出；该类消息只持久化，绝不进入自动回复。
	IsSelf bool
}
...
type OutgoingChatMessage struct {
	// AccountID 是实际执行发送的闲鱼账号标识。
	AccountID string
	// ChatID 是本次出站消息所属的精确会话标识。
	ChatID string
	// BuyerID 是接收本次消息的买家标识。
	BuyerID string
	// Text 是文字正文或已经上传到闲鱼 CDN 的媒体地址。
	Text string
	// MessageKey 是人工发送前创建的本地幂等键；自动化发送可以为空。
	MessageKey string
	// MessageType 是 text、image 或 location，用于聊天旁路选择正确的持久化模型。
	MessageType string
	// PlatformMessageID 是平台确认发送后返回的 PNM ID，自动化媒体使用它防止历史回显重复。
	PlatformMessageID string
	// SentAt 是平台确认的消息时间戳；缺失时聊天服务使用当前 UTC 毫秒时间。
	SentAt int64
}
