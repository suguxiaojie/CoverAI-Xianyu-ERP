...
// SendTextWithResult 发送人工文本并返回平台撤回所需的 PNM ID。
func (a *Account) SendTextWithResult(ctx context.Context, chatID, toUserID, text string) (PlatformSendResult, error) {
	return a.outgoing.sendTextWithResult(ctx, chatID, toUserID, text, "")
}

// SendReplyTextWithResult 发送原生引用文本，并返回平台分配给新消息的 PNM ID。
func (a *Account) SendReplyTextWithResult(ctx context.Context, chatID, toUserID, text, replyToPlatformMessageID string) (PlatformSendResult, error) {
	return a.outgoing.sendTextWithResult(ctx, chatID, toUserID, text, replyToPlatformMessageID)
}
...
// SendImageWithResult 发送人工图片并返回平台撤回所需的 PNM ID。
func (a *Account) SendImageWithResult(ctx context.Context, chatID, toUserID, imageURL string, cardID int64, width, height int) (PlatformSendResult, error) {
	return a.outgoing.sendImageWithResult(ctx, chatID, toUserID, imageURL, cardID, width, height)
}

// SendLocationCardWithResult 通过当前在线连接发送人工位置卡片，并返回平台 PNM ID。
func (a *Account) SendLocationCardWithResult(ctx context.Context, chatID, toUserID, title, description string, latitude, longitude float64) (PlatformSendResult, error) {
	return a.outgoing.sendLocationCardWithResult(ctx, chatID, toUserID, title, description, latitude, longitude)
}

// RecallMessage 撤回当前账号两分钟窗口内的文本或图片消息。
func (a *Account) RecallMessage(ctx context.Context, platformMessageID string, contentType int, text string) error {
	return a.outgoing.recallMessage(ctx, platformMessageID, contentType, text)
}
