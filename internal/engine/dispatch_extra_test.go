...
// systemReplyResolverHandler 为卖家自身发货卡片返回经订单校验的买家目标。
type systemReplyResolverHandler struct {
	recordingHandler
	// target 是测试预设的精确订单、会话和买家目标。
	target SystemKeywordReplyTarget
}

// ResolveSystemKeywordReplyTarget 返回测试预设目标，模拟生产 Adapter 的订单归属查询。
func (handler *systemReplyResolverHandler) ResolveSystemKeywordReplyTarget(context.Context, SystemKeywordReplyTargetRequest) (SystemKeywordReplyTarget, bool, error) {
	return handler.target, true, nil
}

// TestHandleMessage_SystemEventRoutesToAutomation 付款待发货系统卡片即时持久化并进入自动化中心，不进入回复防抖。
func TestHandleMessage_SystemEventRoutesToAutomation(t *testing.T) {
	...
	// 即时持久化发生在同步 handleMessage 内，不需要等待用户消息防抖。
	h.mu.Lock()
	if len(h.chats) != 1 || !h.chats[0].IsSystem {
		h.mu.Unlock()
		t.Fatalf("付款系统卡片未即时投递持久化端口: %+v", h.chats)
	}
	h.mu.Unlock()
	// 等待防抖周期后数量不应增加，证明系统卡片没有进入回复链。
	...
	h.mu.Unlock()
	if chats != 1 {
		t.Errorf("系统卡片错误进入回复防抖，got %d 条持久化投递", chats)
	}
}
...
func TestHandleMessage_SystemEventDispatchedToHandler(t *testing.T) {
	...
	h.recordingHandler.mu.Lock()
	defer h.recordingHandler.mu.Unlock()
	if len(h.chats) != 1 || !h.chats[0].IsSystem {
		t.Fatalf("系统事件没有同步投递聊天持久化: %+v", h.chats)
	}
}

// TestDispatchSystemKeywordReplyUsesSellerShipmentAndOrderIdempotency 验证卖家自身发货卡片按订单买家发送且后续通知不重复。
func TestDispatchSystemKeywordReplyUsesSellerShipmentAndOrderIdempotency(t *testing.T) {
	// store 和 cleanup 是系统关键词回复使用的隔离 SQLite 及释放函数。
	store, cleanup := newReplyStore(t)
	defer cleanup()
	// ctx 是本测试全部本地回复操作共用的上下文。
	ctx := context.Background()
	// insertErr 是创建只按发货系统事件触发规则的数据库结果。
	if _, insertErr := store.DB.ExecContext(ctx, `INSERT INTO keywords(cookie_id,keyword,reply,type,group_id,match_type,message_scope,system_types,enabled,reply_interval_seconds,send_delay_seconds) VALUES('cid','','发货规则回复','text','shipment-group','contains','system','order_shipped',1,1,0)`); insertErr != nil {
		t.Fatal(insertErr)
	}
	// sender 记录平台发送目标和文本，不连接真实 WebSocket。
	sender := &recordingSender{}
	// reply 是启用订单事件幂等的系统关键词回复服务。
	reply := NewReplyService("cid", store, sender, nil, nil, slog.Default())
	// dispatcher 是只调用系统关键词分发方法的最小消息分发器。
	dispatcher := messageDispatcher{cookieID: "cid", reply: reply, logger: slog.Default()}
	// handler 返回当前订单经归属校验的买家目标。
	handler := &systemReplyResolverHandler{target: SystemKeywordReplyTarget{OrderID: "order-1", ChatID: "chat-1", BuyerID: "buyer-1"}}
	// sellerCard 是实时卖家自身发货凭证卡片，现应作为首选触发源。
	sellerCard := ChatMessage{AccountID: "cid", ChatID: "chat-1", SenderUserID: "cid", Text: "[我已发货，请查看发货凭证]", MessageID: "seller-card.PNM", Raw: map[string]any{"1": map[string]any{"10": map[string]any{"extJson": `{"contentType":"26"}`}}}, IsSystem: true, IsSelf: true}
	// shipmentTask 是同一实时卡片解析出的精确订单事实。
	shipmentTask := &automation.Task{AccountID: "cid", TriggerType: automation.TriggerOrderShipped, OrderID: "order-1", ChatID: "chat-1", OccurredAt: time.Now().UnixMilli()}
	dispatcher.dispatchSystemKeywordReply(ctx, handler, sellerCard, shipmentTask)
	// buyerNotice 是同一订单后续买家侧发货通知，必须命中同一订单幂等而不重复发送。
	buyerNotice := ChatMessage{AccountID: "cid", ChatID: "chat-1", SenderUserID: "buyer-1", Text: "[你已发货]", MessageID: "buyer-notice.PNM", Raw: map[string]any{"1": map[string]any{"10": map[string]any{"extJson": `{"contentType":"14"}`}}}, IsSystem: true}
	dispatcher.dispatchSystemKeywordReply(ctx, handler, buyerNotice, &automation.Task{AccountID: "cid", TriggerType: automation.TriggerOrderShipped, ChatID: "chat-1", OccurredAt: time.Now().UnixMilli()})
	if len(sender.texts) != 1 || sender.texts[0].chatID != "chat-1" || sender.texts[0].toUserID != "buyer-1" || sender.texts[0].text != "发货规则回复" {
		t.Fatalf("system keyword sends=%+v", sender.texts)
	}
}
...
// TestHandleMessage_ContentType14Filtered contentType=14 系统提示即时持久化但不进入回复链。
func TestHandleMessage_ContentType14Filtered(t *testing.T) {
	...
	if len(h.chats) != 1 || !h.chats[0].IsSystem {
		h.mu.Unlock()
		t.Fatalf("contentType=14 未即时投递系统持久化: %+v", h.chats)
	}
	h.mu.Unlock()
	time.Sleep(MessageDebounceDelay + 200*time.Millisecond)
	h.mu.Lock()
	defer h.mu.Unlock()
	if len(h.chats) != 1 {
		t.Errorf("contentType=14 错误进入回复防抖，got %d", len(h.chats))
	}
}

// TestHandleMessage_RefundCardFiltered contentType=26 退款卡片即时持久化但不进入回复链。
func TestHandleMessage_RefundCardFiltered(t *testing.T) {
	...
	if len(h.chats) != 1 || !h.chats[0].IsSystem {
		h.mu.Unlock()
		t.Fatalf("退款卡片未即时投递系统持久化: %+v", h.chats)
	}
	h.mu.Unlock()
	time.Sleep(MessageDebounceDelay + 200*time.Millisecond)
	h.mu.Lock()
	defer h.mu.Unlock()
	if len(h.chats) != 1 {
		t.Errorf("退款卡片错误进入回复防抖，got %d", len(h.chats))
	}
}
...
// TestHandleMessage_PersistsOwnWebSocketEchoWithoutDebounce 验证当前账号回显只即时投递一次持久化端口。
func TestHandleMessage_PersistsOwnWebSocketEchoWithoutDebounce(t *testing.T) {
	// acc 是待测账号；h 记录持久化端口调用；cleanup 释放测试数据库和运行时资源。
	acc, h, _, cleanup := newAccountForTest(t)
	defer cleanup()
	defer acc.Stop()

	// newAccountForTest 使用 unb=123；模拟同一账号在闲鱼官方客户端发出的
	// 消息回显，它必须进入持久化端口但不能进入一秒回复防抖。
	acc.handleMessage(plainChatMessage(t, "官方客户端发送", "123", "chat-own"))
	h.mu.Lock()
	if len(h.chats) != 1 || !h.chats[0].IsSelf {
		h.mu.Unlock()
		t.Fatalf("账号自身消息回显未即时投递: %+v", h.chats)
	}
	h.mu.Unlock()
	time.Sleep(MessageDebounceDelay + 200*time.Millisecond)
	h.mu.Lock()
	defer h.mu.Unlock()
	if len(h.chats) != 1 {
		t.Fatalf("账号自身消息回显错误进入防抖链，got %d", len(h.chats))
	}
}
...
func TestExtractMessageReadEvent40103(t *testing.T) {
	...
	if !ok || event.MessageID != "4263141580162.PNM" || event.ChatID != "64725235816" || event.ReadAt != 1786945729928 {
...
// TestHandleMessageContextDeliversSelfEchoWithoutDebounce 验证己方回显立即进入持久化端口且不等待回复防抖。
func TestHandleMessageContextDeliversSelfEchoWithoutDebounce(t *testing.T) {
	// acc、handler、_、cleanup 保存测试账号、消息记录器、未使用存储及关闭函数。
	acc, handler, _, cleanup := newAccountForTest(t)
	defer cleanup()
	defer acc.Stop()
	// decrypted 模拟当前账号在其他官方客户端发出的文本回显。
	decrypted := map[string]any{"1": map[string]any{
		"2": "self-chat@goofish", "3": "self-fast.PNM", "5": float64(1787126165430),
		"10": map[string]any{"reminderContent": "跨平台即时消息", "senderUserId": "123", "senderNick": "我"},
	}}
	// handleMessage 同步处理己方回显；若错误进入一秒防抖，下面的立即断言会失败。
	acc.handleMessage(decrypted)
	handler.mu.Lock()
	defer handler.mu.Unlock()
	if len(handler.chats) != 1 || !handler.chats[0].IsSelf || handler.chats[0].MessageID != "self-fast.PNM" {
		t.Fatalf("己方回显未即时投递: %+v", handler.chats)
	}
}

// TestExtractMessageRecallEvent40104 验证紧凑和嵌套撤回事件都能生成规范结果。
func TestExtractMessageRecallEvent40104(t *testing.T) {
	// compact 是官方 MsgRecallEventModel 解码后的数字字段结构。
	compact := map[string]any{
		"1": "platform-1.PNM", "2": "chat-1@goofish", "5": float64(123456),
		"8": map[string]any{"2": float64(0), "4": "sender-1"},
	}
	// event、ok 保存紧凑事件解析结果。
	event, ok := extractMessageRecallEvent(compact)
	if !ok || event.MessageID != "platform-1.PNM" || event.ChatID != "chat-1" || event.OperatorType != 0 || event.OperatorID != "sender-1" || event.RecalledAt != 123456 {
		t.Fatalf("compact recall=%+v ok=%v", event, ok)
	}
	// nested 是兼容未解包业务类型信封。
	nested := map[string]any{"payload": map[string]any{"bizType": 40104, "messageId": "platform-2.PNM", "cid": "chat-2@goofish", "operatorUid": "sender-2"}}
	// nestedEvent、nestedOK 保存兼容事件解析结果。
	nestedEvent, nestedOK := extractMessageRecallEvent(nested)
	if !nestedOK || nestedEvent.MessageID != "platform-2.PNM" || nestedEvent.ChatID != "chat-2" || nestedEvent.OperatorID != "sender-2" {
		t.Fatalf("nested recall=%+v ok=%v", nestedEvent, nestedOK)
	}
}
