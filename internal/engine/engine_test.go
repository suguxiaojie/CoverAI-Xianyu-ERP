...
// TestExtractSystemCardMessageKeepsFilteredNotice 验证普通回复提取器过滤的系统卡片仍能进入即时持久化模型。
func TestExtractSystemCardMessageKeepsFilteredNotice(t *testing.T) {
	// decrypted 是 contentType=14 的结构化系统卡片。
	decrypted := mustContentType14Notice(t)
	// card 是只供持久化分支使用的系统消息模型。
	card := extractSystemCardMessage(decrypted, "cid", "unb=123;")
	if card == nil || !card.IsSystem || card.ChatID == "" || card.Text == "" {
		t.Fatalf("system card=%+v", card)
	}
}
...
// TestExtractChatMessageMarksOwnWebSocketEcho 验证账号自身的 WebSocket 回显会被保留为只持久化消息。
func TestExtractChatMessageMarksOwnWebSocketEcho(t *testing.T) {
	// decrypted 模拟账号自身发送后由同一 WebSocket 回传的解密聊天信封。
	decrypted := map[string]any{"1": map[string]any{
		"2": "chat-1@goofish", "3": "platform-self-1.PNM", "5": float64(1787126165430),
		"10": map[string]any{"reminderContent": "我在官方客户端发送的消息", "senderUserId": "self-1", "senderNick": "我"},
	}}
	// chat 是保留了平台消息键和己方标记的提取结果，后续分发器据此绕过自动回复。
	chat := extractChatMessage(decrypted, "account-1", "unb=self-1;")
	if chat == nil || !chat.IsSelf || chat.MessageID != "platform-self-1.PNM" || chat.Text != "我在官方客户端发送的消息" {
		t.Fatalf("账号自身发送的 WS 回显未被规范提取: %+v", chat)
	}
}
