...
func (d *messageDispatcher) dispatch(decrypted map[string]any) {
	...
	// isSystemEvent 表示消息需要可靠背压处理，包括自动化事件和所有结构化系统卡片。
	isSystemEvent := automation.ExtractTaskFromWS(d.cookieID, d.currentCookie(), decrypted) != nil || extractSystemCardMessage(decrypted, d.cookieID, d.currentCookie()) != nil
	if isSystemEvent {
		select {
		case d.sem <- struct{}{}:
		case <-ctx.Done():
...
func (d *messageDispatcher) handleMessageContext(ctx context.Context, decrypted map[string]any) {
	// recall、ok 保存平台撤回事件及是否命中 40104 模型。
	if recall, ok := extractMessageRecallEvent(decrypted); ok {
		recall.AccountID = d.cookieID
		// handler 是当前账号可选的业务事件处理器。
		if handler := d.currentHandler(); handler != nil {
			// recallHandler、supported 保存当前集成是否消费撤回事件。
			if recallHandler, supported := handler.(MessageRecallHandler); supported {
				// err 表示撤回事件持久化或广播失败。
				if err := recallHandler.HandleMessageRecall(ctx, recall); err != nil {
					d.logger.Warn("处理聊天撤回事件失败", "err", err, "message_id", recall.MessageID)
				}
			}
		}
		return
	...
	// systemCard 是实时 WS 中的结构化官方卡片；它必须立即持久化且绝不进入 AI 回复防抖。
	systemCard := extractSystemCardMessage(decrypted, d.cookieID, d.currentCookie())
	// task 是同一载荷可能携带的自动化业务事实。
	task := automation.ExtractTaskFromWS(d.cookieID, d.currentCookie(), decrypted)
	if systemCard != nil || task != nil {
		// handler 是当前账号的聊天持久化和自动化事件接收器。
		handler := d.currentHandler()
		if systemCard != nil && handler != nil && d.markAndCheckDedup(decrypted, systemCard) {
			if // persistErr 是实时系统卡片即时持久化错误。
			persistErr := handler.HandleChatMessage(ctx, *systemCard); persistErr != nil {
				d.logger.Error("实时系统卡片持久化失败", "err", persistErr, "chat_id", systemCard.ChatID)
			}
			d.dispatchSystemKeywordReply(ctx, handler, *systemCard, task)
		}
		if task != nil && handler != nil {
			if // taskErr 是同一卡片自动化事实处理错误。
			taskErr := handler.HandleSystemEvent(ctx, *task); taskErr != nil {
				d.logger.Error("处理系统自动化事件失败", "err", taskErr, "trigger", task.TriggerType)
			}
		}
		return
	}
	...
	if chat != nil && chat.Text != "" {
		...
		if chat.IsSelf {
			// handler 保存跨平台己方消息的持久化端口；己方回显必须同步处理，避免随后的已读或撤回事件先到。
			if handler := d.currentHandler(); handler != nil {
				// err 表示即时持久化失败；失败只记录，不得把己方内容送入自动回复。
				if err := handler.HandleChatMessage(ctx, *chat); err != nil {
					d.logger.Error("持久化账号自身聊天回显失败", "err", err, "chat_id", chat.ChatID)
				}
			}
			return
		}
...
// dispatchSystemKeywordReply 让普通非自身系统消息沿用旧路径，并为卖家自身发货事件解析买家和订单幂等键。
func (d *messageDispatcher) dispatchSystemKeywordReply(ctx context.Context, handler Handler, systemCard ChatMessage, task *automation.Task) {
	if d.reply == nil {
		return
	}
	// eventType 是聊天领域从同一实时原始载荷解析出的稳定系统事件。
	eventType := chatdomain.ClassifySystemEvent(systemCard.Raw, systemCard.Text)
	if eventType != automation.TriggerOrderShipped {
		if systemCard.IsSelf {
			return
		}
		// replyErr 是普通非自身系统卡片沿用旧关键词回复流程的结果。
		if replyErr := d.reply.HandleKeywordOnly(ctx, systemCard); replyErr != nil {
			d.logger.Error("系统消息关键词回复失败", "err", replyErr, "chat_id", systemCard.ChatID)
		}
		return
	}
	// resolver 是生产 Adapter 提供的订单归属和买家目标解析端口。
	resolver, supported := handler.(SystemKeywordReplyTargetResolver)
	if !supported {
		if !systemCard.IsSelf {
			// replyErr 是旧 Handler 不支持订单解析时非自身系统卡片的兼容回复结果。
			if replyErr := d.reply.HandleKeywordOnly(ctx, systemCard); replyErr != nil {
				d.logger.Error("系统发货关键词兼容回复失败", "err", replyErr, "chat_id", systemCard.ChatID)
			}
		}
		return
	}
	// orderID 和 occurredAt 优先取同一实时载荷解析出的自动化任务事实。
	orderID := ""
	// occurredAt 是平台系统事件时间，Unix 毫秒。
	var occurredAt int64
	if task != nil && task.TriggerType == automation.TriggerOrderShipped {
		orderID = task.OrderID
		occurredAt = task.OccurredAt
	}
	// target 是按当前账号订单或同会话唯一近期发货事实解析出的买家目标。
	target, matched, resolveErr := resolver.ResolveSystemKeywordReplyTarget(ctx, SystemKeywordReplyTargetRequest{AccountID: d.cookieID, EventType: eventType, OrderID: orderID, ChatID: systemCard.ChatID, BuyerID: systemCard.SenderUserID, OccurredAt: occurredAt})
	if resolveErr != nil {
		d.logger.Error("解析系统发货关键词回复目标失败", "err", resolveErr, "account", d.cookieID, "chat_id", systemCard.ChatID, "order_id", orderID)
		return
	}
	if !matched {
		d.logger.Warn("系统发货关键词回复未找到唯一订单买家，仅保留事件", "account", d.cookieID, "chat_id", systemCard.ChatID, "order_id", orderID)
		return
	}
	// replyMessage 是只替换为经订单校验买家目标的系统事件副本，原始卡片仍保持自身事实。
	replyMessage := systemCard
	replyMessage.ChatID = target.ChatID
	replyMessage.SenderUserID = target.BuyerID
	replyMessage.IsSelf = false
	// replyErr 是经订单解析和订单级幂等保护的发货关键词回复结果。
	if replyErr := d.reply.HandleKeywordEvent(ctx, replyMessage, eventType, target.OrderID); replyErr != nil {
		d.logger.Error("系统发货关键词回复失败", "err", replyErr, "account", d.cookieID, "chat_id", target.ChatID, "order_id", target.OrderID)
	}
}
