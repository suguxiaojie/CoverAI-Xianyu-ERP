...
// TestKeywordGroupMatchTypesAndSystemScope 验证多关键词组的包含、不包含、等于和系统消息白名单语义。
func TestKeywordGroupMatchTypesAndSystemScope(t *testing.T) {
	// contains 是任意一个关键词包含即命中的客户消息规则组。
	contains := []db.Keyword{{Keyword: "密码", MatchType: "contains", MessageScope: "customer"}, {Keyword: "账号", MatchType: "contains", MessageScope: "customer"}}
	if !keywordGroupMatches(contains, chatMsg("需要账号吗", "", "chat")) {
		t.Fatal("包含规则应命中任一关键词")
	}
	// excludes 是所有关键词均未出现时才命中的兜底规则组。
	excludes := []db.Keyword{{Keyword: "人工", MatchType: "excludes", MessageScope: "customer"}, {Keyword: "退款", MatchType: "excludes", MessageScope: "customer"}}
	if !keywordGroupMatches(excludes, chatMsg("你好", "", "chat")) || keywordGroupMatches(excludes, chatMsg("我要退款", "", "chat")) {
		t.Fatal("不包含规则语义异常")
	}
	// equals 是忽略首尾空白和英文大小写后的全文匹配。
	equals := []db.Keyword{{Keyword: "HELLO", MatchType: "equals", MessageScope: "customer"}}
	if !keywordGroupMatches(equals, chatMsg(" hello ", "", "chat")) || keywordGroupMatches(equals, chatMsg("hello呀", "", "chat")) {
		t.Fatal("等于规则语义异常")
	}
	// systemMessage 是 contentType=26 的结构化系统消息。
	systemMessage := chatMsg("我已付款", "", "chat")
	systemMessage.IsSystem = true
	systemMessage.Raw = map[string]any{"1": map[string]any{"10": map[string]any{"extJson": `{"contentType":"26"}`}}}
	// systemRule 是只允许 contentType=26 的系统消息关键词组。
	systemRule := []db.Keyword{{Keyword: "付款", MatchType: "contains", MessageScope: "system", SystemTypes: "order_paid"}}
	if !keywordGroupMatches(systemRule, systemMessage) {
		t.Fatal("白名单系统消息应命中")
	}
	// eventOnlyRule 没有关键词时仅依赖稳定系统事件触发。
	eventOnlyRule := []db.Keyword{{Keyword: "", MatchType: "contains", MessageScope: "system", SystemTypes: "order_paid"}}
	if !keywordGroupMatches(eventOnlyRule, systemMessage) {
		t.Fatal("仅系统事件规则无需关键词也应命中")
	}
	systemRule[0].SystemTypes = "order_shipped"
	if keywordGroupMatches(systemRule, systemMessage) {
		t.Fatal("非白名单系统消息不得命中")
	}
}
