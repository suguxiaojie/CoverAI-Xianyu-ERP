...
// manualCostCandidates 返回当前用户仍需人工确认的历史金额分组。
func (s *Server) manualCostCandidates(w http.ResponseWriter, r *http.Request) {
	// sess 是当前认证用户会话。
	sess := auth.SessionFromContext(r.Context())
	// query 是当前 Dashboard 用户、账号、日期和浏览器时区组成的历史成本范围。
	query := analyticsapp.Query{UserID: sess.UserID, AccountID: r.URL.Query().Get("account_id"), StartDate: r.URL.Query().Get("start_date"), EndDate: r.URL.Query().Get("end_date"), Location: analyticsapp.LocationFromOffset(r.URL.Query().Get("timezone_offset_minutes"))}
	// candidates、queryErr 是应用层返回的人工候选和查询错误。
	candidates, queryErr := s.analyticsApplication().ManualCostCandidates(r.Context(), query)
	if queryErr != nil {
		writeErr(w, http.StatusInternalServerError, "读取历史成本候选失败")
		return
	}
	// response 是逐字段映射后的具名人工候选 DTO。
	response := make([]manualCostCandidateResponse, 0, len(candidates))
	// candidate 是当前映射为 HTTP DTO 的人工金额分组。
	for _, candidate := range candidates {
		// skus 是当前金额分组的全部可选 SKU DTO。
		skus := make([]manualCostSKUResponse, 0, len(candidate.SKUs))
		// sku 是当前映射为人工选择 DTO 的成本 SKU。
		for _, sku := range candidate.SKUs {
			skus = append(skus, manualCostSKUResponse{SKUID: sku.SKUID, Label: sku.Label, PriceCents: sku.PriceCents, CostCents: sku.CostCents})
		}
		response = append(response, manualCostCandidateResponse{AccountID: candidate.AccountID, ItemID: candidate.ItemID, ItemTitle: candidate.ItemTitle, AmountCents: candidate.AmountCents, Quantity: candidate.Quantity, OrderCount: candidate.OrderCount, SuggestedSKUID: candidate.SuggestedSKUID, Confidence: candidate.Confidence, SKUs: skus, SuggestedUnitCostCents: candidate.SuggestedUnitCostCents, ManualOnly: candidate.ManualOnly})
	}
	writeJSON(w, http.StatusOK, response)
}

// confirmManualCostCandidate 校验 JSON 请求并为人工选择的金额分组生成成本快照。
func (s *Server) confirmManualCostCandidate(w http.ResponseWriter, r *http.Request) {
	// sess 是当前认证用户会话。
	sess := auth.SessionFromContext(r.Context())
	// request 是人工确认请求 DTO。
	var request manualCostConfirmationRequest
	if // decodeErr 是人工确认 JSON 请求解析错误。
	decodeErr := json.NewDecoder(r.Body).Decode(&request); decodeErr != nil {
		writeErr(w, http.StatusBadRequest, "历史成本确认参数无效")
		return
	}
	// location 是由浏览器分钟偏移校验后得到的 Dashboard 日期边界时区。
	location := analyticsapp.LocationFromOffset(strconv.Itoa(request.TimezoneOffset))
	// result、confirmErr 是应用层在当前 Dashboard 日期范围内批量生成成本快照的结果和错误。
	result, confirmErr := s.analyticsApplication().ConfirmManualCostCandidate(r.Context(), analyticsapp.ManualCostConfirmation{UserID: sess.UserID, AccountID: request.AccountID, ItemID: request.ItemID, AmountCents: request.AmountCents, Quantity: request.Quantity, StartAt: analyticsapp.DateBoundary(request.StartDate, false, location), EndBefore: analyticsapp.DateBoundary(request.EndDate, true, location), SKUID: request.SKUID, CustomUnitCostCents: request.CustomUnitCostCents})
	if confirmErr != nil {
		writeErr(w, http.StatusBadRequest, "历史成本确认失败")
		return
	}
	writeJSON(w, http.StatusOK, manualCostConfirmationResponse{Success: true, MatchedOrders: result.MatchedOrders, MatchSource: result.MatchSource})
}
...
func (s *Server) orderAnalytics(w http.ResponseWriter, r *http.Request) {
	...
	query := analyticsapp.Query{
		UserID: sess.UserID, AccountID: r.URL.Query().Get("account_id"), StartDate: r.URL.Query().Get("start_date"), EndDate: r.URL.Query().Get("end_date"),
...
func orderAnalyticsResponseFromApplication(result analyticsapp.OrderAnalytics) orderAnalyticsResponse {
	...
	for _, item := range result.ItemStats {
		itemStats = append(itemStats, analyticsItemStatsResponse{ItemID: item.ItemID, ItemTitle: item.ItemTitle, OrderCount: item.OrderCount, TotalAmount: item.TotalAmount, AvgAmount: item.AvgAmount})
	}
	// dailyProfitStats 是应用层按日毛利结果对应的传输 DTO 列表。
	dailyProfitStats := make([]analyticsDailyProfitStatsResponse, 0, len(result.DailyProfitStats))
	// item 是当前按日毛利统计结果。
	for _, item := range result.DailyProfitStats {
		dailyProfitStats = append(dailyProfitStats, analyticsDailyProfitStatsResponse{Date: item.Date, TotalOrders: item.TotalOrders, CoveredOrders: item.CoveredOrders, TotalRevenue: item.RevenueAmount, CoveredRevenue: item.CoveredRevenue, ProductCost: item.CostAmount, PlatformFee: item.PlatformFee, GrossProfit: item.GrossProfit})
	}
	// itemProfitStats 是应用层按商品毛利结果对应的传输 DTO 列表。
	itemProfitStats := make([]analyticsItemProfitStatsResponse, 0, len(result.ItemProfitStats))
	// item 是当前按商品毛利统计结果。
	for _, item := range result.ItemProfitStats {
		itemProfitStats = append(itemProfitStats, analyticsItemProfitStatsResponse{ItemID: item.ItemID, ItemTitle: item.ItemTitle, TotalOrders: item.TotalOrders, CoveredOrders: item.CoveredOrders, CoveredRevenue: item.CoveredRevenue, ProductCost: item.CostAmount, PlatformFee: item.PlatformFee, GrossProfit: item.GrossProfit, GrossMargin: item.GrossMarginPercent, CoverageRate: item.CoveragePercent})
	}
	return orderAnalyticsResponse{
		...
		ProfitStats:      analyticsProfitStatsResponse{TotalOrders: result.ProfitStats.TotalOrders, CoveredOrders: result.ProfitStats.CoveredOrders, UnknownCostOrders: result.ProfitStats.UnknownOrders, CoveredRevenue: result.ProfitStats.CoveredRevenue, ProductCost: result.ProfitStats.CostAmount, PlatformFee: result.ProfitStats.PlatformFee, GrossProfit: result.ProfitStats.GrossProfit, RealizedGrossProfit: result.ProfitStats.RealizedGrossProfit, GrossMargin: result.ProfitStats.GrossMarginPercent, CoverageRate: result.ProfitStats.CoveragePercent},
		DailyProfitStats: dailyProfitStats, ItemProfitStats: itemProfitStats,
	}
}
