...
type cookieLoginInfoRequest struct {
	...
	LoginPassword string `json:"login_password"`
	// ShowBrowser 表示登录时展示浏览器窗口，并在 token 风控中等待人工验证。
	ShowBrowser bool `json:"show_browser"`
...
func (s *Server) listCookieDetails(w http.ResponseWriter, r *http.Request) {
	...
	for _, summary := range summaries {                        // summary 是当前账号的非敏感摘要。
		...
		result = append(result, cookieSummaryResponse{
			...
			AvatarURL:                   normalizeRemoteMediaURL(summary.AvatarURL),
			...
			AutoRequestFlowerEnabled:    tasks.AutoRequestFlowerEnabled,
			RequestFlowerAfterHours:     tasks.RequestFlowerAfterHours,
			RequestFlowerAfterSeconds:   tasks.RequestFlowerAfterSeconds,
			AutoReceiveFlowerEnabled:    tasks.AutoReceiveFlowerEnabled,
			ReceiveFlowerShowBrowser:    tasks.ReceiveFlowerShowBrowser,
			ReceiveFlowerTimeoutSeconds: tasks.ReceiveFlowerTimeoutSeconds,
			AutoReceiptReminderEnabled:  tasks.AutoReceiptReminderEnabled,
			ReceiptReminderAfterDays:    tasks.ReceiptReminderAfterDays,
			ReceiptReminderTime:         tasks.ReceiptReminderTime,
			ReceiptReminderMessage:      tasks.ReceiptReminderMessage,
			ReceiptReminderEnabledAt:    tasks.ReceiptReminderEnabledAt,
...
func (s *Server) getCookieDetails(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, cookieDetailResponse{
		...
		Username: summary.Username, Nickname: cachedCookieSummaryNickname(summary), AvatarURL: normalizeRemoteMediaURL(summary.AvatarURL),
		...
		AutoRequestFlowerEnabled: tasks.AutoRequestFlowerEnabled, RequestFlowerAfterHours: tasks.RequestFlowerAfterHours,
		RequestFlowerAfterSeconds: tasks.RequestFlowerAfterSeconds,
		AutoReceiveFlowerEnabled:  tasks.AutoReceiveFlowerEnabled, ReceiveFlowerShowBrowser: tasks.ReceiveFlowerShowBrowser,
		ReceiveFlowerTimeoutSeconds: tasks.ReceiveFlowerTimeoutSeconds,
		AutoReceiptReminderEnabled:  tasks.AutoReceiptReminderEnabled,
		ReceiptReminderAfterDays:    tasks.ReceiptReminderAfterDays,
		ReceiptReminderTime:         tasks.ReceiptReminderTime,
...
func (s *Server) refreshCookieProfile(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, cookieProfileResponse{
		Success: profile.ErrorMessage == "", ID: cid, Nickname: profile.Nickname, AvatarURL: normalizeRemoteMediaURL(profile.AvatarURL), ProfileError: profile.ErrorMessage,
	})
}
...
// updateCookieLoginInfo 更新账号登录信息（用户名、密码、浏览器显示与 token 风控人工接管设置）。
