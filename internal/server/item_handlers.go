...
func (s *Server) getItem(w http.ResponseWriter, r *http.Request) {
	...
	writeJSON(w, http.StatusOK, itemDetailResponse{
		...
		SKUCount: itemSKUCount(it.SKUs), SKUs: itemSKUsToResponse(it.SKUs),
	})
}
...
func itemImageFromDetail(detail string) string {
	...
	pic, ok := m["pic_info"].(map[string]any); ok {
		if // mediaURL、ok 是商品主图原始地址及其类型判断。
		mediaURL, ok := pic["picUrl"].(string); ok {
			return normalizeRemoteMediaURL(mediaURL)
		}
	}
	if // mediaURL、ok 是兼容字段中的商品主图地址及其类型判断。
	mediaURL, ok := m["item_image"].(string); ok {
		return normalizeRemoteMediaURL(mediaURL)
	}
	return ""
}
