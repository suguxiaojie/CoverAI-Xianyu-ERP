...
func TestSyncItemsFromAccountSuccess(t *testing.T) {
	...
	setTestMTop(srv, withMTopTransport(roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		if strings.Contains(req.URL.String(), "mtop.idle.pc.idleitem.editdetail") {
			// 卖家编辑详情暂时失败时必须保留已有本地多规格配置和旧 SKU。
			body = `{"ret":["FAIL_SYS_TEMPORARILY_UNAVAILABLE::稍后重试"],"data":{}}`
		}
		return &http.Response{
			StatusCode: http.StatusOK,
			Header:     make(http.Header),
...
func TestSyncItemsFromAccountDetectsMultiSpecFromDetail(t *testing.T) {
	...
	setTestMTop(srv, withMTopTransport(roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		} else if strings.Contains(req.URL.String(), "mtop.idle.pc.idleitem.editdetail") {
			body = `{"ret":["SUCCESS::调用成功"],"data":{"quantity":"1952","itemSkuList":[` +
				`{"skuId":"a","inventoryId":"inventory-a","quantity":"955","priceInCent":"13500","features":{"idleOriginalQuantity":"1000","sku_order_num":"0"},"propertyList":[{"propertyText":"套餐","valueText":"一倍额度","enabled":true}]},` +
				`{"skuId":"b","inventoryId":"inventory-b","quantity":"997","priceInCent":"69000","features":{"idleOriginalQuantity":"1000","sku_order_num":"1"},"propertyList":[{"propertyText":"套餐","valueText":"五倍额度","enabled":true}]}` +
				`]}}`
		}
		return &http.Response{StatusCode: http.StatusOK, Header: make(http.Header), Body: io.NopCloser(strings.NewReader(body)), Request: req}, nil
	})))
	...
	// skus、skuErr 验证卖家编辑详情中的当前库存和初始库存已结构化保存。
	skus, skuErr := store.Items.ListSKUs(context.Background(), "acc1", "multi-item")
	if skuErr != nil || len(skus) != 2 || skus[0].Quantity != 955 || skus[0].InitialQuantity != 1000 || skus[1].Quantity != 997 {
		t.Fatalf("skus=%+v err=%v", skus, skuErr)
	}
}
