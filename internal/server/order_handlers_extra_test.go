...
// TestParseOrderListRequestValidatesCreatedRange 验证订单列表 HTTP 查询接受 RFC3339 半开区间并拒绝无效输入。
func TestParseOrderListRequestValidatesCreatedRange(t *testing.T) {
	// validRequest 是同时携带分页、搜索和合法 UTC 时间范围的请求。
	validRequest := httptest.NewRequest(http.MethodGet, "/api/v1/orders?page=2&page_size=30&search=buyer&created_from=2026-08-20T00:00:00Z&created_to=2026-08-21T00:00:00Z", nil)
	// parsed、parseErr 是合法查询的具名解析结果和错误。
	parsed, parseErr := parseOrderListRequest(validRequest)
	if parseErr != nil {
		t.Fatalf("parse valid request: %v", parseErr)
	}
	if parsed.Page != 2 || parsed.PageSize != 30 || parsed.Search != "buyer" || parsed.CreatedFrom.Format(time.RFC3339) != "2026-08-20T00:00:00Z" || parsed.CreatedTo.Format(time.RFC3339) != "2026-08-21T00:00:00Z" {
		t.Fatalf("parsed=%+v", parsed)
	}
	// invalidFormatRequest 携带无法解析的创建时间。
	invalidFormatRequest := httptest.NewRequest(http.MethodGet, "/api/v1/orders?created_from=not-a-time", nil)
	// invalidErr 是非法 RFC3339 时间的解析错误。
	if _, invalidErr := parseOrderListRequest(invalidFormatRequest); invalidErr == nil {
		t.Fatal("invalid RFC3339 time should fail")
	}
	// reversedRequest 携带不晚于开始时间的上界。
	reversedRequest := httptest.NewRequest(http.MethodGet, "/api/v1/orders?created_from=2026-08-21T00:00:00Z&created_to=2026-08-20T00:00:00Z", nil)
	// reversedErr 是倒置时间范围的解析错误。
	if _, reversedErr := parseOrderListRequest(reversedRequest); reversedErr == nil {
		t.Fatal("reversed time range should fail")
	}
}

// TestParseOrderListRequestValidatesAmountRange 验证金额查询严格转换为整数分并拒绝非法或倒置边界。
func TestParseOrderListRequestValidatesAmountRange(t *testing.T) {
	// validRequest 是同时携带单小数下界和两位小数上界的合法查询。
	validRequest := httptest.NewRequest(http.MethodGet, "/api/v1/orders?min_amount=135.5&max_amount=690.00", nil)
	// parsed、parseErr 是合法金额范围的解析结果和错误。
	parsed, parseErr := parseOrderListRequest(validRequest)
	if parseErr != nil || parsed.MinAmountCents == nil || *parsed.MinAmountCents != 13550 || parsed.MaxAmountCents == nil || *parsed.MaxAmountCents != 69000 {
		t.Fatalf("parsed=%+v err=%v", parsed, parseErr)
	}
	// invalidValues 保存必须被拒绝的金额格式或倒置范围。
	invalidValues := []string{
		"/api/v1/orders?min_amount=-1",
		"/api/v1/orders?min_amount=1.001",
		"/api/v1/orders?min_amount=20&max_amount=10",
		"/api/v1/orders?max_amount=999999999999999999999999",
	}
	// requestURL 是当前待验证的非法金额查询地址。
	for _, requestURL := range invalidValues {
		// invalidRequest 是当前非法金额范围请求。
		invalidRequest := httptest.NewRequest(http.MethodGet, requestURL, nil)
		// invalidErr 是当前非法金额范围的解析错误。
		_, invalidErr := parseOrderListRequest(invalidRequest)
		if invalidErr == nil {
			t.Fatalf("invalid amount query should fail: %s", requestURL)
		}
	}
}
...
func TestRefreshOrdersNoBrowser(t *testing.T) {
	...
	if !strings.Contains(result.Message, "订单同步完成") {
...
func TestManualShipOrdersStatusOnlySuccess(t *testing.T) {
	...
	// client 返回当前确认发货接口要求的明确订单成功字段，避免复用账号资料响应伪造成功。
	client := mtop.NewClient()
	client.HTTPClient = &http.Client{Transport: roundTripFunc(func(request *http.Request) (*http.Response, error) {
		return &http.Response{StatusCode: http.StatusOK, Header: make(http.Header), Request: request, Body: io.NopCloser(strings.NewReader(`{"ret":["SUCCESS::调用成功"],"data":{"orderId":"ord-m"}}`))}, nil
	})}
...
func TestOrderResponseMappingAndErrorClassification(t *testing.T) {
	...
	row := orderapp.OrderRow{OrderID: "mapped-order", ItemID: "mapped-item", ItemTitle: "测试商品", ItemDetail: `{"pic_info":{"picUrl":"https://img.example/mapped.png"}}`, BuyerName: "测试买家", BuyerAvatar: "https://img.example/buyer.png", ChatID: "mapped-chat", OrderStatus: "2"}
	// view 用于本次流程后续判断的view
	view := orderDTOFromRow(row)
	if view.OrderStatus != "pending_ship" || view.Status != "pending_ship" || view.ItemImage != "https://img.example/mapped.png" || view.BuyerName != "测试买家" || view.BuyerAvatarURL != "https://img.example/buyer.png" || view.ChatID != "mapped-chat" {
