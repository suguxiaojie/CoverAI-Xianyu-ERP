...
func (a *orderHTTPAdapter) Refresh(ctx context.Context, userID int64, cookieID, status string) (orderRefreshResponse, error) {
	...
	return orderRefreshResponse{PartialFailure: result.PartialFailure, Message: result.Message, Summary: orderRefreshSummary{AccountTotal: result.Summary.AccountTotal, AccountSucceeded: result.Summary.AccountSucceeded, AccountFailed: result.Summary.AccountFailed, Discovered: result.Summary.Discovered, ListUpdated: result.Summary.ListUpdated, SoftDeleted: result.Summary.SoftDeleted, DetailTotal: result.Summary.DetailTotal, Total: result.Summary.Total, Updated: result.Summary.Updated, NoChange: result.Summary.NoChange, Failed: result.Summary.Failed, ConflictSkipped: result.Summary.ConflictSkipped}, Results: refreshResultsFromApplication(result.Results)}, nil
}
...
func refreshResultsFromApplication(items []orderapp.RefreshOrderResult) []orderRefreshResultDTO {
	...
	for _, item := range items {
		...
		if item.CookieID != "" {
			...
			row.ConflictSkipped = intPointer(item.ConflictSkipped)
			if item.Success {
...
type orderListResult struct {
	...
	SettlementSummary orderSettlementSummaryResponse
}

// orderAmountFromCents 把非负人民币分格式化为固定两位小数的元金额文本。
func orderAmountFromCents(cents int64) string {
	if cents < 0 {
		cents = 0
	}
...
func orderDTOFromRow(row orderapp.OrderRow) orderDTO {
	...
	return orderDTO{
		...
		ItemImage: itemImageFromDetail(row.ItemDetail), BuyerID: row.BuyerID, ChatID: row.ChatID, BuyerName: row.BuyerName, BuyerAvatarURL: normalizeRemoteMediaURL(row.BuyerAvatar),
		...
		IsBargain: row.IsBargain, SystemShipped: row.SystemShipped, ShipmentProofAvailable: row.ShipmentProofAvailable,
		ShipmentProofUnavailableReason: shipmentProofUnavailableReason(row.ShipmentProofAvailable, row.SystemShipped, status),
		...
		CreatedAt: row.CreatedAt, UpdatedAt: row.UpdatedAt, PaidAt: row.PaidAt, ShippedAt: row.ShippedAt, ReceivedAt: row.ReceivedAt,
		CompletedAt: row.CompletedAt, RefundedAt: row.RefundedAt, CancelledAt: row.CancelledAt,
	}
}

// shipmentProofUnavailableReason 根据本地凭证、ERP 发货标记和订单状态解释按钮不可用原因。
func shipmentProofUnavailableReason(available, systemShipped bool, status string) string {
	if available {
		return ""
	}
	if systemShipped {
		return "该订单为升级前 ERP 发货，未保存发货凭证"
	}
	if status == "shipped" || status == "received" || status == "completed" || status == "refunding" || status == "refunded" {
		return "该订单不是通过 ERP 发货，暂无可查看凭证"
	}
	return "订单尚未通过 ERP 完成发货"
}
...
func orderDTOFromOrder(order *orderapp.Order, item *orderapp.ItemInfo) orderDTO {
	...
	return orderDTO{
		...
		BuyerID: order.BuyerID, ChatID: order.ChatID, SpecName: order.SpecName, SpecValue: order.SpecValue,
		...
		CreatedAt: order.CreatedAt, UpdatedAt: order.UpdatedAt, PaidAt: order.PaidAt, ShippedAt: order.ShippedAt, ReceivedAt: order.ReceivedAt,
		CompletedAt: order.CompletedAt, RefundedAt: order.RefundedAt, CancelledAt: order.CancelledAt,
	}
}
...
func (a *orderHTTPAdapter) List(ctx context.Context, query orderListQuery) (orderListResult, error) {
	...
	return orderListResult{
		...
		SettlementSummary: orderSettlementSummaryResponse{
			OrderCount: result.Settlement.OrderCount, GrossAmount: orderAmountFromCents(result.Settlement.GrossAmountCents),
			ServiceFee: orderAmountFromCents(result.Settlement.ServiceFeeCents), PendingAmount: orderAmountFromCents(result.Settlement.PendingAmountCents),
			ServiceFeeRate: "1.6%",
		},
	}, nil
}
...
// ShipWithEvidence 转发聊天付款卡片的单订单凭证发货请求。
func (a *orderHTTPAdapter) ShipWithEvidence(ctx context.Context, request orderapp.ShipmentEvidenceRequest) (orderapp.ShipmentEvidenceResult, error) {
	return a.services.ShipWithEvidence(ctx, request)
}

// ShipmentProof 读取当前用户订单的 ERP 发货凭证。
func (a *orderHTTPAdapter) ShipmentProof(ctx context.Context, userID int64, orderID string) (orderapp.ShipmentProofResult, error) {
	return a.services.ShipmentProof(ctx, userID, orderID)
}

// RequestRedFlower 执行人工确认的订单求花并保留应用层稳定状态。
func (a *orderHTTPAdapter) RequestRedFlower(ctx context.Context, userID int64, orderID string) (orderapp.RedFlowerRequestResult, error) {
	return a.services.RequestRedFlower(ctx, orderapp.RedFlowerRequest{UserID: userID, OrderID: orderID})
}

// RedFlowerStatus 读取订单持久化求花状态，不触发平台请求。
func (a *orderHTTPAdapter) RedFlowerStatus(ctx context.Context, userID int64, orderID string) (orderapp.RedFlowerRequestStatusResult, error) {
	return a.services.RedFlowerStatus(ctx, userID, orderID)
}
