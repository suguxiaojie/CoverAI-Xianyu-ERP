...
func (s *Server) completeQRVerification(w http.ResponseWriter, r *http.Request) {
	...
	// req 保存具名扫码验证完成请求；先校验本地策略，避免无效模式仍调用平台完成接口。
	...
	req.CaptchaMode = strings.TrimSpace(req.CaptchaMode)
	if req.CaptchaMode != "" && req.CaptchaMode != "automatic" && req.CaptchaMode != "manual" {
		writeErrCode(w, http.StatusBadRequest, "invalid_captcha_mode", "Token 滑块处理方式无效", "")
		return
	}
	// qrService 是已完成平台依赖校验的二维码服务。
	qrService := s.qrLoginApplication()
	...
	if sess != nil {
		...
		persisted, persistErr := s.accountLoginApplication().PersistQRLoginSuccess(r.Context(), sess.UserID, sessionID, result, req.TargetAccountID, req.CaptchaMode)
		if persistErr != nil {
			if s.Logger != nil {
