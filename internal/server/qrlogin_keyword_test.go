...
type fakeQRLoginService struct {
	...
	completeUNB      string
	completeErr      error
	// completeCalls 记录平台验证完成接口调用次数，用于确认无效本地模式不会触发平台动作。
	completeCalls int
}
...
func TestCompleteQRVerificationPersistsAndReenablesAccount(t *testing.T) {
	...
	// req 用于本次流程后续判断的req
	req := httptest.NewRequest(http.MethodPost, "/qr-login/complete-verification/s1", strings.NewReader(`{"captcha_mode":"manual"}`))
	req.AddCookie(cookie)
	// rec 用于本次流程后续判断的rec
	rec := httptest.NewRecorder()
	h.ServeHTTP(rec, req)
	...
	// showBrowser 保存扫码请求显式人工模式落库后的兼容字段。
	var showBrowser int
	if // showBrowserErr 保存人工模式兼容字段的查询错误。
	showBrowserErr := store.DB.QueryRowContext(ctx, `SELECT show_browser FROM cookies WHERE id=?`, "acc1").Scan(&showBrowser); showBrowserErr != nil || showBrowser != 1 {
		t.Fatalf("扫码人工模式未保存: show_browser=%d err=%v", showBrowser, showBrowserErr)
	}
...
// TestCompleteQRVerificationRejectsInvalidCaptchaModeBeforePlatform 验证未知模式在平台验证和凭证写入前返回 400。
func TestCompleteQRVerificationRejectsInvalidCaptchaModeBeforePlatform(t *testing.T) {
	// srv、store、cleanup 保存隔离 Server、数据库和释放函数。
	srv, store, cleanup := newTestServer(t)
	defer cleanup()
	// qr 是记录平台调用次数的二维码服务替身。
	qr := &fakeQRLoginService{completeCookies: "unb=acc1; token=fresh;", completeUNB: "acc1"}
	setTestQRLogin(srv, qr)
	ownQRSession(t, srv, store, "invalid-mode")
	// handler 是包含认证和二维码路由的测试处理器。
	handler := srv.Router()
	// sessionCookie 是当前用户登录管理后台后的会话 Cookie。
	sessionCookie := loginHelper(t, handler)
	// request 携带不受支持的滑块枚举。
	request := httptest.NewRequest(http.MethodPost, "/qr-login/complete-verification/invalid-mode", strings.NewReader(`{"captcha_mode":"unexpected"}`))
	request.AddCookie(sessionCookie)
	// recorder 保存无效模式响应。
	recorder := httptest.NewRecorder()
	handler.ServeHTTP(recorder, request)
	if recorder.Code != http.StatusBadRequest || qr.completeCalls != 0 {
		t.Fatalf("无效模式不应调用平台: status=%d calls=%d body=%s", recorder.Code, qr.completeCalls, recorder.Body.String())
	}
}
