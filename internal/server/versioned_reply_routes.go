...
func (s *Server) mountVersionedReplyRoutes(r chi.Router) {
	r.Group(func(r chi.Router) {
		...
		r.Get("/api/v1/reply-rule-groups/{cid}", s.listKeywordGroups)
		r.Post("/api/v1/reply-rule-groups/{cid}", s.saveKeywordGroup)
		r.Put("/api/v1/reply-rule-groups/{cid}/{group_id}", s.saveKeywordGroup)
		r.Delete("/api/v1/reply-rule-groups/{cid}/{group_id}", s.deleteKeywordGroup)
		r.Get("/api/v1/global-reply-rule-groups", s.listGlobalKeywordGroups)
		r.Post("/api/v1/global-reply-rule-groups", s.saveGlobalKeywordGroup)
		r.Put("/api/v1/global-reply-rule-groups/{group_id}", s.saveGlobalKeywordGroup)
		r.Delete("/api/v1/global-reply-rule-groups/{group_id}", s.deleteGlobalKeywordGroup)
		r.Put("/api/v1/global-reply-rule-groups/enabled", s.setAllGlobalKeywordGroupsEnabled)
		r.Put("/api/v1/global-reply-rule-groups/{group_id}/enabled", s.setGlobalKeywordGroupEnabled)
		r.Put("/api/v1/global-reply-rule-groups/{group_id}/accounts/{account_id}/enabled", s.setGlobalKeywordGroupAccountEnabled)
