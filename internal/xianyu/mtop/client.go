...
// ConsignAPI 是当前卖家工作台“无需寄件”确认发货端点。
const ConsignAPI = "https://h5api.m.goofish.com/h5/mtop.taobao.idle.logistics.merchant.consign.dummy/1.0/"
...
type ClientImpl struct {
	...
	// ItemEditDetailURL 是测试环境覆盖的卖家商品编辑详情接口；生产为空时使用官方网页端点。
	ItemEditDetailURL string
	LoginUserURL      string
	RateCreateURL     string
	RateListURL       string
	// RedFlowerURL 是测试或兼容环境覆盖的小红花求花接口地址；生产为空时使用官方端点。
	RedFlowerURL string
	// ReceiptReminderURL 是测试环境覆盖的官方提醒买家确认收货接口；生产为空时使用官方端点。
	ReceiptReminderURL  string
	PolishItemURL       string
	PolishItemBackupURL string
	ChatUserQueryURL    string
	// UserCreditURL 是测试环境覆盖的公开个人主页信用查询接口；生产为空时使用官方网页端点。
	UserCreditURL string
	// AdjustPriceRenderURL 是测试环境覆盖的待付款改价表单接口；生产为空时使用官方网页版端点。
	AdjustPriceRenderURL string
	// AdjustPriceSubmitURL 是测试环境覆盖的待付款真实改价接口；生产为空时使用官方网页版端点。
	AdjustPriceSubmitURL string
	// CloseOrderReasonsURL 是测试环境覆盖的动态关闭原因接口；生产为空时使用官方网页版端点。
	CloseOrderReasonsURL string
	// CloseOrderSellerURL 是测试环境覆盖的卖家关单接口；生产为空时使用官方网页版端点。
	CloseOrderSellerURL string
	// RefundDetailURL 是测试环境覆盖的只读退款详情端点。
	RefundDetailURL string
	// RefundActionURL 是测试环境覆盖的退款动作端点；生产根据平台动态 API 构造固定官方域名。
	RefundActionURL string
	// MerchantRefundVerifyURL 是测试环境覆盖的 PC 支付验证地址接口。
	MerchantRefundVerifyURL string
	// MerchantRefundAgreeURL 是测试环境覆盖的 Merchant 同意退款接口。
	MerchantRefundAgreeURL string
	// MerchantRefundRefuseRenderURL 是测试环境覆盖的 Merchant 拒绝表单接口。
	MerchantRefundRefuseRenderURL string
	// MerchantRefundRefuseURL 是测试环境覆盖的 Merchant 拒绝退款接口。
	MerchantRefundRefuseURL string
}
...
type ItemListItem struct {
	...
	ItemStatus  int
	IsMultiSpec bool
	// SKUDetailLoaded 表示卖家编辑详情已成功返回；只有为 true 时同步层才允许更新或清空本地 SKU。
	SKUDetailLoaded bool
	// SKUSyncedAt 是卖家编辑详情成功解析的 Unix 秒时间。
	SKUSyncedAt int64
	// SKUs 保存卖家编辑详情返回的当前规格、售价和剩余库存。
	SKUs []ItemSKU
}
