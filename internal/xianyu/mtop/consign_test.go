...
// TestConsignSuccess 验证官方 data.orderId 明确成功后无需重试。
func TestConsignSuccess(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		requests.Add(1)
		fmt.Fprint(w, `{"ret":["SUCCESS::调用成功"],"data":{"orderId":"order-1"}}`)
	}))
	defer server.Close()
...
// TestConsignEvidenceUsesOfficialMerchantPayload 验证当前官方接口名、描述和 JSON 字符串图片数组共同参与签名与请求体。
func TestConsignEvidenceUsesOfficialMerchantPayload(t *testing.T) {
	// receivedData 保存测试服务收到的 data 表单值。
	var receivedData string
	// server 验证 query API 名并返回官方 data.orderId 成功形态。
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// api 是本次请求声明的 MTOP 方法名。
		if api := r.URL.Query().Get("api"); api != "mtop.taobao.idle.logistics.merchant.consign.dummy" {
			t.Fatalf("api=%q", api)
		}
		// parseErr 是 data 表单解析错误。
		if parseErr := r.ParseForm(); parseErr != nil {
			t.Fatal(parseErr)
		}
		receivedData = r.Form.Get("data")
		fmt.Fprint(w, `{"ret":["SUCCESS::调用成功"],"data":{"orderId":"5127372002248048713"}}`)
	}))
	defer server.Close()
	// client 将生产端点替换为本地服务器，禁止测试访问闲鱼。
	client := &ClientImpl{HTTPClient: server.Client(), ConsignURL: server.URL + "/"}
	// result、requestErr 是带描述和两张凭证的提交结果。
	result, requestErr := client.ConsignEvidenceContext(context.Background(), consignCookies, ConsignEvidenceRequest{
		OrderID: "5127372002248048713", TradeText: "在线交付完成", ImageURLs: []string{"https://img.example/one.png", "https://img.example/two.jpg"},
	})
	if requestErr != nil || result == nil || !result.Success {
		t.Fatalf("result=%+v err=%v", result, requestErr)
	}
	// expectedData 与官方卖家工作台 JSON.stringify(picList) 的字符串字段保持一致。
	expectedData := `{"orderId":"5127372002248048713","tradeText":"在线交付完成","picList":"[\"https://img.example/one.png\",\"https://img.example/two.jpg\"]","newUnconsign":true}`
	if receivedData != expectedData {
		t.Fatalf("data=%s want=%s", receivedData, expectedData)
	}
}
...
func TestBuildConsignQuery(t *testing.T) {
	...
	if !strings.Contains(q, "t=123") || !strings.Contains(q, "sign=SIGN") ||
		!strings.Contains(q, "api=mtop.taobao.idle.logistics.merchant.consign.dummy") ||
