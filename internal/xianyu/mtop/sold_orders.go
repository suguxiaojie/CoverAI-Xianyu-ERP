...
type SoldOrder struct {
	...
	// CreatedAt 是平台订单创建时间的 UTC RFC3339 文本；响应没有可靠时间时为空。
	CreatedAt string
}

var _ SoldOrderFetcher = (*ClientImpl)(nil)

// soldOrdersPageTimeout 限制单页订单发现请求，平台无响应时尽快结束任务并允许用户重试；测试可临时缩短。
var soldOrdersPageTimeout = 12 * time.Second
...
func (c *ClientImpl) FetchSoldOrdersPage(ctx context.Context, cookies string, pageNumber, pageSize int) (*SoldOrdersPage, error) {
	...
	// pageCtx、pageCancel 为单页发现建立独立上限，不能占满三十分钟后台任务预算。
	pageCtx, pageCancel := context.WithTimeout(ctx, soldOrdersPageTimeout)
	defer pageCancel()
	// req、err 用于本次流程后续判断的req、err
	req, err := http.NewRequestWithContext(pageCtx, http.MethodPost, endpoint+"?"+buildSoldOrdersQuery(timestamp, sign), strings.NewReader("data="+url.QueryEscape(dataVal)))
	if err != nil {
		return nil, err
	}
...
// soldOrderCreatedAt 按平台常见字段优先级提取订单创建时间，只接受可验证的时间格式。
func soldOrderCreatedAt(item, common map[string]any) string {
	// keys 是订单创建时间的已知字段别名，按语义精确度从高到低排列。
	keys := []string{"orderCreateTime", "orderTime", "createTime", "gmtCreate", "createdAt"}
	// containers 是可能承载订单时间的结构；不遍历买家或商品对象，避免误取其他实体时间。
	containers := []map[string]any{common}
	// containerKey 是当前待检查的订单时间子结构名称。
	for _, containerKey := range []string{"timeInfoVO", "orderInfoVO", "tradeInfoVO"} {
		// container、ok 是当前订单时间子结构及其类型断言结果。
		if container, ok := item[containerKey].(map[string]any); ok {
			containers = append(containers, container)
		}
	}
	containers = append(containers, item)
	// container 是当前待检查的订单级结构。
	for _, container := range containers {
		// key 是当前订单时间候选字段。
		for _, key := range keys {
			// normalized 是候选字段成功解析后的 UTC RFC3339 时间。
			if normalized := normalizeSoldOrderCreatedAt(container[key]); normalized != "" {
				return normalized
			}
		}
	}
...
// normalizeSoldOrderCreatedAt 把平台秒／毫秒时间戳或北京时间文本规范为 UTC RFC3339。
func normalizeSoldOrderCreatedAt(value any) string {
	// raw 是平台时间值的去空白文本；空值不覆盖本地已有订单时间。
	raw := strings.TrimSpace(mtopString(value))
	if raw == "" {
		return ""
	}
	// unixValue、parseErr 是数字时间戳及其解析错误。
	if unixValue, parseErr := strconv.ParseInt(raw, 10, 64); parseErr == nil {
		if len(raw) == 13 {
			return time.UnixMilli(unixValue).UTC().Format(time.RFC3339)
		}
		if len(raw) == 10 {
			return time.Unix(unixValue, 0).UTC().Format(time.RFC3339)
		}
	}
	// parsed、parseErr 是带显式时区的 RFC3339 时间及其解析错误。
	if parsed, parseErr := time.Parse(time.RFC3339, raw); parseErr == nil {
		return parsed.UTC().Format(time.RFC3339)
	}
	// platformLocation 是无时区平台文本使用的北京时间固定偏移，避免测试机本地时区改变结果。
	platformLocation := time.FixedZone("Asia/Shanghai", 8*60*60)
	// layout 是当前尝试的订单时间文本格式。
	for _, layout := range []string{"2006-01-02 15:04:05", "2006-01-02 15:04", "2006/01/02 15:04:05", "2006/01/02 15:04"} {
		// parsed、parseErr 是按北京时间解释的平台文本和解析错误。
		if parsed, parseErr := time.ParseInLocation(layout, raw, platformLocation); parseErr == nil {
			return parsed.UTC().Format(time.RFC3339)
		}
	}
	return ""
